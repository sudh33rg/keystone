...
function deliver(message: unknown): void {
  if (message && typeof message === 'object' && (message as { type?: string }).type === 'APPLICATION_STATE') {
...
if (!nativeApi && typeof EventSource !== 'undefined') {
  ...
  source.addEventListener('message', event => deliver(JSON.parse((event as MessageEvent<string>).data) as unknown));
...
async function postBrowserMessage(message: unknown): Promise<void> {
  ...
  if (response.status === 409) {
    ...
    deliver({ type: 'NOTIFICATION', level: 'error', message: 'The Browser View was refreshed because another surface changed Keystone state. Retry the action.' });
    return;
  }
  if (!response.ok) {
    const detail = await response.json().catch(() => ({})) as { error?: string };
    deliver({ type: 'NOTIFICATION', level: 'error', message: detail.error ?? `Browser View command failed (${response.status}).` });
    return;
  }
  const result = await response.json().catch(() => ({})) as { stateVersion?: number };
  if (Number.isSafeInteger(result.stateVersion) && Number(result.stateVersion) > lastStateVersion) lastStateVersion = Number(result.stateVersion);
}

export const vscode = {
  postMessage(message: unknown): void {
    if (nativeApi) { nativeApi.postMessage(message); return; }
    void postBrowserMessage(message);
  },
  onMessage(listener: Listener): () => void { listeners.add(listener); return () => listeners.delete(listener); },
