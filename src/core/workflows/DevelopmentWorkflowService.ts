...
export class DevelopmentWorkflowService {
  ...
  private projectWorkbench(workflow: DevelopmentWorkflowSnapshot): { stageStates: WorkbenchStageState[]; summary: WorkbenchWorkflowState["summary"]; diagnostics: WorkbenchDefineState["diagnostics"]; staleness: WorkbenchWorkflowState["staleness"] } {
    ...
    const stage = !specApproved ? "define" : !planApproved ? "plan" : !implementationComplete ? "build" : "validate";
    const states: WorkbenchStageState[] = [
      ...
      stageState("validate", implemented ? implementationComplete ? "current" : "ready" : "blocked", implemented ? [] : [{ code: "NO_IMPLEMENTATION_CHANGES", message: "Validation requires implementation progress or manually supplied changes.", recoveryAction: "Complete or record implementation work in Build." }], []),
      stageState("review", implementationComplete ? "ready" : "blocked", implementationComplete ? [] : [{ code: "VALIDATION_INCOMPLETE", message: "Review is unavailable until required implementation and validation are complete.", recoveryAction: "Complete required tasks and validation first." }], []),
      stageState("complete", implementationComplete ? "optional" : "unavailable", implementationComplete ? [] : [{ code: "WORKFLOW_INCOMPLETE", message: "Completion actions are unavailable while required work remains.", recoveryAction: "Finish required workflow tasks and review." }], ["Git, PR, export, and handoff actions are optional and capability-driven."])
