...
import type { Phase7ChangeSet, Phase7ChangedEntity, Phase7CoverageGap, Phase7ImpactAnalysis, Phase7ImpactItem, Phase7TestMapping } from "../../shared/contracts/impactQa";

export class Phase7ImpactAnalysisService {
  constructor(private readonly snapshot: IntelligenceSnapshot, private readonly now = () => new Date().toISOString()) {}
  analyze(input: { workflowId: string; changeSet: Phase7ChangeSet; changedEntities: Phase7ChangedEntity[]; intelligenceRevision: string; depth: number; maxNodes: number; maxEdges: number }): Phase7ImpactAnalysis {
    ...
    const impacts: Phase7ImpactItem[] = [];
    ...
    const gaps: Phase7CoverageGap[] = roots.filter((id) => !covered.has(id)).map((id) => ({ id: `gap:${id}`, entityId: id, reason: "Changed production entity has no evidence-backed mapped test.", confidence: 1, recommendedTestLayer: "unit", blocking: publicEntity(this.snapshot, id) }));
    ...
    const riskFactors: Phase7ImpactAnalysis["risk"]["factors"] = [];
