...
import type { QaChangeSet, QaChangedFile } from "../../shared/contracts/impactQa";

export interface DetectedWorkspaceChanges { source: "git" | "workspace-snapshot"; baseRevision?: string; headRevision?: string; files: Array<Omit<QaChangedFile, "source">> }
...
export class WorkspaceChangeSetService {
  ...
  async detect(input: { workflowId: string; developmentResultId?: string; workspaceFiles: Set<string>; manualFiles?: string[] }): Promise<QaChangeSet> {
    const detected = await this.provider.detect();
    const raw: Array<Omit<QaChangedFile, "source">> = detected?.files ?? (input.manualFiles ?? []).map((path) => ({ path, changeType: "unknown" as const }));
    ...
    const byPath = new Map<string, QaChangedFile>();
  ...
  freshness(saved: QaChangeSet, currentHash: string): { stale: boolean; reason?: string } { return saved.contentHash === currentHash ? { stale: false } : { stale: true, reason: "Workspace changes differ from the accepted change-set snapshot." }; }
}
...
function mergeRanges(left: QaChangedFile["changedRanges"], right: QaChangedFile["changedRanges"]): QaChangedFile["changedRanges"] { const values = [...(left ?? []), ...(right ?? [])]; return [...new Map(values.map((item) => [JSON.stringify(item), item])).values()]; }
