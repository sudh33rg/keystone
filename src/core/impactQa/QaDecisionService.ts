...
import type { Phase7QaDecision } from "../../shared/contracts/impactQa";

export class QaDecisionService {
  decide(input: { workflowId: string; qaPlanId: string; qaExecutionId?: string; planCurrent: boolean; impactCurrent: boolean; requiredExecuted: boolean; requiredPassed: boolean; executionSucceeded: boolean; blockingGapIds: string[]; warningGapIds: string[]; optionalSkipped: boolean; cancelled: boolean; evidenceIds: string[] }): Phase7QaDecision {
    ...
    let decision: Phase7QaDecision["decision"]; if (input.cancelled) decision = "cancelled"; else if (!input.planCurrent || !input.impactCurrent || !input.requiredExecuted) decision = "incomplete"; else if (!input.requiredPassed || !input.executionSucceeded || input.blockingGapIds.length) decision = "failed"; else if (input.optionalSkipped || input.warningGapIds.length) decision = "passed-with-warnings"; else decision = "passed";
