...
export async function enhanceIntent(input: EnhanceIntentInput): Promise<EnhancementSession> {
  ...
  const turns = [
    ...
    { role: "user" as const, content: input.text.trim(), timestamp: now }
  ];
  ...
  if (input.mode === "auto") {
    ...
    const safeToProceed =
      ...
      proposal.goal.trim().length > 5 &&
      proposal.acceptanceCriteria.length > 0;
    proposal = {
      ...proposal,
      status: safeToProceed ? "ready" : "needs-input",
      questions: safeToProceed
        ? []
        : [
            grounded
              ? "Confirm the intended observable result before automatic analysis continues."
              : "Identify the repository area or behavior this should affect."
          ],
      assumptions: proposal.assumptions.length
        ? proposal.assumptions
        : [
            "Use existing repository patterns and preserve public behavior unless the intent explicitly changes it."
          ]
    };
  }
  const assistantContent =
    proposal.status === "ready" ? proposal.enhancedIntent : proposal.questions.join("\n");
  ...
  return {
    ...
    mode: input.mode,
    originalIntent,
    enhancedIntent: proposal.enhancedIntent,
    status: proposal.status,
    confidence: proposal.confidence,
    questions: proposal.questions.slice(0, 3),
    assumptions: proposal.assumptions.slice(0, 8),
    evidence,
    goal: proposal.goal,
    acceptanceCriteria: proposal.acceptanceCriteria.slice(0, 8),
    constraints: proposal.constraints.slice(0, 8),
    unresolvedGaps,
    qualityScore,
    modelUsed,
    diagnostics,
    turns: [...turns, { role: "assistant", content: assistantContent, timestamp: now }],
    updatedAt: now
  };
}
...
function ambiguityOf(text: string): {
  vague: boolean;
  missingAction: boolean;
  missingTarget: boolean;
} {
  const tokens = text.toLowerCase().match(/[a-z0-9_./-]+/g) ?? [];
  const missingAction = !tokens.some((token) =>
    /^(add|build|create|fix|change|update|remove|refactor|test|explain|review|migrate|improve|implement)$/.test(
      token
    )
  );
  const missingTarget = !tokens.some(
    (token) =>
      token.includes("/") ||
      /(?:api|ui|page|service|class|function|test|auth|database|component|extension|prompt|context)/.test(
        token
      )
  );
  return {
    vague: tokens.length < 5 || missingAction || missingTarget,
    missingAction,
    missingTarget
  };
}
...
function fallbackProposal(
  ...
  evidence: string[],
  mode: EnhancementMode,
  ambiguity: ReturnType<typeof ambiguityOf>
): Proposal {
  ...
  const context = evidence
    .slice(0, 4)
    .map((item) => item.split(" (")[0].replace(/^active:/, ""))
    .join(", ");
  const enhancedIntent = [
    original,
    ...followups,
    context ? `Use repository evidence from ${context}.` : "",
    "Preserve existing behavior outside the requested scope and validate the affected tests."
  ]
    .filter(Boolean)
    .join(" ");
  const questions: string[] = [];
  if (ambiguity.missingAction)
    questions.push("What should change: add, fix, refactor, test, or explain?");
  if (ambiguity.missingTarget)
    questions.push(
      `Which behavior or area is the target${context ? ` (likely: ${context})` : ""}?`
    );
  if (ambiguity.vague && questions.length < 3)
    questions.push("What observable result will confirm this is complete?");
  return {
    enhancedIntent,
    status:
      mode === "manual" && ambiguity.vague && followups.length === 0 ? "needs-input" : "ready",
    confidence: ambiguity.vague ? (mode === "auto" && evidence.length ? 0.65 : 0.55) : 0.82,
    questions,
    assumptions: [],
    goal: original,
    acceptanceCriteria: [
      "The requested behavior is observable and covered by the smallest relevant validation set."
    ],
    constraints: [
