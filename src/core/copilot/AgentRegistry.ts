...
export class CopilotAgentDiscoveryService {
  ...
  discover(signal?: AbortSignal): Promise<CopilotAgentDescriptor[]> {
    return this.adapter.discoverAgents(signal);
  }
  ...
  parseConfigured(
    ...
    return value.slice(0, 100).flatMap((raw) => {
      ...
      const parsed = CopilotAgentDescriptorSchema.safeParse({
        ...
        taskCategories: candidate.taskCategories ?? [],
        invocationMethod: candidate.invocationMethod,
        ...
        evidence: [
          {
            ...
            source,
            statement:
              "Loaded as inert, schema-validated configuration; no code or commands were executed.",
          },
        ],
...
export class AgentProfileService {
  merge(
    discovered: CopilotAgentDescriptor[],
    configured: CopilotAgentDescriptor[],
  ): CopilotAgentDescriptor[] {
    ...
    for (const agent of [...result.values()])
      if (agent.source === "user-alias" && agent.aliasFor) {
        ...
        result.set(agent.id, {
          ...
          capabilities: agent.capabilities.length
            ? agent.capabilities
            : (target?.capabilities ?? []),
          taskCategories: agent.taskCategories.length
            ? agent.taskCategories
            : (target?.taskCategories ?? []),
          invocationMethod: target?.invocationMethod,
          evidence: [
            ...agent.evidence,
            ...(target
              ? [
...
export const AGENT_RECOMMENDATION_WEIGHTS = Object.freeze({
  available: 100,
  unknown: 10,
  capability: 45,
  category: 30,
  ...
  priorSelection: 20,
  restriction: -60,
  missingCapability: -100,
});

export class AgentRecommendationService {
  recommend(
    task: DevelopmentTask,
    agents: CopilotAgentDescriptor[],
    mode: AgentSelectionMode = "recommended",
    ...
    const candidates = agents
      .map((agent) => {
        const matching = task.requiredCapabilities.filter((item) =>
          agent.capabilities.includes(item),
        );
        const missing = task.requiredCapabilities.filter(
          (item) => !agent.capabilities.includes(item),
        );
        const blocking = agent.restrictions.filter((item) => item.blocking);
        let score =
          agent.availability === "available"
            ? AGENT_RECOMMENDATION_WEIGHTS.available
            : agent.availability === "unknown"
              ? AGENT_RECOMMENDATION_WEIGHTS.unknown
              : -1000;
        score +=
          matching.length * AGENT_RECOMMENDATION_WEIGHTS.capability +
          (agent.taskCategories.includes(task.category)
            ? AGENT_RECOMMENDATION_WEIGHTS.category
            : 0) +
  ...
  async select(
    ...
    if (this.persistence)
      await this.persistence.update((state) => ({
        ...state,
        selections: { ...state.selections, [taskId]: agent.id },
      }));
    return agent.id;
  }
}
...
export class CopilotAgentRegistry {
  ...
  constructor(
    ...
    this.discovery = new CopilotAgentDiscoveryService(adapter);
    this.selections = new AgentSelectionService(persistence);
  }

  setConfiguredProfiles(configured: CopilotAgentDescriptor[]): void {
    this.configured = structuredClone(configured).slice(0, 200);
  }
  setSelectionRules(rules: AgentSelectionRule[]): void {
    this.rules = structuredClone(rules).slice(0, 100);
  }
  async refresh(
    configured: CopilotAgentDescriptor[] = this.configured,
    signal?: AbortSignal,
  ): Promise<CopilotAgentDescriptor[]> {
    this.setConfiguredProfiles(configured);
    const discovered = await this.discovery.discover(signal);
    this.agents = this.profiles.merge(discovered, this.configured);
    if (this.persistence)
      await this.persistence.update((state) => ({
        ...state,
        agents: this.agents,
  ...
  restore(): void {
    this.agents = this.persistence?.snapshot.agents ?? [];
  }
  getProfiles(): CopilotAgentDescriptor[] {
    return structuredClone(this.agents);
  }
  getProfile(agentId: string): CopilotAgentDescriptor | undefined {
    return this.agents.find((item) => item.id === agentId);
  }
  async addUnverifiedAlias(label: string): Promise<CopilotAgentDescriptor> {
    const normalized = label.trim();
    if (!normalized) throw new Error("An intended agent label is required.");
    const id = `user-alias:${
      normalized
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
    ...
    const agent = CopilotAgentDescriptorSchema.parse({
      ...
      description:
        "User-entered intended agent label; runtime availability and direct invocation are unverified.",
      source: "user-alias",
      availability: "unknown",
      capabilities: [],
      taskCategories: [],
      restrictions: [
        {
          kind: "invocation",
          description:
            "Keystone cannot verify or directly invoke this intended agent. Use supported assisted or clipboard delegation.",
          blocking: false,
        },
      ],
      confidence: 0.25,
      evidence: [
        {
          ...
          statement: "The user explicitly entered this intended agent label.",
        },
      ],
    });
    this.agents = this.profiles.merge(this.agents, [agent]);
    if (this.persistence)
      await this.persistence.update((state) => ({
        ...state,
        agents: this.agents,
  ...
  recommend(
    ...
    rules: AgentSelectionRule[] = this.rules,
  ): AgentRecommendation {
