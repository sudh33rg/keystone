...
import type { IntelligenceFamilySummary, IntelligenceStageResult } from '../../intelligence/pipeline';
...
export type WebviewToExtensionMessage =
  ...
  | { type: 'RECORD_CONTEXT_FEEDBACK'; intent: string; path?: string; rating: 'useful' | 'irrelevant' | 'helpful' | 'unhelpful' }
  ...
  | { type: 'APPROVE_DELEGATION'; mode: string; prompt: string; storyId?: string; agent?: string; skills?: string[]; instructions?: string[]; contextPackId?: string }
  ...
  | { type: 'RESTORE_TASK_HANDOFF'; packageText: string; passphrase: string; manualSyncConfirmed: boolean }
...
export type ExtensionToWebviewMessage =
  ...
  | { type: 'CONTEXT_ORIGINAL_RESULT'; path: string; content: string; truncated: boolean; changed: boolean; currentHash: string }
  | { type: 'VALIDATION_RESULT'; results: ValidationRunResult[] }
  | { type: 'QA_BACKGROUND_STATUS'; status: 'running' | 'complete' | 'cancelled' | 'failed'; message?: string; progress?: number; result?: GapAnalysisResult }
...
export interface WorkspaceSummary {
  ...
  languageCapabilities?: Array<{ id: string; label: string; level: string; extensions: readonly string[]; files?: number; baseline?: string; semanticProvider?: string; semanticFiles?: number; deterministicFiles?: number; failedSemanticFiles?: number; capabilities?: object; warnings?: readonly string[] }>;
  universalTextFiles?: number;
  okf?: { profile: string; version: string; extractionRunId: string; units: number; relationships: number; observations: number; evidence: number; active: number; deleted: number; graphNodes: number; graphEdges: number; cpgBindings: number; validated: boolean; portableBundle?: { path: string; conceptFiles: number; validated: boolean; profile: string; generatedAt: string }; evidenceSamples: Array<{ id: string; path: string; method: string; observedAt: string }> };
}
...
export interface KeystoneWebviewState {
  ...
  ingestion?: { active: boolean; progress: number; stage: string; message: string; persistedPath: string };
...
export interface KeystoneTaskResult {
  ...
  textKeywords?: string[];
  confidence?: number;
  confidenceDetails?: { overall: number; signals: Array<{ name: string; score: number; weight: number }> };
  route: string;
  reason: string;
  ...
  prMarkdown: string;
  detailedRisks: Record<'architectureImpact' | 'securityRisk' | 'performanceRisk' | 'testGaps' | 'dependencyChanges', { area: string; level: 'low' | 'medium' | 'high'; detail: string }>;
  ...
  contextSections?: Array<{ path: string; reason: string; preview: string; estimatedTokens: number; sourceHash?: string; score?: number; evidence?: Array<{ okfId?: string; kind: string; label: string; startLine?: number; endLine?: number }> }>;
