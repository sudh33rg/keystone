...
import type { FetchLike, FetchLikeResponse, ValueEdgeConnection, ValueEdgeFeature, ValueEdgePublishResult } from './types';
...
export class ValueEdgeClient {
  ...
  constructor(
    ...
    if (!/^https?:\/\//i.test(this.baseUrl)) throw new Error('ValueEdge base URL must use HTTP or HTTPS.');
    if (!connection.sharedSpaceId.trim() || !connection.workspaceId.trim() || !connection.clientId.trim() || !clientSecret.trim()) {
      throw new Error('ValueEdge base URL, shared space, workspace, client ID, and client secret are required.');
    }
  }

  async fetchFeature(featureId: string): Promise<ValueEdgeFeature> {
    return this.withSession(async () => {
      const raw = await this.requestJson(`${this.contextPath}/features/${encodeURIComponent(featureId)}?fields=id,name,description,phase,release,sprint,owner`);
  ...
  async publishBacklogStories(featureId: string, stories: readonly SDLCBacklogStory[]): Promise<ValueEdgePublishResult[]> {
    ...
    return this.withSession(async () => {
      ...
      for (const story of publishable) {
        ...
        results.push({ localId: story.id, externalId: stringValue(created.id, ''), kind: story.kind, title: story.title });
      }
      return results;
    });
  }
  ...
  private async signIn(): Promise<void> {
    const response = await this.fetchImpl(`${this.baseUrl}/authentication/sign_in`, {
      ...
      body: JSON.stringify({ client_id: this.connection.clientId, client_secret: this.clientSecret }),
    });
    await assertOk(response, 'ValueEdge sign-in');
    const cookies = response.headers.getSetCookie?.() ?? splitSetCookie(response.headers.get('set-cookie'));
    this.cookie = cookies.map(value => value.split(';', 1)[0]).filter(Boolean).join('; ');
  }

  private async signOut(): Promise<void> {
    try { await this.fetchImpl(`${this.baseUrl}/authentication/sign_out`, { method: 'POST', headers: this.cookie ? { cookie: this.cookie } : undefined }); }
    finally { this.cookie = ''; }
  }
...
async function assertOk(response: FetchLikeResponse, operation: string): Promise<void> {
  ...
  throw new Error(`${operation} failed (${response.status} ${response.statusText})${body ? `: ${body.slice(0, 500)}` : ''}`);
}
...
function stringValue(value: unknown, fallback: string): string { return typeof value === 'string' && value.trim() ? value : typeof value === 'number' ? String(value) : fallback; }
function optionalString(value: unknown): string | undefined { const result = stringValue(value, ''); return result || undefined; }
function referenceLabel(value: unknown): string | undefined { const object = asObject(value); return optionalString(object.name ?? object.label ?? value); }
function splitSetCookie(value: string | null): string[] { return value ? value.split(/,(?=\s*[^;,]+=)/) : []; }
