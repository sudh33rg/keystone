...
export class ValueEdgeClient {
  ...
  async testConnection(): Promise<{ workspaceUrl: string }> {
    return this.withSession(async () => {
      await this.requestJson(`${this.contextPath}?fields=id,name`);
      return { workspaceUrl: this.contextPath };
    });
  }
...
function formatDescription(story: SDLCBacklogStory): string {
  return `${story.description}${story.storyPoints ? `\n\nStory points: ${story.storyPoints}` : ""}\n\nAcceptance criteria:\n${story.acceptanceCriteria.map((item) => `- ${item}`).join("\n")}\n\nGenerated by Keystone StoryForge Discovery from approved repository intelligence.`;
}
