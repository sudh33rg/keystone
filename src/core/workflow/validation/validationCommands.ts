...
export async function detectValidationCommands(root: string): Promise<DetectedValidationCommands> {
  ...
  const hasRootSuffix = (suffix: string): boolean =>
    rootEntries.some((entry) => entry.endsWith(suffix));

  const node = await readJson<{ scripts?: Record<string, unknown>; packageManager?: string }>(
  ...
  if (node) {
    ...
    const command = (name: string): string =>
      manager === "npm" ? `npm run ${name}` : `${manager} run ${name}`;
    const available = (name: string): boolean =>
      typeof scripts[name] === "string" && Boolean(String(scripts[name]).trim());
    const all = ["typecheck", "check", "lint", "test"]
      .filter(
        (name, index, values) =>
          available(name) &&
          (name !== "check" || !available("typecheck")) &&
          values.indexOf(name) === index
      )
      .map(command);
    const impacted = available("test:changed")
  ...
  if (
    (await anyExists(root, ["pyproject.toml", "pytest.ini", "setup.cfg", "tox.ini"])) ||
    (await directoryExists(root, "tests"))
  ) {
    if (await looksLikePython(root, rootEntries))
      sets.push({ ecosystem: "python", all: ["python -m pytest"], impacted: ["python -m pytest"] });
  }
  if (await exists(root, "go.mod"))
    sets.push({ ecosystem: "go", all: ["go test ./..."], impacted: ["go test ./..."] });
  if (await exists(root, "Cargo.toml"))
    sets.push({ ecosystem: "rust", all: ["cargo test"], impacted: ["cargo test"] });

  if (
    hasRootSuffix(".sln") ||
    hasRootSuffix(".csproj") ||
    hasRootSuffix(".fsproj") ||
    hasRootSuffix(".vbproj")
  ) {
    sets.push({ ecosystem: "dotnet", all: ["dotnet test"], impacted: ["dotnet test"] });
  }

  if (await exists(root, "gradlew"))
    sets.push({ ecosystem: "gradle", all: ["./gradlew test"], impacted: ["./gradlew test"] });
  else if (
    ...
    sets.push({ ecosystem: "gradle", all: ["gradle test"], impacted: ["gradle test"] });
  if (await exists(root, "pom.xml"))
    sets.push({ ecosystem: "maven", all: ["mvn test"], impacted: ["mvn test"] });
  ...
  if (composer) {
    const hasComposerTest = Boolean(
      composer.scripts && typeof composer.scripts.test !== "undefined"
    );
    const hasPhpUnit =
      (await anyExists(root, ["phpunit.xml", "phpunit.xml.dist", "vendor/bin/phpunit"])) ||
      Boolean(composer["require-dev"]?.["phpunit/phpunit"]);
    if (hasComposerTest)
      sets.push({ ecosystem: "php:composer", all: ["composer test"], impacted: ["composer test"] });
    else if (hasPhpUnit)
      sets.push({
        ecosystem: "php:phpunit",
        all: ["vendor/bin/phpunit"],
        impacted: ["vendor/bin/phpunit"]
      });
  }

  if (await exists(root, "Gemfile")) {
    if ((await directoryExists(root, "spec")) || (await exists(root, ".rspec")))
      sets.push({
        ecosystem: "ruby:rspec",
        all: ["bundle exec rspec"],
        impacted: ["bundle exec rspec"]
      });
    else if ((await directoryExists(root, "test")) || (await exists(root, "Rakefile")))
      sets.push({
        ecosystem: "ruby:rake",
        all: ["bundle exec rake test"],
        impacted: ["bundle exec rake test"]
      });
  }

  if (await exists(root, "Package.swift"))
    sets.push({ ecosystem: "swift", all: ["swift test"], impacted: ["swift test"] });
  if (await exists(root, "build.sbt"))
    sets.push({ ecosystem: "scala:sbt", all: ["sbt test"], impacted: ["sbt test"] });
  if (await exists(root, "pubspec.yaml"))
    sets.push({ ecosystem: "dart", all: ["dart test"], impacted: ["dart test"] });
  if (await exists(root, "mix.exs"))
    sets.push({ ecosystem: "elixir", all: ["mix test"], impacted: ["mix test"] });
  if (await exists(root, "rebar.config"))
    sets.push({ ecosystem: "erlang:rebar3", all: ["rebar3 eunit"], impacted: ["rebar3 eunit"] });

  if (await exists(root, "stack.yaml"))
    sets.push({ ecosystem: "haskell:stack", all: ["stack test"], impacted: ["stack test"] });
  else if (hasRootSuffix(".cabal") || (await exists(root, "cabal.project")))
    sets.push({
      ecosystem: "haskell:cabal",
      all: ["cabal test all"],
      impacted: ["cabal test all"]
    });

  if ((await exists(root, "Project.toml")) && (await looksLikeJuliaProject(root))) {
    sets.push({
      ecosystem: "julia",
      all: ['julia --project -e "using Pkg; Pkg.test()"'],
      impacted: ['julia --project -e "using Pkg; Pkg.test()"']
    });
  }

  if (
    ((await exists(root, "Makefile.PL")) || (await exists(root, "Build.PL"))) &&
    (await directoryExists(root, "t"))
  ) {
  ...
  if (await exists(root, "DESCRIPTION")) {
    sets.push({
      ecosystem: "r",
      all: ["R CMD check --no-manual ."],
      impacted: ["R CMD check --no-manual ."]
    });
  }
  ...
  if ((await exists(root, "CMakeLists.txt")) && (await exists(root, "build/CTestTestfile.cmake"))) {
    sets.push({
      ...
      impacted: ["ctest --test-dir build --output-on-failure"]
    });
  ...
  if (await containsFileWithSuffix(path.join(root, "tests"), ".Tests.ps1")) {
    sets.push({
      ecosystem: "powershell:pester",
      all: ['pwsh -NoProfile -Command "Invoke-Pester -CI"'],
      impacted: ['pwsh -NoProfile -Command "Invoke-Pester -CI"']
    });
  }
  if (
    (await containsFileWithSuffix(path.join(root, "test"), ".bats")) ||
    (await containsFileWithSuffix(path.join(root, "tests"), ".bats"))
  ) {
  ...
  return {
    all,
    impacted,
    ecosystem: sets.length ? sets.map((set) => set.ecosystem).join("+") : "unknown"
  };
}

async function nodePackageManager(
  root: string,
  declared?: string
...
async function directoryExists(root: string, name: string): Promise<boolean> {
  try {
    return (await fs.stat(path.join(root, name))).isDirectory();
  } catch {
    return false;
  }
}
