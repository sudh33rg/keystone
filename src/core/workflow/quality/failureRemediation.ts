...
export type FailureRemediationKind =
  "product-defect" | "test-defect" | "flaky-test" | "environment" | "unknown";
...
export function planFailureRemediation(input: FailureRemediationInput): FailureRemediationProposal {
  const classification = classifyFailure({
    ...
    failureStackTrace: input.failureStackTrace,
  ...
  const evidence = [
    ...
    input.failureStackTrace ?? "No stack trace was provided."
  ].filter(Boolean);
  return {
...
function actionsFor(kind: FailureRemediationKind): string[] {
  if (kind === "product-defect")
    return [
      "Reproduce the product defect independently of the test.",
      "Trace the affected implementation and impacted tests.",
      "Prepare the smallest behavior-preserving fix and rerun affected validation."
    ];
  if (kind === "test-defect")
    return [
      "Confirm the approved behavior and current public contract.",
      "Update the test only when its expectation or selector is demonstrably obsolete.",
      "Preserve regression coverage and add evidence linking the test to the acceptance criterion."
    ];
  if (kind === "flaky-test")
    return [
      "Collect repeated-run evidence and isolate timing, shared-state, ordering, or network causes.",
      "Prefer deterministic synchronization over retries or increased timeouts.",
      "Quarantine only through an explicit, time-bounded user decision."
    ];
  if (kind === "environment")
    return [
      "Capture toolchain, dependency, service, and environment differences.",
      "Fix reproducibility before changing product or test behavior.",
      "Rerun the unchanged test in the corrected environment."
    ];
  return [
    "Collect a minimal reproduction and complete failure evidence.",
    "Classify the failure before proposing code or test changes.",
    "Require user review before delegation."
  ];
}
