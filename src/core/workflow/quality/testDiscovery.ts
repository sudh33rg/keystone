...
const TEST_PATTERNS: Record<TestFrameworkName, string[]> = {
  ...
  xunit: ["**/*Test.cs"],
  cypress: ["cypress/e2e/**/*.cy.{ts,js}", "cypress/integration/**/*.spec.{ts,js}"],
  playwright: ["tests/**/*.spec.{ts,js}", "e2e/**/*.spec.{ts,js}"],
  unknown: []
};
 ...
 */
function detectFromPackageJson(pkg: Record<string, unknown>): TestFrameworkName {
