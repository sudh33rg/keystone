...
export type SDLCStoryType =
  | 'research' | 'specification' | 'design' | 'development'
  | 'existing-test-analysis' | 'new-test-creation' | 'test-impact-analysis'
  | 'failed-test-investigation' | 'flaky-test-analysis'
  | 'security-review' | 'performance-review' | 'modernization-review'
  | 'code-review' | 'pr-review' | 'documentation' | 'completion';

export type SDLCStoryStatus =
  | 'draft' | 'ready' | 'in-progress' | 'awaiting-delegation-approval'
  | 'delegated' | 'awaiting-validation' | 'review-required' | 'completed'
  | 'blocked' | 'paused' | 'cancelled' | 'superseded' | 'handed-off';
...
const transitions: Record<SDLCStoryStatus, readonly SDLCStoryStatus[]> = {
  ...
  'in-progress': ['awaiting-delegation-approval', 'awaiting-validation', 'review-required', 'paused', 'blocked', 'cancelled', 'handed-off'],
...
const criteria: Record<SDLCStoryType, readonly string[]> = {
  research: ['Relevant OKF evidence is linked', 'Affected architecture, flows, tests, risks, and constraints are identified'],
  specification: ['Functional and non-functional behavior is documented', 'Acceptance criteria are explicit and user-approved'],
  design: ['Implementation boundaries and dependencies are documented', 'The design respects repository conventions and read-only Git policy'],
  development: ['Implementation addresses the approved specification', 'Changed files and symbols are recorded'],
  'existing-test-analysis': ['Relevant existing tests are identified', 'Coverage relationships and gaps are recorded'],
  'test-impact-analysis': ['Impacted tests are selected from change and graph evidence', 'Regression risk is documented'],
  'new-test-creation': ['Missing tests are implemented or explicitly deferred with reason', 'New tests map to acceptance criteria'],
  'failed-test-investigation': ['Every observed failure is classified', 'Real defects, environment failures, and test defects are distinguished'],
  'flaky-test-analysis': ['Flakiness evidence is recorded', 'Quarantine or stabilization guidance is documented without silent mutation'],
  'security-review': ['Security-sensitive paths and trust boundaries are reviewed', 'Findings are resolved, accepted, or blocked with evidence'],
  'performance-review': ['Performance-sensitive paths are reviewed', 'Findings are resolved, accepted, or blocked with evidence'],
  'modernization-review': ['Modernization impact is assessed', 'Any recommendation preserves behavior and remains user-approved'],
  'code-review': ['Implementation is checked against specification and architecture', 'QA, security, and performance evidence is reviewed'],
  'pr-review': ['Read-only diff review content is prepared', 'No remote pull/merge-request mutation was performed'],
  documentation: ['User-facing and engineering documentation impact is resolved', 'Documentation matches implemented behavior'],
  completion: ['All dependent stories are completed', 'No unresolved blocker remains and the exact outcome is recorded'],
};
...
const definitions: readonly Definition[] = [
  ...
  ['flaky-test-analysis', 'Analyze flaky tests', ['existing-test-analysis', 'test-impact-analysis']],
  ...
  ['code-review', 'Review implementation', ['new-test-creation', 'failed-test-investigation', 'flaky-test-analysis', 'security-review', 'performance-review', 'modernization-review']],
...
export class SDLCEngine {
  createPlan(intent: string, context: SDLCPlanningContext = {}): SDLCPlan {
    ...
    const stories = definitions.map(([type, title, dependencies]): SDLCStory => ({
      id: ids.get(type)!, intentId, type, title, objective: title,
  ...
  approveSpecification(plan: SDLCPlan): SDLCPlan {
    ...
    if (story.status !== 'ready' && story.status !== 'in-progress') throw new Error('Research must be completed before specification approval.');
    const now = new Date().toISOString();
    const updated = this.updateStory(plan, story.id, {
      ...story, status: 'completed', satisfiedCriteria: [...story.acceptanceCriteria],
      evidence: unique([...story.evidence, 'User approved the specification in Keystone.']),
      decisions: unique([...story.decisions, 'Specification approved by user']), updatedAt: now,
    });
    return this.unlock({ ...updated, specificationStatus: 'approved', backlogStories: updated.backlogStories.map(story => story.status === 'draft' ? { ...story, status: 'approved' as const } : story), updatedAt: now });
  }

  rejectSpecification(plan: SDLCPlan, reason = 'Specification rejected by user.'): SDLCPlan {
    const story = this.story(plan, 'specification');
    const now = new Date().toISOString();
    return { ...this.updateStory(plan, story.id, { ...story, status: 'review-required', decisions: unique([...story.decisions, reason]), updatedAt: now }), specificationStatus: 'rejected', updatedAt: now };
  }

  transition(plan: SDLCPlan, storyId: string, next: SDLCStoryStatus, update: SDLCStoryUpdate = {}): SDLCPlan {
    const story = this.storyById(plan, storyId);
    if (story.type === 'specification' && next === 'completed') throw new Error('Use the explicit specification approval action.');
    if (!transitions[story.status].includes(next)) throw new Error(`Invalid SDLC transition ${story.status} → ${next}`);
  ...
  recordEvidence(plan: SDLCPlan, storyId: string, evidence: readonly string[], satisfiedCriteria: readonly string[] = []): SDLCPlan {
    const story = this.storyById(plan, storyId);
    return this.updateStory(plan, storyId, { ...story, evidence: unique([...story.evidence, ...evidence]), satisfiedCriteria: unique([...story.satisfiedCriteria, ...satisfiedCriteria]), updatedAt: new Date().toISOString() });
  }

  prepareDelegation(plan: SDLCPlan, storyId: string, input: { agent: string; skills?: readonly string[]; instructions?: readonly string[]; prompt: string; contextPackId?: string }): SDLCPlan {
    const story = this.storyById(plan, storyId);
    if (story.status !== 'in-progress') throw new Error('Delegation can only be prepared for an in-progress story.');
    const now = new Date().toISOString();
    const delegation: SDLCDelegation = {
      id: randomUUID(), status: 'pending-approval', agent: input.agent.trim() || 'GitHub Copilot',
      skills: unique(input.skills ?? []), instructions: unique(input.instructions ?? []),
      promptHash: createHash('sha256').update(input.prompt).digest('hex'),
    };
    return this.updateStory(plan, storyId, { ...story, status: 'awaiting-delegation-approval', contextPackId: input.contextPackId, delegation, updatedAt: now });
  }

  approveDelegation(plan: SDLCPlan, storyId: string): SDLCPlan {
    const story = this.storyById(plan, storyId);
    if (story.status !== 'awaiting-delegation-approval' || !story.delegation) throw new Error('There is no delegation awaiting approval.');
    const now = new Date().toISOString();
    return this.updateStory(plan, storyId, { ...story, status: 'delegated', delegation: { ...story.delegation, status: 'delegated', approvedAt: now, delegatedAt: now }, decisions: unique([...story.decisions, 'Delegation approved by user']), updatedAt: now });
  }

  completeDelegation(plan: SDLCPlan, storyId: string, evidence: readonly string[] = []): SDLCPlan {
    const story = this.storyById(plan, storyId);
    if (story.status !== 'delegated' || !story.delegation) throw new Error('The story is not delegated.');
    const now = new Date().toISOString();
    return this.updateStory(plan, storyId, { ...story, status: 'awaiting-validation', delegation: { ...story.delegation, status: 'completed', completedAt: now }, evidence: unique([...story.evidence, ...evidence]), updatedAt: now });
  }

  recordValidation(plan: SDLCPlan, storyId: string, input: { status: 'passed' | 'failed'; commands: readonly string[]; evidence: readonly string[] }): SDLCPlan {
    const story = this.storyById(plan, storyId);
    const now = new Date().toISOString();
    const run: SDLCValidationRun = { id: randomUUID(), status: input.status, commands: unique(input.commands), evidence: unique(input.evidence), completedAt: now };
    const status: SDLCStoryStatus = input.status === 'failed' ? 'review-required' : story.status;
    return this.updateStory(plan, storyId, { ...story, status, validationRuns: [...story.validationRuns, run], evidence: unique([...story.evidence, ...run.evidence]), updatedAt: now });
  }

  recordFinding(plan: SDLCPlan, storyId: string, input: Omit<SDLCFinding, 'id' | 'recordedAt'>): SDLCPlan {
    const story = this.storyById(plan, storyId);
    const finding: SDLCFinding = { ...input, id: randomUUID(), evidence: unique(input.evidence), recordedAt: new Date().toISOString() };
    return this.updateStory(plan, storyId, { ...story, findings: [...story.findings, finding], updatedAt: finding.recordedAt });
  }

  resolveFinding(plan: SDLCPlan, storyId: string, findingId: string, status: 'accepted' | 'resolved'): SDLCPlan {
    const story = this.storyById(plan, storyId);
    if (!story.findings.some(finding => finding.id === findingId)) throw new Error(`Unknown SDLC finding: ${findingId}`);
    const now = new Date().toISOString();
    return this.updateStory(plan, storyId, { ...story, findings: story.findings.map(finding => finding.id === findingId ? { ...finding, status, resolvedAt: now } : finding), updatedAt: now });
  }

  isComplete(plan: SDLCPlan): boolean { return plan.stories.every(story => story.status === 'completed'); }

  private assertDependencies(plan: SDLCPlan, story: SDLCStory): void {
    const incomplete = story.dependencies.filter(id => this.storyById(plan, id).status !== 'completed');
    if (incomplete.length) throw new Error('Story dependencies are not complete.');
  }

  private assertCompletable(story: SDLCStory): void {
    const missing = story.acceptanceCriteria.filter(item => !story.satisfiedCriteria.includes(item));
    if (missing.length) throw new Error(`Acceptance criteria are not satisfied: ${missing.join('; ')}`);
    if (!story.evidence.length) throw new Error('Completion requires at least one evidence record.');
    if (story.blockers.length) throw new Error('A story with unresolved blockers cannot be completed.');
    const unresolved = story.findings.filter(finding => finding.status === 'open' && (finding.severity === 'high' || finding.severity === 'critical'));
    if (unresolved.length) throw new Error('High or critical findings must be resolved or explicitly accepted before completion.');
    if (requiresValidation(story.type) && !story.validationRuns.some(run => run.status === 'passed')) throw new Error('This SDLC story requires a passing validation run before completion.');
  }

  private unlock(plan: SDLCPlan): SDLCPlan {
    const now = new Date().toISOString();
    return { ...plan, stories: plan.stories.map(story => story.status === 'draft' && story.dependencies.every(id => this.storyById(plan, id).status === 'completed') ? { ...story, status: 'ready' as const, updatedAt: now } : story), updatedAt: now };
  }
  ...
  private updateStory(plan: SDLCPlan, id: string, story: SDLCStory): SDLCPlan {
    return { ...plan, stories: plan.stories.map(item => item.id === id ? story : item), updatedAt: story.updatedAt };
  }
}

function requiresValidation(type: SDLCStoryType): boolean {
  return new Set<SDLCStoryType>([
    'development', 'existing-test-analysis', 'test-impact-analysis', 'new-test-creation', 'failed-test-investigation',
    'flaky-test-analysis', 'security-review', 'performance-review', 'modernization-review', 'code-review', 'pr-review',
  ]).has(type);
}
...
function buildResearchDocument(intentId: string, intent: string, context: SDLCPlanningContext, generatedAt: string): SDLCResearchDocument {
  ...
  const evidenceMatrix: SDLCResearchEvidence[] = dedupeEvidence([
    ...explicitEvidence,
    ...(context.relevantFiles ?? []).map((file, index) => ({ id: `file-${index}-${stableFragment(file)}`, kind: 'file' as const, label: file, summary: 'Repository file selected by intent and graph retrieval.', path: file })),
    ...(context.relevantSymbols ?? []).map((symbol, index) => ({ id: `symbol-${index}-${stableFragment(symbol)}`, kind: 'symbol' as const, label: symbol, summary: 'Symbol selected by semantic or structural intelligence.' })),
    ...(context.relevantApis ?? []).map((api, index) => ({ id: `api-${index}-${stableFragment(api)}`, kind: 'api' as const, label: api, summary: 'API contract affected by the intent.' })),
  ...
  const affectedFlows = unique([
    ...
    ...(context.relevantSymbols ?? []).map(symbol => `Trace callers, references, state changes, and side effects around ${symbol}.`),
  ]);
  const affectedTests = unique([...(context.relatedTests ?? []), ...(context.missingTests ?? []).map(test => `Missing coverage: ${test}`)]);
  const risks = unique([
  ...
  const unknowns = unique([
    ...(evidenceMatrix.length ? [] : ['No repository evidence has been selected; intelligence must be refreshed before approval.']),
    ...(context.relatedTests?.length ? [] : ['No existing tests were mapped to the affected code.']),
    ...(context.relevantApis?.length || context.relevantServices?.length || context.relevantSymbols?.length ? [] : ['The primary implementation boundary is not yet proven.']),
    ...(context.functionalRequirements?.length ? [] : ['Functional edge cases and failure behavior require explicit confirmation.']),
  ]);
  const implementationTargets = unique([...(context.relevantApis ?? []), ...(context.relevantServices ?? []), ...(context.relevantSymbols ?? []), ...(context.dataEntities ?? [])]);
  const recommendedApproach = unique([
    ...implementationTargets.slice(0, 8).map(target => `Implement and validate the smallest behavior slice around ${target}.`),
  ...
  const testingStrategy = unique([
    ...
    ...(context.missingTests ?? []).map(test => `Add or explicitly defer missing coverage: ${test}`),
  ...
  const section = (name: string, items: readonly string[], empty = 'No evidence was available yet; refresh repository intelligence before approval.'): string => `## ${name}\n\n${items.length ? items.map(item => `- ${item}`).join('\n') : `- ${empty}`}`;
  const matrix = evidenceMatrix.length ? evidenceMatrix.map(item => `| ${item.kind} | ${item.label.replaceAll('|', '\\|')} | ${item.summary.replaceAll('|', '\\|')} | ${item.okfId ? `\`${item.okfId}\`` : '—'} |`).join('\n') : '| — | No evidence | Refresh intelligence | — |';
  const markdown = [
    `# ${title}`, '', `**Intent ID:** ${intentId}`, `**Generated:** ${generatedAt}`, '',
    '## Problem Statement', '', intent, '',
    section('Functional Requirements', context.functionalRequirements ?? [], 'Functional requirements must be confirmed during specification review.'), '',
    section('Non-Functional Requirements', context.nonFunctionalRequirements ?? [], 'No additional non-functional requirement was supplied.'), '',
    '## Evidence Matrix', '', '| Kind | Evidence | Meaning | OKF ID |', '|---|---|---|---|', matrix, '',
    section('Repository Evidence', repositoryEvidence), '', section('Architecture and Boundaries', affectedArchitecture), '',
    section('Flows and Symbols', affectedFlows), '', section('Test Landscape', affectedTests), '', section('Risks', risks, 'No explicit risk signal was detected; reviewers must still validate the changed paths.'), '',
    section('Constraints', constraints), '', section('Unknowns Requiring Approval', unknowns, 'No unresolved research unknown was identified.'), '',
    section('Recommended Approach', recommendedApproach), '', section('Testing Strategy', testingStrategy),
  ].join('\n');
...
function buildBacklogStories(intentId: string, intent: string, context: SDLCPlanningContext): SDLCBacklogStory[] {
  ...
  if (targets.length > 1 || interfaces.length > 1) {
    drafts.push({
      kind: 'user-story', title: 'Integrate repository boundaries and preserve contracts',
  ...
  drafts.push({
    kind: 'quality-story', title: 'Prove existing coverage and change impact',
  ...
  if ((context.modernizationNotes?.length ?? 0) > 0) drafts.push({
    ...
    acceptanceCriteria: ['Modernization recommendations are scoped and user-approved.', 'Compatibility and migration consequences are explicit.'],
    linkedSdlcStoryTypes: ['modernization-review'], evidence: unique(context.modernizationNotes ?? []), dependencies: [], scope: { files: allFiles, symbols: allSymbols, interfaces },
  });
  drafts.push({
    kind: 'quality-story', title: 'Complete engineering and read-only PR review', description: 'Review specification coverage, architecture, QA, security, performance, documentation, and remaining risk.',
    acceptanceCriteria: ['Review evidence covers every approved user and quality story.', 'Reviewer-ready PR title, description, risks, validation, and unresolved questions are prepared without remote mutation.'],
    linkedSdlcStoryTypes: ['code-review', 'pr-review', 'documentation', 'completion'], evidence: unique([...(context.evidence ?? []).map(item => item.okfId ?? item.label)]), dependencies: drafts.map(item => item.title), scope: { files: allFiles, symbols: allSymbols, interfaces },
  });
  const ids = new Map<string, string>();
  for (const draft of drafts) ids.set(draft.title, `${draft.kind}-${createHash('sha256').update(`${intentId}\0${draft.title}`).digest('hex').slice(0, 16)}`);
  return drafts.map(draft => ({ ...draft, evidence: draft.evidence.length ? draft.evidence : [`Intent evidence: ${intent}`], id: ids.get(draft.title)!, dependencies: draft.dependencies.map(value => ids.get(value) ?? value), status: 'draft' }));
}
