...
export class SDLCPlanStore {
  ...
  private get target(): string {
    return path.join(this.workspaceRoot, ".keystone", "state", "sdlc", "active-plan.json");
  }
  private get intentRoot(): string {
    return path.join(this.workspaceRoot, ".keystone", "state", "intents");
  }
  async read(): Promise<SDLCPlan | undefined> {
    try {
      const parsed = JSON.parse(await fs.readFile(this.target, "utf8")) as Partial<SDLCPlan>;
      if (!parsed.intent || !parsed.id || !parsed.intentId || !Array.isArray(parsed.stories))
        return undefined;
      const generated = new SDLCEngine().createPlan(parsed.intent);
      const researchDocument = parsed.researchDocument ?? {
        ...generated.researchDocument,
        id: `research-${parsed.intentId}`
      };
      const specificationDocument =
        parsed.specificationDocument ??
        restoreSpecificationDocument(
          parsed.intentId,
          parsed.intent,
          researchDocument,
