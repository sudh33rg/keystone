...
export const MODERNIZATION_PATTERNS: readonly ModernizationPattern[] = Object.freeze([
  {
    ...
    description:
      "Capture current behavior before any migration step that can alter runtime behavior.",
  ...
  },
  {
    ...
    validation: [
      "Old and new paths can run side by side",
      "Rollback can route traffic to the existing path"
    ]
  },
  {
    ...
    strategy: "incremental-upgrade",
    description:
      "Upgrade dependencies in small batches with lockfile, compile, and test validation.",
    validation: [
      "Dependency graph has no downgraded packages",
...
export function actionFromPattern(
