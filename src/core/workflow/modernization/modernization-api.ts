...
export class ModernizationPlatformApi {
  ...
  async assess(repository: RepositoryModel, knowledgeGraph?: KnowledgePlatformGraph, correlationId?: string): Promise<LegacyAssessmentReport> {
    ...
    const readinessScore = clamp(100 - (technicalDebtScore * 0.55 + complexityScore * 0.25) + readinessCredits(metrics), 0, 100);
    ...
    await this.publish('ModernizationAssessed', { assessmentId: report.id, repositoryId: repository.id, readinessScore: report.readinessScore }, correlationId);
    return report;
  }

  async mapCapabilities(repository: RepositoryModel, knowledgeGraph?: KnowledgePlatformGraph): Promise<readonly BusinessCapability[]> {
    const nodes = knowledgeGraph?.nodes.filter(node => node.type === 'Service' || node.type === 'Module' || node.type === 'Component') ?? [];
  ...
  async discoverArchitecture(repository: RepositoryModel, knowledgeGraph?: KnowledgePlatformGraph): Promise<ArchitectureDiscovery> {
    ...
    return deepFreeze({
      ...
      confidence: round(Math.min(0.9, 0.45 + components.length * 0.04 + repository.frameworks.length * 0.03)),
    });
  }

  async analyzeGaps(assessment: LegacyAssessmentReport, targetArchitecture: TargetArchitecture): Promise<readonly ModernizationGap[]> {
    const gaps: ModernizationGap[] = [];
    if (assessment.metrics.tests === 0) {
      gaps.push(gap('testing-safety-net', 'testing', 'No visible test safety net', 'No test files detected', 'Characterization tests protect behavior before migration', 'critical', 'medium', ['Repository file inventory']));
    }
    if (assessment.metrics.documentation === 0) {
      gaps.push(gap('documentation-baseline', 'documentation', 'Missing documentation baseline', 'No documentation files detected', 'Architecture and migration decisions are documented', 'medium', 'low', ['Repository documentation inventory']));
    }
    if (assessment.metrics.dependencies > 40) {
      gaps.push(gap('dependency-surface', 'dependency', 'Large dependency surface', `${assessment.metrics.dependencies} dependency signals`, 'Dependencies are grouped, upgraded, and validated incrementally', 'high', 'high', ['Repository dependency index']));
    }
    if (targetArchitecture.style !== 'unknown' && assessment.riskProfile.some(risk => risk.area === 'architecture')) {
      gaps.push(gap('architecture-target', 'architecture', 'Architecture needs target alignment', 'Current boundaries are inferred or weak', `${targetArchitecture.name} with explicit boundaries`, 'high', 'high', ['Architecture discovery']));
    }
    for (const risk of assessment.riskProfile.filter(r => r.area === 'technology-stack' || r.area === 'security')) {
      gaps.push(gap(`risk-${risk.id}`, risk.area, risk.description, 'Current stack contains migration risk', 'Risk mitigated under target platform constraints', risk.severity, risk.severity === 'critical' ? 'high' : 'medium', risk.evidence));
    }
    return Object.freeze(gaps.map(deepFreeze));
  }

  recommendTargets(assessment: LegacyAssessmentReport, currentArchitecture?: ArchitectureDiscovery): readonly TargetArchitectureRecommendation[] {
    const candidates: TargetArchitecture[] = [
      {
        ...
        technologyPreferences: assessment.technologyInventory.filter(item => item.kind === 'language').map(item => item.name),
      },
      {
        id: 'target-service-oriented',
        name: 'Service-oriented architecture',
        style: 'service-oriented',
        principles: ['Isolate capabilities', 'Explicit API contracts', 'Independent operational ownership'],
      ...
      },
      {
        ...
        style: 'microservices',
        principles: ['Route incrementally', 'Dual-run critical behavior', 'Retire legacy by capability'],
        technologyPreferences: ['feature-flags', 'eventing', 'contract-tests'],
      },
    ];
    return Object.freeze(candidates.map(target => {
      const score = target.style === 'modular-monolith'
      ...
      return deepFreeze({
        ...
        tradeoffs: {
          migrationCost: target.style === 'microservices' ? 'high' as const : 'medium' as const,
          operationalRisk: target.style === 'microservices' ? 'high' as const : 'medium' as const,
          businessRisk: assessment.riskProfile.some(risk => risk.area === 'business-capability') ? 'high' as const : 'medium' as const,
          reversibility: target.style === 'modular-monolith' ? 'low' as const : 'medium' as const,
        },
        rationale: Object.freeze([
          `Readiness score ${assessment.readinessScore}`,
          `${assessment.riskProfile.length} risk item(s) considered`,
          currentArchitecture ? `Current style ${currentArchitecture.style}` : 'Current architecture not supplied',
        ]),
      });
  ...
  async propose(request: ModernizationRequest, correlationId?: string): Promise<ModernizationProposal> {
    const coverage = scanCoverage(request);
    if (!coverage.complete) {
      throw new Error(`Modernization discovery requires a complete repository scan (${coverage.analyzedFiles}/${coverage.expectedFiles} files indexed)`);
    }
    const assessment = await this.assess(request.repository, request.knowledgeGraph, correlationId);
    const architecture = await this.discoverArchitecture(request.repository, request.knowledgeGraph);
    ...
    await this.publish('ModernizationProposed', { proposalId: proposal.id, repositoryId: request.repository.id }, correlationId);
    return proposal;
  }

  /** Converts an explicit user choice into the detailed, traceable plan and specifications. */
  async planAccepted(proposalId: string, input: ModernizationDecisionInput, correlationId?: string): Promise<ModernizationPlan> {
    const proposal = this.proposals.get(proposalId);
    if (!proposal) throw new Error(`Modernization proposal not found: ${proposalId}`);
    if (!input.accepted) throw new Error('Modernization plan generation requires explicit user acceptance');
    const recommendationCategories = new Set(proposal.technologyRecommendations.map(item => item.category));
    for (const [category, technology] of Object.entries(input.acceptedTechnologies ?? {})) {
      if (!recommendationCategories.has(category as TechnologyRecommendation['category'])) throw new Error(`Unknown modernization technology category: ${category}`);
      if (!technology.trim()) throw new Error(`Select a non-empty technology for ${category}`);
      if (technology.length > 200) throw new Error(`Technology choice for ${category} exceeds 200 characters`);
    }
    const recommended = input.selectedTargetId
      ? proposal.architectureRecommendations.find(item => item.target.id === input.selectedTargetId)?.target
      : proposal.architectureRecommendations[0]?.target;
    const targetArchitecture = input.customTarget ?? recommended;
    if (!targetArchitecture) throw new Error('Select a Keystone target or provide a custom target architecture');
    if (!targetArchitecture.id.trim() || !targetArchitecture.name.trim()) throw new Error('The selected target architecture requires a non-empty id and name');
    const technologies = Object.freeze(Object.fromEntries(proposal.technologyRecommendations.map(item => [
      item.category,
    ...
    const decision: ModernizationDecision = deepFreeze({
      ...
      source: input.customTarget || proposal.technologyRecommendations.some(item => input.acceptedTechnologies?.[item.category] && input.acceptedTechnologies[item.category] !== item.recommendedTechnology)
        ? 'user-defined' : 'keystone-recommendation',
      targetArchitecture: deepFreeze({ ...targetArchitecture, technologyPreferences: Object.values(technologies) }),
      technologies,
      notes: Object.freeze([...(input.notes ?? [])]),
    });
    const plan = await this.buildPlanFromDiscovery(proposal, decision, correlationId);
    await this.publish('ModernizationDecisionAccepted', { proposalId, planId: plan.id, source: decision.source }, correlationId);
    return plan;
  }
  ...
  async plan(request: ModernizationRequest, correlationId?: string): Promise<ModernizationPlan> {
    ...
    const capabilities = await this.mapCapabilities(request.repository, request.knowledgeGraph);
    const architecture = await this.discoverArchitecture(request.repository, request.knowledgeGraph);
    const targetArchitecture = request.targetArchitecture ?? defaultTargetArchitecture(request.repository, architecture);
    ...
    await this.publish('ModernizationPlanned', { planId: plan.id, repositoryId: request.repository.id, phases: plan.phases.length, strategy }, correlationId);
    return plan;
  }

  private async buildPlanFromDiscovery(proposal: ModernizationProposal, decision: ModernizationDecision, correlationId?: string): Promise<ModernizationPlan> {
    ...
    const phases = buildPhases(proposal.assessment, proposal.capabilities, gaps, proposal.constraints);
    const plan: ModernizationPlan = deepFreeze({
      ...
      decision,
      specifications: specificationsFor(phases, decision.targetArchitecture, decision, proposal.objectives, proposal.constraints.map(item => item.description)),
    });
    this.plans.set(plan.id, plan);
    await this.publish('ModernizationPlanned', { planId: plan.id, repositoryId: proposal.repositoryId, phases: phases.length, strategy }, correlationId);
    return plan;
  }

  async validate(plan: ModernizationPlan, correlationId?: string): Promise<ModernizationValidationReport> {
    const blockers: string[] = [];
    const warnings: string[] = [];
    const phaseIds = new Set(plan.phases.map(phase => phase.id));
    for (const phase of plan.phases) {
      for (const prerequisite of phase.prerequisites) {
        if (!phaseIds.has(prerequisite)) blockers.push(`Phase ${phase.id} references missing prerequisite ${prerequisite}`);
      }
      if (phase.validation.length === 0) blockers.push(`Phase ${phase.id} has no functional-equivalence validation`);
      if (phase.rollback.length === 0) warnings.push(`Phase ${phase.id} has no rollback notes`);
      if (phase.transformations.some(action => !action.reversible)) warnings.push(`Phase ${phase.id} contains non-reversible transformation(s)`);
    }
    if (plan.gaps.some(g => g.priority === 'critical') && !plan.phases.some(phase => phase.requiresApproval)) {
    ...
    const report: ModernizationValidationReport = deepFreeze({
      ...
      blockers,
      warnings,
      requiredApprovals: plan.phases.filter(phase => phase.requiresApproval).map(phase => phase.id),
    });
    await this.publish('ModernizationValidated', { planId: plan.id, valid: report.valid, blockers: blockers.length }, correlationId);
    return report;
  }

  impact(planId: string): ModernizationImpact {
    const plan = this.requirePlan(planId);
    const impactedAssets = [...new Set(plan.phases.flatMap(phase => phase.scope).filter(Boolean))];
    return deepFreeze({
      planId,
  ...
  estimateCost(planId: string): ModernizationCostEstimate {
    const plan = this.requirePlan(planId);
    const complexityMultiplier = round(1 + plan.metrics.highRiskItems * 0.15 + Math.max(0, plan.metrics.totalPhases - 3) * 0.05);
  ...
  trackExecution(planId: string, completedPhases: readonly string[] = [], issues: readonly string[] = []): ModernizationExecutionStatus {
    const plan = this.requirePlan(planId);
    const completed = new Set(completedPhases);
    const active = plan.phases.find(phase => !completed.has(phase.id) && phase.prerequisites.every(prerequisite => completed.has(prerequisite)));
    const completedEffort = plan.phases.filter(phase => completed.has(phase.id)).reduce((sum, phase) => sum + phase.estimatedEffortDays, 0);
  ...
  async govern(planId: string, approvedPhases: readonly string[] = [], correlationId?: string): Promise<ModernizationGovernanceReport> {
    const plan = this.requirePlan(planId);
    const validation = await this.validate(plan, correlationId);
    const approved = new Set(approvedPhases);
    const missingApprovals = validation.requiredApprovals.filter(phaseId => !approved.has(phaseId));
    const policyViolations = [
      ...validation.blockers,
      ...(missingApprovals.length ? [`Missing approval(s): ${missingApprovals.join(', ')}`] : []),
      ...(plan.metrics.highRiskItems > 0 && plan.metrics.validationChecks === 0 ? ['High-risk plan requires validation checks'] : []),
    ];
    return deepFreeze({
      planId,
  ...
  async execute(planId: string, workflows: WorkflowPlatformApi, correlationId?: string): Promise<ModernizationPlan> {
    const plan = this.requirePlan(planId);
    const validation = await this.validate(plan, correlationId);
    if (!validation.valid) {
      throw new Error(`Cannot execute invalid modernization plan: ${validation.blockers.join('; ')}`);
    }
    ...
    await this.publish('ModernizationExecutionStarted', { planId, workflowId: execution.workflowId, state: execution.state }, correlationId);
    return updated;
  }
  ...
  plansForRepository(repositoryId: string): readonly ModernizationPlan[] {
    return Object.freeze([...this.plans.values()].filter(plan => plan.repositoryId === repositoryId));
  }
  ...
  statistics(): ModernizationPlatformStats {
    ...
    return deepFreeze({
      ...
      openHighRiskItems: plans.reduce((sum, plan) => sum + plan.risks.filter(risk => risk.severity === 'high' || risk.severity === 'critical').length, 0),
    });
  }
  ...
  private async publish(eventType: string, payload: Record<string, unknown>, correlationId?: string): Promise<void> {
...
function assessmentMetrics(repository: RepositoryModel): LegacyAssessmentMetrics {
  ...
  return deepFreeze({
    ...
    averageFileLines: round(lineCounts.reduce((sum, lines) => sum + lines, 0) / Math.max(1, lineCounts.length)),
    maxFileLines: lineCounts.length ? Math.max(...lineCounts) : 0,
  });
}

function assessRisks(repository: RepositoryModel, metrics: LegacyAssessmentMetrics, graph?: KnowledgePlatformGraph): readonly ModernizationRisk[] {
  const risks: ModernizationRisk[] = [];
  if (metrics.tests === 0) {
    risks.push(risk('missing-tests', 'testing', 'No test files were discovered before modernization.', 'critical', 0.85, 'critical', ['Create characterization tests before code transformation'], ['Repository file inventory']));
  }
  if (metrics.documentation === 0) {
    risks.push(risk('missing-docs', 'documentation', 'No documentation baseline was discovered.', 'medium', 0.7, 'medium', ['Create architecture summary and migration ADR before execution'], ['Repository documentation inventory']));
  }
  if (metrics.maxFileLines > 500) {
    risks.push(risk('large-files', 'code', 'Large source files increase transformation blast radius.', metrics.maxFileLines > 1000 ? 'high' : 'medium', 0.65, 'high', ['Split transformations by file and validate behavior after each phase'], [`Maximum file length ${metrics.maxFileLines}`]));
  }
  if (repository.dependencies.length > 60) {
    risks.push(risk('dependency-volume', 'dependency', 'Large dependency surface increases upgrade compatibility risk.', 'high', 0.72, 'high', ['Modernize dependencies in isolated batches with lockfile validation'], [`${repository.dependencies.length} dependency signals`]));
  }
  const weakArchitecture = !graph || graph.nodes.filter(node => node.category === 'architecture').length === 0;
  if (weakArchitecture) {
    risks.push(risk('architecture-unknowns', 'architecture', 'Architecture boundaries are not explicitly modeled.', 'medium', 0.6, 'high', ['Run architecture discovery and require review for boundary changes'], ['Knowledge graph architecture nodes absent or unavailable']));
  }
  if (repository.frameworks.some(framework => framework.category === 'database')) {
    risks.push(risk('database-coupling', 'database', 'Database framework signals require data compatibility validation.', 'medium', 0.55, 'high', ['Add schema migration and rollback validation'], repository.frameworks.filter(f => f.category === 'database').map(f => f.name)));
  }
...
function technologyInventory(repository: RepositoryModel): readonly TechnologyInventoryItem[] {
  ...
  for (const language of repository.languages) {
    items.push({ name: language.language, kind: 'language', evidence: [`${language.files} file(s)`] });
  }
  for (const framework of repository.frameworks) {
    items.push({ name: framework.name, kind: framework.category === 'database' ? 'database' : 'framework', evidence: framework.evidence });
  }
...
function uniqueDependencies(dependencies: readonly RepositoryDependency[]): readonly RepositoryDependency[] {
...
function recommendationsFor(risks: readonly ModernizationRisk[], metrics: LegacyAssessmentMetrics): readonly string[] {
  const recommendations = ['Run modernization through approval-gated incremental phases'];
  if (risks.some(risk => risk.area === 'testing')) recommendations.push('Build characterization tests before transforming production code');
  if (risks.some(risk => risk.area === 'architecture')) recommendations.push('Confirm architecture boundaries before choosing a migration strategy');
  if (metrics.dependencies > 0) recommendations.push('Group dependency upgrades by ecosystem and verify each batch independently');
  if (metrics.documentation === 0) recommendations.push('Create migration ADRs and rollback runbooks as first-class deliverables');
  return Object.freeze(recommendations);
}

function moduleToNode(module: { readonly id: string; readonly name: string; readonly path: string }): KnowledgeNode {
  return {
    id: module.id,
    type: 'Module',
...
function assetReferencesFor(node: KnowledgeNode, repository: RepositoryModel): readonly string[] {
  ...
  const matched = repository.files
    .filter(file => file.path.toLowerCase().includes(title) || file.symbols.some(symbol => symbol.name.toLowerCase().includes(title)))
    .map(file => file.id);
...
function buildComponents(repository: RepositoryModel, graph?: KnowledgePlatformGraph): readonly ArchitectureComponent[] {
  const graphComponents = graph?.nodes.filter(node => node.type === 'Service' || node.type === 'Component' || node.type === 'Module') ?? [];
  if (graphComponents.length > 0) {
    return Object.freeze(graphComponents.map(node => deepFreeze({
      id: `component-${node.id}`,
      name: node.title,
      kind: node.type === 'Service' ? 'service' : node.type === 'Package' ? 'package' : 'module',
      assets: assetReferencesFor(node, repository),
      dependencies: graph?.edges.filter(edge => edge.sourceId === node.id).map(edge => edge.targetId) ?? [],
    } satisfies ArchitectureComponent)));
  }
  const directories = repository.directories.filter(dir => !dir.parentPath || dir.parentPath === '.').slice(0, 12);
  const components = directories.map(dir => {
    ...
    return deepFreeze({
      ...
      dependencies: dependenciesForFiles(repository, files.map(file => file.id)),
    } satisfies ArchitectureComponent);
  });
...
function dependenciesForFiles(repository: RepositoryModel, fileIds: readonly string[]): readonly string[] {
  const fileSet = new Set(fileIds);
  return Object.freeze(repository.dependencies.filter(dep => fileSet.has(dep.sourceAssetId)).map(dep => dep.target));
}

function buildBoundaries(repository: RepositoryModel, components: readonly ArchitectureComponent[]): readonly ArchitectureBoundary[] {
  return Object.freeze(components.map(component => deepFreeze({
    id: `boundary-${component.id}`,
    name: `${component.name} boundary`,
    assets: component.assets,
    risk: component.dependencies.length > 15 ? 'high' : component.assets.length > 20 ? 'medium' : 'low',
  } satisfies ArchitectureBoundary)));
}

function inferArchitectureStyle(repository: RepositoryModel, graph: KnowledgePlatformGraph | undefined, components: readonly ArchitectureComponent[]): ArchitectureDiscovery['style'] {
  if (graph?.edges.some(edge => edge.relationship === 'Publishes' || edge.relationship === 'Consumes')) return 'event-driven';
  if (components.filter(component => component.kind === 'service').length > 1) return 'service-oriented';
  if (repository.frameworks.some(framework => framework.name.toLowerCase().includes('express') || framework.name.toLowerCase().includes('spring'))) return 'layered';
  return components.length > 1 ? 'modular-monolith' : 'unknown';
}

function defaultTargetArchitecture(repository: RepositoryModel, architecture: ArchitectureDiscovery): TargetArchitecture {
  return deepFreeze({
    id: `target-${repository.id}`,
    name: architecture.style === 'unknown' ? 'Incremental modular architecture' : `Modernized ${architecture.style} architecture`,
    style: architecture.style === 'unknown' ? 'modular-monolith' : architecture.style,
    principles: ['Preserve business behavior', 'Modernize incrementally', 'Keep rollback paths explicit'],
    technologyPreferences: repository.languages.map(language => language.language),
  });
}

function chooseStrategy(assessment: LegacyAssessmentReport, gaps: readonly ModernizationGap[], architecture: ArchitectureDiscovery): ModernizationStrategy {
  if (assessment.readinessScore < 35 || gaps.some(gapItem => gapItem.priority === 'critical')) return 'strangler-fig';
...
function buildPhases(
  ...
  constraints: readonly { readonly id: string; readonly description: string; readonly severity: ModernizationSeverity }[],
  ...
  phases.push(phase({
    ...
    prerequisites: [],
    risks: assessment.riskProfile.filter(riskItem => riskItem.area === 'testing' || riskItem.area === 'documentation').map(riskItem => riskItem.id),
    transformations: [
      actionFromPattern(patternsForArea('testing')[0], capabilities.flatMap(capability => capability.assets).slice(0, 20)),
      actionFromPattern(patternsForArea('documentation')[0], []),
    ],
    validation: [validationCheck('baseline-tests', 'Critical current behavior', 'characterization-test', ['Baseline behavior is reproducible before migration'])],
    rollback: ['Safety-net phase is additive; remove generated tests/docs if needed'],
    estimatedEffortDays: 3,
    requiresApproval: false,
  }));

  const sortedGaps = [...gaps].sort((a, b) => severityWeight(b.priority) - severityWeight(a.priority));
  let order = 2;
  for (const gapItem of sortedGaps.slice(0, 8)) {
    phases.push(phase({
      ...
      risks: assessment.riskProfile.filter(riskItem => riskItem.area === gapItem.area || riskItem.severity === gapItem.priority).map(riskItem => riskItem.id),
      transformations: transformationsForGap(gapItem),
      validation: validationForGap(gapItem),
      rollback: ['Keep previous implementation path available until validation passes', 'Revert the phase changes as one unit if checks fail'],
      estimatedEffortDays: effortDays(gapItem.effort, gapItem.priority),
      requiresApproval: gapItem.priority === 'high' || gapItem.priority === 'critical' || constraints.some(c => c.severity === 'critical'),
    }));
    order += 1;
  }
  ...
  phases.push(phase({
    ...
    transformations: [],
    validation: [validationCheck('final-equivalence', 'Modernized system behavior', 'integration-test', ['No functional regression', 'All accepted risks are documented'])],
...
function validationForGap(gapItem: ModernizationGap): readonly FunctionalEquivalenceCheck[] {
  const verification = gapItem.area === 'api'
    ? 'contract-test'
    : gapItem.area === 'database' || gapItem.area === 'architecture'
      ? 'integration-test'
      : 'characterization-test';
  return Object.freeze([validationCheck(`validation-${gapItem.id}`, gapItem.title, verification, [
...
function planMetrics(phases: readonly MigrationPhase[], assessment: LegacyAssessmentReport): ModernizationMetrics {
  return deepFreeze({
    totalPhases: phases.length,
    estimatedEffortDays: phases.reduce((sum, phaseItem) => sum + phaseItem.estimatedEffortDays, 0),
    highRiskItems: assessment.riskProfile.filter(riskItem => riskItem.severity === 'high' || riskItem.severity === 'critical').length,
    reversibleTransformations: phases.flatMap(phaseItem => phaseItem.transformations).filter(action => action.reversible).length,
...
function risk(
  ...
  return deepFreeze({ id, area, description, severity, probability: round(probability), impact, mitigation, evidence });
}
...
function readinessCredits(metrics: LegacyAssessmentMetrics): number {
  return (metrics.tests > 0 ? 8 : 0) + (metrics.documentation > 0 ? 5 : 0) + (metrics.buildDefinitions > 0 ? 5 : 0);
}
...
function heat(risks: readonly ModernizationRisk[], area: string): number {
  ...
  return Math.min(100, scoped.reduce((sum, risk) => sum + severityWeight(risk.severity) * risk.probability * 25, 0));
}
...
function scanCoverage(request: ModernizationRequest) {
  ...
  const complete = expectedFiles === indexedFiles && indexedFiles === request.repository.files.length;
  return deepFreeze({
    expectedFiles,
    analyzedFiles,
    excludedPaths,
    ...
    evidence: Object.freeze([
      ...
      excludedPaths.length ? `Explicit exclusions: ${excludedPaths.join(', ')}` : 'No repository paths were reported as excluded',
    ]),
  });
}

function recommendTechnologies(assessment: LegacyAssessmentReport): readonly TechnologyRecommendation[] {
  const languages = assessment.technologyInventory.filter(item => item.kind === 'language').map(item => item.name.toLowerCase());
  const runtime = languages.some(language => language.includes('java')) ? ['Java 21 LTS', 'Java 25 LTS']
    : languages.some(language => language.includes('python')) ? ['Python 3.13', 'Python 3.12']
      : languages.some(language => language.includes('c#')) ? ['.NET 10 LTS', '.NET 8 LTS']
        : ['Node.js 24 LTS', 'Node.js 22 LTS'];
  const framework = languages.some(language => language.includes('java')) ? ['Spring Boot 4', 'Quarkus']
    : languages.some(language => language.includes('python')) ? ['FastAPI', 'Django']
      : languages.some(language => language.includes('c#')) ? ['ASP.NET Core', 'Minimal APIs']
        : ['TypeScript', 'NestJS'];
  const entries: Array<[TechnologyRecommendation['category'], string | undefined, readonly string[]]> = [
    ['runtime', languages[0], runtime],
    ['framework', assessment.technologyInventory.find(item => item.kind === 'framework')?.name, framework],
    ['testing', assessment.metrics.tests ? 'Existing test stack' : undefined, ['Characterization + contract tests', 'Integration test harness']],
  ...
  return Object.freeze(entries.map(([category, currentTechnology, choices]) => deepFreeze({
    id: `technology-${category}`,
    category,
    currentTechnology,
    recommendedTechnology: choices[0],
    alternatives: Object.freeze(choices.slice(1)),
    rationale: Object.freeze([`Fits the detected ${languages.join(', ') || 'unknown'} ecosystem`, 'Supports incremental migration and automated validation']),
    migrationNotes: Object.freeze(['Validate compatibility in an isolated phase', 'Keep the previous runtime path available until acceptance checks pass']),
...
function specificationsFor(
  ...
  const technologies = decision ? Object.entries(decision.technologies).map(([category, technology]) => `${category}: ${technology}`) : target.technologyPreferences;
  return Object.freeze(phases.map(phaseItem => deepFreeze({
    ...
    functionalRequirements: Object.freeze([
      ...phaseItem.goals.map(goal => `The modernized system shall ${goal.charAt(0).toLowerCase()}${goal.slice(1)}.`),
    ...
    acceptanceCriteria: Object.freeze(phaseItem.validation.flatMap(check => check.acceptanceCriteria)),
    validation: Object.freeze(phaseItem.validation.map(check => `${check.verification}: ${check.scope}`)),
    rollout: Object.freeze(['Deploy behind a controlled release boundary', 'Collect validation evidence', `Obtain approval: ${phaseItem.requiresApproval ? 'required' : 'not required'}`]),
    rollback: phaseItem.rollback,
    traceability: Object.freeze([`phase:${phaseItem.id}`, ...phaseItem.risks.map(riskId => `risk:${riskId}`), ...(decision ? [`proposal:${decision.proposalId}`] : [])]),
  })));
}
