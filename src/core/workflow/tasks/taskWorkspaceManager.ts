...
export class TaskWorkspaceManager {
  ...
  async create(seed: TaskWorkspaceSeed): Promise<TaskWorkspaceRef> {
    ...
    const ref: TaskWorkspaceRef = {
      id: name,
      name,
      relativePath: `.keystone/tasks/${name}`,
      absolutePath,
      status: "research-ready"
    };
    await Promise.all([
      this.write(absolutePath, "task.json", {
        id: name,
        intent: seed.intent,
      ...
      this.write(absolutePath, "research-status.json", {
        intentId: research.intentId,
      ...
      this.write(absolutePath, "progress.json", {
        status: "research-ready",
        percent: 10,
        completed: ["Repository intelligence gathered", "Repository R&D generated"],
        current: "Awaiting R&D review before planning",
        blockers: [],
        updatedAt: now
      }),
      this.write(absolutePath, "context.json", {
        relevantFiles: seed.relevantFiles,
        relevantSymbols: seed.relevantSymbols,
        tests: seed.tests,
  ...
  async createModernization(plan: ModernizationPlan): Promise<TaskWorkspaceRef> {
    const { name, absolutePath } = await this.allocate(
      `modernize-${slug(plan.targetArchitecture.name).slice(0, 46)}`
    );
    const now = new Date().toISOString();
    const ref: TaskWorkspaceRef = {
      id: name,
    ...
    await Promise.all([
      ...
      this.write(absolutePath, "plan.json", {
        status: "approved",
        sourcePlanId: plan.id,
        strategy: plan.strategy,
        targetArchitecture: plan.targetArchitecture,
      ...
      this.write(absolutePath, "progress.json", {
        ...
        percent: 15,
        completed: [
          "Repository-wide assessment completed",
          "Technology choices accepted",
          "Detailed specifications generated"
        ],
        current: firstPhase ? `Ready to begin: ${firstPhase.name}` : "Ready for implementation",
      ...
      this.write(absolutePath, "context.json", {
        ...
        risks: plan.risks,
        targetArchitecture: plan.targetArchitecture,
        technologyDecisions: plan.decision?.technologies ?? {},
        workflowRequest: plan.workflowRequest
      }),
  ...
  ): Promise<TaskWorkspaceRef> {
    const now = new Date().toISOString();
    const existing =
      (await this.read<Record<string, unknown>>(ref.absolutePath, "progress.json")) ?? {};
    const existingStatus =
      (await this.read<Record<string, unknown>>(ref.absolutePath, "status.json")) ?? {};
    await Promise.all([
      this.write(ref.absolutePath, "progress.json", {
        ...existing,
        status,
    ...
    await Promise.all([
      ...
      this.write(ref.absolutePath, "progress.json", {
        ...
        completed: [
          ...
          "Repository R&D reviewed and approved",
          "Implementation specification and backlog generated"
        ],
        current: "Awaiting specification approval",
        blockers: [],
        updatedAt: now
      }),
  ...
  async approveResearch(ref: TaskWorkspaceRef, intentId: string): Promise<void> {
    ...
    if (task.intentId && task.intentId !== intentId)
      throw new Error("The active task research does not belong to this intent.");
    await Promise.all([
      this.write(ref.absolutePath, "task.json", {
        ...task,
        intentId,
        researchStatus: "approved",
        updatedAt: now
      }),
      this.write(ref.absolutePath, "research-status.json", {
        intentId,
        status: "approved",
        reviewedAt: now,
        updatedAt: now
  ...
  private async finalize(
    ...
    const done = await this.update(ref, status, {
      percent: status === "done" ? 100 : 0,
      current: reason ?? (status === "done" ? "Completed" : "Cancelled"),
      completed: status === "done" ? ["Task completed and temporary workspace finalized"] : []
    });
  ...
  async exportForHandoff(ref: TaskWorkspaceRef, targetRoot: string): Promise<string> {
    ...
    await this.write(destination, "handoff.json", {
      sourceTask: ref.id,
      exportedAt: new Date().toISOString(),
      requiresManualRepositorySync: true
    });
    return destination;
  }
  ...
  async importHandoffPackage(packageValue: Record<string, any>): Promise<string> {
    ...
    await Promise.all([
      ...
      this.write(
        destination,
        "instructions.md",
        packageValue.continuation?.suggestedFirstPrompt ?? "Continue from the restored task state."
      ),
      this.write(destination, "handoff.json", {
  ...
  async createFromHandoff(packageValue: TaskStatePackage): Promise<TaskWorkspaceRef> {
    ...
    const { name, absolutePath } = await this.allocate(
      `restored-${slug(packageValue.task.normalizedProblemStatement).slice(0, 46)}`
    );
    const now = new Date().toISOString();
    const blocked = packageValue.progress.blockers.length > 0;
    const status: TaskWorkspaceStatus = blocked
      ? "blocked"
      : packageValue.progress.progressPercentage > 0
        ? "in-progress"
        : "planned";
    const ref: TaskWorkspaceRef = {
      id: name,
    ...
    await Promise.all([
      this.write(absolutePath, "task.json", {
        id: name,
        kind: "restored-handoff",
        sourcePackageId: packageValue.packageId,
        sourceTaskId: packageValue.taskId,
        intent: packageValue.task.originalUserRequest,
        intentType: "restored-handoff",
        route: "human-review",
        createdAt: now,
        updatedAt: now
      }),
      ...
      this.write(
        absolutePath,
        "instructions.md",
        `${packageValue.continuation.manualRepositorySyncReminder}\n\nNext action: ${packageValue.continuation.exactNextRecommendedAction}\n\nDo not assume reported tests still pass; rerun them in this synchronized repository.\n`
      ),
      this.write(absolutePath, "agents.json", [
        {
          id: "continuation-planner",
          role: "Reconcile restored state with the current repository"
        },
        { id: "executor", role: "Continue only the recorded pending work" },
        { id: "qa", role: "Revalidate reported and pending tests" },
        { id: "reviewer", role: "Review restored security, performance, and decision evidence" }
      ]),
      this.write(absolutePath, "plan.json", {
        status,
        ...packageValue.plan,
      ...
      this.write(absolutePath, "progress.json", {
        status,
        percent: packageValue.progress.progressPercentage,
        completed: packageValue.progress.completedWorkSummary,
        current:
          packageValue.progress.currentActivity ??
          packageValue.continuation.exactNextRecommendedAction,
        blockers: packageValue.progress.blockers,
        openQuestions: packageValue.progress.openQuestions,
        updatedAt: now
      }),
      this.write(absolutePath, "context.json", {
        ...packageValue.context,
        tests: packageValue.quality.testsPlanned,
        qaChecks: packageValue.quality.qualityChecksStillRequired,
        securityFindings: packageValue.quality.securityFindings,
  ...
  private async findBySourcePackage(packageId: string): Promise<TaskWorkspaceRef | undefined> {
    ...
    for (const name of await fs.readdir(this.tasksRoot)) {
      ...
      if (
        task?.sourcePackageId === packageId &&
        state?.status &&
        state.status !== "done" &&
  ...
  async latestActive(): Promise<TaskWorkspaceRef | undefined> {
    ...
    for (const name of await fs.readdir(this.tasksRoot)) {
      if (!/^\d{4}_/.test(name)) continue;
      const absolutePath = path.join(this.tasksRoot, name);
      const status = await this.read<{ status?: TaskWorkspaceStatus; updatedAt?: string }>(
        absolutePath,
        "status.json"
      );
    ...
    return candidates.sort(
      (left, right) =>
        right.updatedAt.localeCompare(left.updatedAt) || right.name.localeCompare(left.name)
    )[0];
  }
  ...
  private async nextSequence(): Promise<number> {
    ...
    let max = names.reduce(
      (value, name) => Math.max(value, Number(name.match(/^(\d{4})_/)?.[1] ?? 0)),
      0
    );
    try {
      const completed = await fs.readFile(path.join(this.tasksRoot, "completed.jsonl"), "utf8");
      for (const line of completed.split(/\r?\n/).filter(Boolean))
        max = Math.max(max, Number((JSON.parse(line).id as string)?.match(/^(\d{4})_/)?.[1] ?? 0));
    } catch {
      /* No completed task archive yet. */
    }
    return max + 1;
  }
...
function initialSpecification(
  seed: TaskWorkspaceSeed,
  research: NonNullable<TaskWorkspaceSeed["research"]>
): string {
...
function skill(seed: TaskWorkspaceSeed): string {
...
function instructions(seed: TaskWorkspaceSeed): string {
...
function agents(seed: TaskWorkspaceSeed) {
  return [
    { id: "planner", role: "Maintain plan and scope" },
    { id: "executor", role: "Perform approved implementation only" },
    { id: "qa", role: `Run ${seed.qaChecks.length} QA checks` },
    { id: "reviewer", role: "Review security, performance, and modernization risks" }
  ];
}
function modernizationSpecification(plan: ModernizationPlan): string {
  return `# Accepted Modernization Specification\n\n## Objective\n\nMigrate incrementally to **${plan.targetArchitecture.name}** using the **${plan.strategy}** strategy while preserving observable behavior.\n\n## Architecture Principles\n\n${plan.targetArchitecture.principles.map((item) => `- ${item}`).join("\n")}\n\n## Accepted Technologies\n\n${
    Object.entries(plan.decision?.technologies ?? {})
      .map(([category, technology]) => `- ${category}: ${technology}`)
...
function modernizationSkill(plan: ModernizationPlan): string {
