...
import type { DeliveryPersistenceStore } from "../persistence/DeliveryPersistenceStore";
...
export interface PullRequestProvider {
  detect(root: string, state: GitRepositoryState): Promise<PullRequestProviderCapability>;
  discoverTemplates(root: string): Promise<Array<{ id: string; name: string; body: string }>>;
  create?(draft: PullRequestDraft): Promise<{ number: number; url: string; evidence: string[] }>;
  openAssisted?(draft: PullRequestDraft): Promise<void>;
  get?(url: string): Promise<{ status: string; evidence: string[] }>;
}
...
export class DeliveryCoordinator {
  ...
  readonly readiness: SourceControlReadinessService;
  readonly changes: GitChangeSetService;
  readonly commits: CommitPlanningService;
  readonly approvals: GitMutationApprovalService;
  readonly mutations: GitMutationService;
  readonly pullRequests: PullRequestCapabilityService;
  readonly drafts: PullRequestDraftService;
