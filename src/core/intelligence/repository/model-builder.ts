...
export class RepositoryModelBuilder {
  ...
  build(rootPath: string): RepositoryModel {
    ...
    const files = this.discoverFiles(absoluteRoot).map((filePath) =>
      this.buildSourceFile(repositoryId, absoluteRoot, filePath)
    );
  ...
  private discoverFiles(rootPath: string): string[] {
    ...
    const walk = (dir: string): void => {
      ...
      try {
        ...
        this.excludedPaths.push(normalizePath(path.relative(rootPath, dir)) || ".");
        return;
      }
      for (const entry of entries) {
        ...
        if (LANGUAGE_BY_EXTENSION[ext]) {
          try {
            if (fs.statSync(absolutePath).size > 5_000_000) {
              this.excludedPaths.push(normalizePath(path.relative(rootPath, absolutePath)));
              continue;
            }
          } catch {
            this.excludedPaths.push(normalizePath(path.relative(rootPath, absolutePath)));
  ...
  private extractSymbols(
    ...
    const patterns: Array<{ kind: RepositorySymbolKind; pattern: RegExp }> =
      language === "python"
        ? [
  ...
  private packageDependencies(repositoryId: string, rootPath: string): RepositoryDependency[] {
    ...
    const add = (
      ...
      scope: RepositoryDependency["scope"]
    ): void => {
  ...
  private modulesFor(repositoryId: string, rootPath: string, files: readonly SourceFile[]) {
    return [...new Set(files.map((file) => file.path.split("/")[0]).filter(Boolean))].map(
      (name) => ({
