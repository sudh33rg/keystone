...
export function analyzeDeadCode(intelligence: RepoIntelligence, graph: RepositoryGraphAnalysis, semantic: TypeScriptSemanticResult): DeadCodeCandidate[] {
  ...
  return intelligence.symbols.filter(symbol =>
    ['function', 'class'].includes(symbol.kind) &&
    /^(?:typescript|typescriptreact|javascript|javascriptreact)$/.test(intelligence.files.find(file => file.path === symbol.filePath)?.language ?? '') &&
    ...
    !/^(main|constructor|render|setup|teardown|beforeEach|afterEach|if|for|while|switch|catch|return|throw|new)$/i.test(symbol.name)
  ).map(symbol => ({
    filePath: symbol.filePath, line: symbol.line, name: symbol.name, kind: symbol.kind,
    confidence: 0.8,
    reasons: ['not exported', 'no bound incoming call, callback, inheritance, API, test, or entry-point evidence']
  })).sort((a, b) => b.confidence - a.confidence || a.filePath.localeCompare(b.filePath) || a.line - b.line);
}
