...
export function evaluateIntelligenceHealth(
  ...
  const mappedTests = tests.filter(
    (test) => test.targetFile && filePaths.has(test.targetFile)
  ).length;
  ...
  const checks = [
    check(
      ...
      ratio(stages.filter((stage) => stage.status === "complete").length, stages.length),
      `${stages.filter((stage) => stage.status === "complete").length}/${stages.length} stages complete`
    ),
    check(
      "coverage",
      ingestion.completedWithoutFileCap,
      ingestion.completedWithoutFileCap ? 1 : 0.5,
      ingestion.completedWithoutFileCap
        ? "All discovered supported files were processed without a file cap"
        : "Discovery did not complete"
    ),
    check(
      "local-imports",
      local.length === 0 || resolvedLocal === local.length,
      local.length === 0 ? 1 : ratio(resolvedLocal, local.length),
      `${resolvedLocal}/${local.length} local imports resolve to indexed files`
    ),
    check(
      "test-mapping",
      tests.length === 0 || mappedTests === tests.length,
      tests.length === 0 ? 1 : ratio(mappedTests, tests.length),
      `${mappedTests}/${tests.length} tests map to indexed source files`
    ),
    check(
      "cpg-coverage",
      eligible === 0 || ingestion.cpgIndexedFiles === eligible,
      eligible === 0 ? 1 : ratio(ingestion.cpgIndexedFiles, eligible),
      `${ingestion.cpgIndexedFiles}/${eligible} eligible files received CPG projections`
    )
  ];
  ...
  if (cpgStage) {
    ...
    checks.push(
      check(
        ...
        configuredDiagnostics === 0 ? 1 : 0,
        `${configuredDiagnostics} compiler syntax/config diagnostic(s) were observed in configured TypeScript projects; full typechecking belongs to validation`
      )
    );
    ...
    const fallbackScore =
      fallbackFiles === 0
        ? 1
        : Math.max(0, 1 - fallbackDiagnostics / Math.max(fallbackFiles * 2, 1));
    checks.push(
      check(
        "fallback-semantic-diagnostics",
        fallbackDiagnostics === 0,
        fallbackScore,
        `${fallbackDiagnostics} compiler syntax/config diagnostic(s) were observed across ${fallbackFiles} files without a dedicated tsconfig`
      )
    );
  }
  if (runtime) {
    checks.push(
      check(
        ...
        runtime.degraded
          ? runtime.warnings.join(" ") || "Runtime evidence is unavailable"
          : `${runtime.evidence.length} runtime evidence signal(s) mapped`
      )
    );
  }
  ...
  return {
    status:
      score < 50 || stages.some((stage) => stage.status === "failed")
        ? "unhealthy"
        : score < 90
          ? "degraded"
          : "healthy",
