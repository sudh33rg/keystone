...
export interface RepositoryEvolution {
  ...
  readonly coupling: ReadonlyArray<{ fileA: string; fileB: string; commits: number; strength: number }>;
  readonly commitsAnalyzed: number;
  readonly degraded: boolean;
  readonly warnings: readonly string[];
}

export async function buildRepositoryEvolution(root: string, incremental: IncrementalUpdatePlan, maxCommits = 200): Promise<RepositoryEvolution> {
  ...
  try {
    ...
    for (const files of commits) {
      ...
      for (let left = 0; left < bounded.length; left += 1) for (let right = left + 1; right < bounded.length; right += 1) {
        const key = `${bounded[left]}\0${bounded[right]}`; pairCounts.set(key, (pairCounts.get(key) ?? 0) + 1);
      }
    }
    const coupling = [...pairCounts.entries()].filter(([, count]) => count >= 2).map(([key, count]) => {
      const [fileA, fileB] = key.split('\0');
      return { fileA, fileB, commits: count, strength: count / Math.max(fileCommits.get(fileA) ?? count, fileCommits.get(fileB) ?? count) };
    }).sort((a, b) => b.strength - a.strength || b.commits - a.commits).slice(0, 500);
    return { changes, coupling, commitsAnalyzed: commits.length, degraded: false, warnings: [] };
  } catch (error) {
    return { changes, coupling: [], commitsAnalyzed: 0, degraded: true, warnings: [`Git coupling unavailable: ${error instanceof Error ? error.message : String(error)}`] };
  }
}
...
function parseCommits(output: string): string[][] {
  ...
  for (const raw of output.split(/\r?\n/)) {
    const line = raw.trim();
    if (line === '__KEYSTONE_COMMIT__') { if (current) commits.push(current); current = []; }
