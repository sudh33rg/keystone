...
export function planIncrementalUpdate(
  ...
  if (!previous?.files.length) {
    return plan(
      "full",
      current.files.map((file) => ({ path: file.path, kind: "added" })),
      "No previous fingerprint store is available."
    );
  }
  ...
  for (const file of previous.files)
    if (!newFiles.has(file.path)) changes.push({ path: file.path, kind: "deleted" });
  changes.sort((a, b) => a.path.localeCompare(b.path));
  const meaningful = changes.filter((change) => change.kind !== "unchanged");
  const structural = meaningful.filter(
    (change) => change.kind === "structural" || change.kind === "added" || change.kind === "deleted"
  );
  if (!meaningful.length)
    return plan("skip", changes, "All indexed file content hashes are unchanged.");
  if (!structural.length)
    return plan(
      "file-local",
      changes,
      `${meaningful.length} file(s) changed without altering extracted structure.`
    );
  const ratio = structural.length / Math.max(current.files.length, 1);
  if (structural.length > 30 || ratio > 0.5)
    return plan("full", changes, `${structural.length} structural changes require a full rebuild.`);
  if (structural.length > 10)
    return plan(
      "architecture",
      changes,
      `${structural.length} structural changes require architecture refresh.`
    );
  return plan(
    "graph",
    changes,
    `${structural.length} structural change(s) require graph relationship refresh.`
  );
}
...
function classifyFile(previous: RepoFile, current: RepoFile): FileChangeKind {
  ...
  if (
    ...
    previous.structuralHash === current.structuralHash
  )
...
function plan(
  action: IngestionUpdateAction,
  changes: readonly FileChange[],
  reason: string
): IncrementalUpdatePlan {
  const filesToAnalyze = changes
    .filter((change) => change.kind !== "unchanged" && change.kind !== "deleted")
