...
export interface RepositoryGraphAnalysis {
  ...
  readonly hubs: ReadonlyArray<{ path: string; incoming: number; outgoing: number; degree: number; evidence: readonly EvidenceMetadata[] }>;
  readonly entryPoints: readonly string[];
  readonly orphanSourceFiles: readonly string[];
  readonly cycles: ReadonlyArray<readonly string[]>;
  readonly communities: ReadonlyArray<{ id: string; files: readonly string[]; internalEdges: number; evidence: readonly EvidenceMetadata[] }>;
  readonly flows: ReadonlyArray<{ entryPoint: string; files: readonly string[]; depth: number; evidence: readonly EvidenceMetadata[] }>;
  impactedBy(changedFiles: readonly string[], maxDepth?: number): { files: string[]; tests: string[]; depth: number };
}
...
export function analyzeRepositoryGraph(intelligence: RepoIntelligence): RepositoryGraphAnalysis {
  ...
  const localEdges = intelligence.dependencies
    .filter(edge => (edge.kind === 'local' || edge.kind === 'import') && files.has(edge.from) && files.has(edge.to))
  ...
  })).filter(item => item.degree > 0).sort((a, b) => b.degree - a.degree || a.path.localeCompare(b.path)).slice(0, 20);
  ...
  const flows = entryPoints.map(entryPoint => traceFlow(entryPoint, outgoing, localEdges, 8, 100));
  ...
  return {
    ...
    impactedBy(changedFiles, maxDepth = 6) {
      ...
      while (frontier.length && depth < maxDepth && seen.size < 2000) {
        ...
        for (const file of frontier) {
          for (const dependent of incoming.get(file) ?? []) {
            if (!seen.has(dependent)) { seen.add(dependent); next.push(dependent); }
          }
        }
        frontier = next;
      }
      ...
      const tests = intelligence.tests
        .filter(test => impacted.includes(test.testFile) || Boolean(test.targetFile && impacted.includes(test.targetFile)))
...
function findCommunities(files: Set<string>, edges: ReadonlyArray<RepositoryGraphEdge>): Array<{ id: string; files: readonly string[]; internalEdges: number; evidence: readonly EvidenceMetadata[] }> {
  const undirected = new Map<string, Set<string>>();
  for (const edge of edges) {
    const left = undirected.get(edge.from) ?? new Set<string>(); left.add(edge.to); undirected.set(edge.from, left);
    const right = undirected.get(edge.to) ?? new Set<string>(); right.add(edge.from); undirected.set(edge.to, right);
  }
  const seen = new Set<string>();
  const result: Array<{ id: string; files: readonly string[]; internalEdges: number; evidence: readonly EvidenceMetadata[] }> = [];
  for (const file of [...files].sort()) {
    ...
    while (queue.length) {
      ...
      for (const neighbor of undirected.get(current) ?? []) if (!seen.has(neighbor)) { seen.add(neighbor); queue.push(neighbor); }
    }
    component.sort();
    const members = new Set(component);
    const internalEdges = edges.filter(edge => members.has(edge.from) && members.has(edge.to));
    result.push({ id: `community:${component[0]}`, files: component, internalEdges: internalEdges.length, evidence: edgeEvidence(internalEdges) });
  }
  return result.sort((a, b) => b.files.length - a.files.length || a.id.localeCompare(b.id));
}

function traceFlow(entryPoint: string, outgoing: Map<string, Set<string>>, edges: ReadonlyArray<RepositoryGraphEdge>, maxDepth: number, limit: number) {
  ...
  while (queue.length && seen.size < limit) {
    ...
    for (const target of outgoing.get(current.file) ?? []) {
      if (!seen.has(target)) { seen.add(target); queue.push({ file: target, depth: current.depth + 1 }); }
    }
  }
  return { entryPoint, files: [...seen].sort(), depth: reachedDepth, evidence: edgeEvidence(edges.filter(edge => seen.has(edge.from) && seen.has(edge.to))) };
}
...
function edgeEvidence(edges: readonly RepositoryGraphEdge[]): EvidenceMetadata[] {
  ...
  for (const edge of edges) {
    ...
    const key = [edge.evidence.source, edge.evidence.evidencePath ?? '', edge.evidence.evidenceLine ?? '', edge.evidence.extractorVersion].join('|');
...
function findCycles(files: Set<string>, outgoing: Map<string, Set<string>>): Array<readonly string[]> {
  ...
  const visit = (node: string): void => {
    ...
    for (const target of outgoing.get(node) ?? []) {
      if (!indexByNode.has(target)) { visit(target); lowLink.set(node, Math.min(lowLink.get(node)!, lowLink.get(target)!)); }
