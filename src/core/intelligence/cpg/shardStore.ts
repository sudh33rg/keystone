...
export class CpgShardStore {
  ...
  async put(graph: CodePropertyGraph): Promise<void> {
    ...
    if (existing?.contentHash === graph.contentHash && await exists(path.join(this.root, existing.shard))) {
    ...
    this.current.set(graph.sourcePath, { sourcePath: graph.sourcePath, shard, contentHash: graph.contentHash, nodeCount: graph.nodes.length, edgeCount: graph.edges.length, capabilities: graph.capabilities });
    this.written += 1;
  }

  async finalize(): Promise<{ manifest: CpgShardManifest; reused: number; written: number; deleted: number }> {
    const previous = await this.previousManifest();
    let deleted = 0;
    for (const [sourcePath, entry] of Object.entries(previous?.files ?? {})) {
      if (!this.current.has(sourcePath)) { await fs.rm(path.join(this.root, entry.shard), { force: true }); deleted += 1; }
    }
    const manifest: CpgShardManifest = { version: 1, generatedAt: new Date().toISOString(), files: Object.fromEntries([...this.current.entries()].sort(([a], [b]) => a.localeCompare(b))) };
    await fs.mkdir(this.root, { recursive: true });
  ...
  async get(sourcePath: string): Promise<CodePropertyGraph | undefined> {
    ...
    try { return JSON.parse((await gunzipAsync(await fs.readFile(path.join(this.root, entry.shard)))).toString('utf8')) as CodePropertyGraph; } catch { return undefined; }
  }

  async manifest(): Promise<CpgShardManifest | undefined> {
    try { return JSON.parse(await fs.readFile(path.join(this.root, 'manifest.json'), 'utf8')) as CpgShardManifest; } catch { return undefined; }
  }

  private async previousManifest(): Promise<CpgShardManifest | undefined> {
    if (!this.priorLoaded) { this.priorManifest = await this.manifest(); this.priorLoaded = true; }
