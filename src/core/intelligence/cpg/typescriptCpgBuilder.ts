...
export function buildTypeScriptCpg(input: TypeScriptCpgInput): CodePropertyGraph {
  ...
  const linkedEdges = edges.map((item) => ({
    ...item,
...
function addControlDependence(
  sourceFile: ts.SourceFile,
  ids: Map<ts.Node, string>,
  edges: CpgEdge[]
): void {
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isIfStatement(node)) {
      ...
      for (const statement of node.statements) {
        ...
        if (source && target)
          edges.push(
            edge(source, target, "cdg", { branch: ts.isDefaultClause(node) ? "default" : "case" })
          );
      }
    }
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isIdentifier(node) && !declarationNames.has(node) && isValueReference(node)) {
      ...
      const before = candidates.filter(
        (candidate) => candidate.getStart(sourceFile) <= node.getStart(sourceFile)
      );
      const definition = (before.length ? before : candidates).sort(
        (a, b) => Math.abs(node.pos - a.pos) - Math.abs(node.pos - b.pos)
      )[0];
      const source = definition ? ids.get(definition) : undefined;
      const target = ids.get(node);
      if (source && target)
        edges.push(edge(source, target, "dfg", { variable: node.text, resolution: "lexical" }));
    }
    ts.forEachChild(node, visit);
  };
  visit(sourceFile);
