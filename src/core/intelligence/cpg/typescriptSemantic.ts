...
export function analyzeTypeScriptProject(workspaceRoot: string, sourcePaths: readonly string[]): TypeScriptSemanticResult {
  ...
  for (const configPath of configPaths) {
    ...
    const rootNames = parsed.fileNames.filter(file => eligible.has(normalize(file)) && !assigned.has(normalize(file)));
    if (!rootNames.length) continue;
    rootNames.forEach(file => assigned.add(normalize(file)));
    programs.push({ program: ts.createProgram({ rootNames, options: { ...parsed.options, noEmit: true, skipLibCheck: true } }), config: configPath });
  }
  const remaining = [...eligible.values()].filter(file => !assigned.has(normalize(file)));
  if (remaining.length) programs.push({ program: ts.createProgram({
    rootNames: remaining,
    options: {
      allowJs: true,
      checkJs: false,
    ...
    for (const diagnostic of emittedDiagnostics) {
      ...
      if (diagnosticExamples.length < 20) {
        const location = diagnostic.file && diagnostic.start !== undefined
          ...
          : config ? relative(workspaceRoot, config) : 'fallback';
        diagnosticExamples.push(`${location} ${code}: ${ts.flattenDiagnosticMessageText(diagnostic.messageText, ' ')}`);
      }
    }
    diagnostics += programDiagnostics;
    if (config) { configuredDiagnostics += programDiagnostics; configuredFiles += program.getRootFileNames().length; }
    else { fallbackDiagnostics += programDiagnostics; fallbackFiles += program.getRootFileNames().length; }
      ...
      if ((ts.isClassDeclaration(node) || ts.isInterfaceDeclaration(node)) && node.name) {
        for (const clause of node.heritageClauses ?? []) {
          for (const heritageType of clause.types) {
            ...
            const declaration = symbol?.declarations?.find(item => inside(workspaceRoot, item.getSourceFile().fileName));
            ...
            relationships.push({
              ...
              sourcePath: relative(workspaceRoot, sourceFile.fileName), sourceLine: sourcePosition.line + 1, sourceName: node.name.text,
              targetPath: relative(workspaceRoot, targetFile.fileName), targetLine: targetPosition.line + 1, targetName: symbol?.getName() ?? heritageType.getText(sourceFile),
              confidence: 1
            });
          }
        }
        if (ts.isClassDeclaration(node)) {
          ...
          for (const member of node.members) {
            ...
            for (const base of checker.getBaseTypes(classType) ?? []) {
              ...
              if (!declaration || !inside(workspaceRoot, declaration.getSourceFile().fileName)) continue;
              const targetFile = declaration.getSourceFile();
              const sourcePosition = sourceFile.getLineAndCharacterOfPosition(member.getStart(sourceFile));
              const targetPosition = targetFile.getLineAndCharacterOfPosition(declaration.getStart(targetFile));
              relationships.push({
                kind: 'overrides', sourcePath: relative(workspaceRoot, sourceFile.fileName), sourceLine: sourcePosition.line + 1,
                sourceName: `${node.name.text}.${member.name.text}`, targetPath: relative(workspaceRoot, targetFile.fileName), targetLine: targetPosition.line + 1,
                targetName: `${containerName(declaration)}.${baseMember?.getName() ?? member.name.text}`, confidence: 1
              });
            }
          }
        }
      ...
      if (ts.isCallExpression(node) || ts.isNewExpression(node)) {
        ...
        if (ts.isCallExpression(node)) {
          for (const argument of node.arguments) {
            ...
            if (!callbackDeclaration || !inside(workspaceRoot, callbackDeclaration.getSourceFile().fileName)) continue;
            const targetFile = callbackDeclaration.getSourceFile();
            const sourcePosition = sourceFile.getLineAndCharacterOfPosition(node.getStart(sourceFile));
            const targetPosition = targetFile.getLineAndCharacterOfPosition(callbackDeclaration.getStart(targetFile));
            callbacks.push({
              registrar: node.expression.getText(sourceFile), callback: callbackName(argument, callbackDeclaration),
              sourcePath: relative(workspaceRoot, sourceFile.fileName), sourceLine: sourcePosition.line + 1,
              targetPath: relative(workspaceRoot, targetFile.fileName), targetLine: targetPosition.line + 1,
              confidence: ts.isArrowFunction(argument) || ts.isFunctionExpression(argument) ? 1 : 0.9
            });
          }
        }
      }
  ...
  const usedConfigs = programs.flatMap(item => item.config ? [relative(workspaceRoot, item.config)] : []);
  return {
...
function callbackTarget(argument: ts.Expression, checker: ts.TypeChecker): ts.Declaration | undefined {
...
function callbackName(argument: ts.Expression, declaration: ts.Declaration): string {
  if (ts.isIdentifier(argument) || ts.isPropertyAccessExpression(argument)) return argument.getText();
...
function dedupeCallbacks(items: SemanticCallbackEdge[]): SemanticCallbackEdge[] {
  return [...new Map(items.map(item => [`${item.sourcePath}:${item.sourceLine}:${item.registrar}:${item.targetPath}:${item.targetLine}:${item.callback}`, item])).values()];
}

function containerName(node: ts.Node): string {
  let current: ts.Node | undefined = node.parent;
  while (current) {
    if ((ts.isClassDeclaration(current) || ts.isInterfaceDeclaration(current)) && current.name) return current.name.text;
...
function dedupeRelationships(items: SemanticTypeRelationship[]): SemanticTypeRelationship[] {
  return [...new Map(items.map(item => [`${item.kind}:${item.sourcePath}:${item.sourceLine}:${item.targetPath}:${item.targetLine}`, item])).values()];
}

function dedupe(calls: SemanticCallEdge[]): SemanticCallEdge[] {
  return [...new Map(calls.map(call => [`${call.sourcePath}:${call.sourceLine}:${call.targetPath}:${call.targetLine}`, call])).values()];
}
