...
import type { KeystoneKnowledgeRelationship, KeystoneKnowledgeUnit, KeystoneOkfSnapshot } from './types';
...
export interface OkfBundleValidationIssue { readonly file: string; readonly message: string; }
export interface OkfBundleValidationResult { readonly valid: boolean; readonly concepts: number; readonly issues: readonly OkfBundleValidationIssue[]; }

export async function writePortableOkfBundle(workspaceRoot: string, snapshot: KeystoneOkfSnapshot, targetRoot: string): Promise<OkfBundleWriteResult> {
  ...
  try { await fs.rename(targetRoot, previous); } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error; }
  await fs.rename(candidate, targetRoot);
  await fs.rm(previous, { recursive: true, force: true });
  return { root: targetRoot, concepts: activeUnits.length, files: activeUnits.length + groups.size + 3, digest, validation };
}
...
function renderConcept(
  ...
  const evidence = unit.provenance.evidenceIds.map(id => evidenceById.get(id)).filter((value): value is NonNullable<typeof value> => Boolean(value));
  ...
  const lines: string[] = [
    ...
    ...(evidence.length ? ['sources:', ...evidence.flatMap(item => {
  ...
  const relations = [...outgoing.map(item => ({ direction: 'outgoing' as const, item })), ...incoming.map(item => ({ direction: 'incoming' as const, item }))];
  if (relations.length) {
    lines.push('', '## Relationships', '');
    for (const relation of relations.sort((a, b) => `${a.item.kind}:${a.item.id}`.localeCompare(`${b.item.kind}:${b.item.id}`))) {
      const peerId = relation.direction === 'outgoing' ? relation.item.targetId : relation.item.sourceId;
  ...
  if (evidence.length) {
    ...
    for (const item of evidence) {
      const range = item.source.startLine ? `:${item.source.startLine}${item.source.endLine && item.source.endLine !== item.source.startLine ? `-${item.source.endLine}` : ''}` : '';
      const id = sourceIds.get(item.id)!;
      lines.push(`- \`${item.source.workspaceRelativePath}${range}\` — ${item.method}; extractor \`${item.extractor}@${item.extractorVersion}\`; confidence ${unit.confidence.score.toFixed(2)}.[^${id}]`);
    }
    lines.push('');
    evidence.forEach(item => {
      const id = sourceIds.get(item.id)!;
      lines.push(`[^${id}]: ${sourceTitle(item)} — ${workspaceCitation(item.source.workspaceRelativePath, item.source.startLine)}`);
    });
  }
  return `${lines.join('\n').trim()}\n`;
}

function renderRootIndex(snapshot: KeystoneOkfSnapshot, groups: ReadonlyMap<string, readonly KeystoneKnowledgeUnit[]>, conceptPaths: ReadonlyMap<string, string>): string {
  const lines = [
    '---',
  ...
  for (const [kind, units] of [...groups.entries()].sort(([a], [b]) => a.localeCompare(b))) lines.push(`- [${typeLabel(kind)}](${pluralize(kind)}/index.md) — ${units.length}`);
  const repositories = snapshot.units.filter(item => item.kind === 'repository' && item.lifecycle !== 'deleted');
...
function renderKindIndex(kind: string, units: readonly KeystoneKnowledgeUnit[], conceptPaths: ReadonlyMap<string, string>): string {
  const directory = pluralize(kind);
  const lines = [`# ${typeLabel(kind)}`, '', `Concepts of type **${typeLabel(kind)}** generated from repository evidence.`, ''];
  for (const unit of [...units].sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    lines.push(`- [${unit.name}](${relative.startsWith('.') ? relative : `./${relative}`}) — ${unit.description ?? humanSummary(unit)}`);
  }
  return `${lines.join('\n')}\n`;
}
...
function renderProperties(properties: Readonly<Record<string, unknown>>): string {
  ...
  return entries.map(([key, value]) => `- **${humanize(key)}:** ${markdownValue(value)}`).join('\n');
}
...
function humanSummary(unit: KeystoneKnowledgeUnit): string { return `${typeLabel(unit.kind)} discovered by Keystone with ${unit.confidence.level} confidence (${unit.confidence.score.toFixed(2)}).`; }
function resourceFor(unit: KeystoneKnowledgeUnit): string | undefined {
  const filePath = typeof unit.properties.filePath === 'string' ? unit.properties.filePath : typeof unit.properties.path === 'string' ? unit.properties.path : undefined;
  return filePath ? `workspace:///${slash(filePath)}` : `keystone://knowledge/${unit.id}`;
}
function workspaceCitation(filePath: string, line?: number): string { return `workspace:///${slash(filePath)}${line ? `#L${line}` : ''}`; }
function sourceId(value: string): string { return `source-${slug(value).slice(0, 48)}`; }
function sourceTitle(item: KeystoneOkfSnapshot['evidence'][number]): string { return `${item.source.workspaceRelativePath}${item.source.symbol ? ` · ${item.source.symbol}` : ''}`; }
function actorSegment(value: string): string { return value.toLowerCase().replace(/[^a-z0-9._-]+/g, '-').replace(/^-|-$/g, '') || 'keystone-extractor'; }
function conceptRelativePath(unit: KeystoneKnowledgeUnit): string { return `${pluralize(unit.kind)}/${slug(unit.name)}-${unit.id.slice(-10)}.md`; }
function pluralize(kind: string): string { return `${kind.replace(/-/g, '_')}s`; }
function typeLabel(kind: string): string { return `Keystone ${humanize(kind)}`; }
function humanize(value: string): string { return value.replace(/[-_]/g, ' ').replace(/\b\w/g, letter => letter.toUpperCase()); }
function slug(value: string): string { return value.toLowerCase().normalize('NFKD').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').slice(0, 70) || 'concept'; }
...
function groupRelationships(items: readonly KeystoneKnowledgeRelationship[], key: 'sourceId' | 'targetId'): Map<string, KeystoneKnowledgeRelationship[]> {
...
async function digestDirectory(root: string): Promise<string> {
  ...
  for (const file of await walk(root, () => true)) {
    hash.update(slash(path.relative(root, file))).update('\0').update(await fs.readFile(file)).update('\0');
  }
