...
export class SemanticDeltaBuilder {
  merge(base: IntelligenceSnapshot, result: SemanticProjectResult): IntelligenceSnapshot {
    ...
    for (const [fileId, contribution] of next) {
      ...
      if (!prior || prior.sourceHash !== contribution.sourceHash || prior.parserVersion !== contribution.parserVersion) {
        affected.add(fileId);
        entityReplacementFiles.add(fileId);
      }
    }
    for (const fileId of previous.keys()) if (!next.has(fileId)) { affected.add(fileId); entityReplacementFiles.add(fileId); }
    let expanded = true;
    while (expanded) {
      expanded = false;
    ...
    const removedEntityIds = new Set(base.symbols.filter((item) => replaceEntityFiles.has(item.ownerFileId ?? item.fileId)).map((item) => item.id));
    ...
    return normalizeGraph({
      ...
      symbols: [...base.symbols.filter((item) => !removedEntityIds.has(item.id)), ...result.entities.filter((item) => addEntityFiles.has(item.ownerFileId ?? item.fileId))],
  ...
  async mergeYielding(base: IntelligenceSnapshot, result: SemanticProjectResult, signal?: AbortSignal): Promise<IntelligenceSnapshot> {
    ...
    const replaceFiles = hasPriorSemantic ? affected : new Set(next.keys());
    const replaceEntityFiles = hasPriorSemantic ? entityReplacementFiles : new Set(next.keys());
    const removedEntityIds = new Set((await filterYielding(base.symbols, (item) => replaceEntityFiles.has(item.ownerFileId ?? item.fileId), signal)).map((item) => item.id));
    ...
    return normalizeGraph({
      ...
      symbols: [...await filterYielding(base.symbols, (item) => !removedEntityIds.has(item.id), signal), ...await filterYielding(result.entities, (item) => addEntityFiles.has(item.ownerFileId ?? item.fileId), signal)],
...
function normalizeGraph(snapshot: IntelligenceSnapshot): IntelligenceSnapshot {
  ...
  const relationships = candidateRelationships.filter((item) => entityIds.has(item.sourceId) && entityIds.has(item.targetId));
  ...
  const diagnostics = snapshot.diagnostics.filter((item) => item.code !== "semantic-dangling-relationship");
  if (dropped > 0) diagnostics.push({ code: "semantic-dangling-relationship", severity: "warning", message: `${dropped} semantic relationship(s) were omitted because an endpoint was not present in this generation.` });
  return { ...snapshot, symbols, relationships, evidence: uniqueById(snapshot.evidence).filter((item) => subjectIds.has(item.subjectId)), diagnostics };
}
