...
parentPort?.on("message", async (message: unknown) => {
  if (!message || typeof message !== "object" || !("id" in message) || typeof message.id !== "number" || !("type" in message)) return;
  ...
  try {
    ...
    if (!cancelled.delete(message.id)) parentPort?.postMessage({ id: message.id, ok: true, result });
  } catch (cause) {
    parentPort?.postMessage({ id: message.id, ok: false, error: cause instanceof Error ? cause.message : String(cause) });
  }
});
