...
export function analyzeLanguageFile(filePath: string, text: string): LanguageAnalysisResult {
  const language =
    registry.identify(filePath) ?? identifySpecialFile(filePath) ?? UNIVERSAL_TEXT_DEFINITION;
  ...
  lines.forEach((line, index) => {
    ...
    for (const match of lineSymbols) {
      ...
      if (!seenSymbols.has(key)) {
        ...
        symbols.push({
          ...
          filePath,
          line: lineNo,
          exportStatus: exportStatus(language.id, line)
        });
      }
    ...
    if (supportsControlFlow &&
      ...
      const kind =
        stripStrings(line).match(
          /\b(if|else\s+if|for|foreach|while|switch|case|catch|match|when|try|except|guard)\b/
        )?.[1] ?? "branch";
    ...
    for (const relationship of typeRelationshipMatches(language.id, line, lineNo))
      typeRelationships.push(relationship);
    const api = apiMatch(language.id, line, filePath, lineNo);
  ...
  return {
    ...
    tests,
    apis,
    syntaxTree: parseStructuralSyntax(language.id, text)
  };
}
...
function symbolMatches(id: string, line: string): Array<Pick<CodeSymbol, "name" | "kind">> {
  ...
  for (const [kind, pattern] of patterns) {
    ...
    const name = match?.groups?.name ?? match?.groups?.name2 ?? match?.[1];
...
function commonPatterns(id: string): Array<[CodeSymbol["kind"], RegExp]> {
  ...
  const byId: Record<string, Array<[CodeSymbol["kind"], RegExp]>> = {
    ...
    ruby: [["function", /^\s*def\s+(?<name>[\w!?=]+)/]],
    php: [["function", /\bfunction\s+(?<name>\w+)/]],
    go: [["function", /^\s*func\s+(?:\([^)]*\)\s*)?(?<name>\w+)/]],
    rust: [["function", /\bfn\s+(?<name>\w+)/]],
    swift: [["function", /\bfunc\s+(?<name>\w+)/]],
    kotlin: [["function", /\bfun\s+(?<name>\w+)/]],
    dart: [["function", /\b(?:void|[A-Za-z_]\w*(?:<[^>]+>)?)\s+(?<name>\w+)\s*\(/]],
    java: [
      [
        "method",
        /\b(?:public|private|protected|static|final|synchronized|abstract|native|\s)+[\w<>,?\[\]]+\s+(?<name>\w+)\s*\(/
      ]
    ],
    csharp: [
      [
    ...
    erlang: [["function", /^\s*(?<name>[a-z][\w@]*)\s*\([^)]*\)\s*->/]],
    haskell: [["function", /^\s*(?<name>[a-z][\w']*)\s+[^=]*=/]],
    r: [["function", /^\s*(?<name>[A-Za-z.]\w*)\s*<-\s*function\s*\(/]],
    shell: [["function", /^\s*(?:function\s+)?(?<name>[A-Za-z_]\w*)\s*\(\)\s*\{/]],
    powershell: [["function", /^\s*function\s+(?<name>[\w-]+)/i]],
    javascript: [
      [
        "function",
        /\b(?:function\s+|const\s+|let\s+|var\s+)(?<name>[A-Za-z_$][\w$]*)\s*(?:=\s*(?:async\s*)?\([^)]*\)\s*=>|\()/
      ],
      [
        "method",
        /^\s*(?:(?:public|private|protected|static|async|get|set|override)\s+)*(?<name>[A-Za-z_$][\w$]*)\s*\([^)]*\)\s*(?::[^={]+)?\s*\{/
      ]
    ],
    typescript: [
      [
      ...
      [
        "method",
        /^\s*(?:(?:public|private|protected|static|async|readonly|abstract|override|get|set)\s+)*(?<name>[A-Za-z_$][\w$]*)\s*\([^)]*\)\s*(?::[^={]+)?\s*\{/
      ]
    ],
    sql: [
      [
        "type",
        /\bCREATE\s+(?:OR\s+REPLACE\s+)?(?:TABLE|VIEW|FUNCTION|PROCEDURE|TRIGGER)\s+(?:IF\s+NOT\s+EXISTS\s+)?["`[]?(?<name>[\w.]+)/i
      ]
    ],
    graphql: [["type", /^\s*(?:type|input|interface|enum|scalar|union|directive)\s+(?<name>\w+)/]],
    protobuf: [["type", /^\s*(?:message|enum|service)\s+(?<name>\w+)/]],
    terraform: [
      [
        "constant",
        /^\s*(?:resource|data|module|variable|output|provider)\s+"[^"]+"(?:\s+"(?<name>[^"]+)")?/
      ]
    ],
    dockerfile: [["constant", /^\s*FROM\s+\S+\s+AS\s+(?<name>[A-Za-z0-9_.-]+)/i]],
    make: [["function", /^\s*(?<name>[A-Za-z0-9_.-]+)\s*:(?!=)/]],
    cmake: [
      [
        "function",
        /^\s*(?:function|macro|add_executable|add_library)\s*\(\s*(?<name>[A-Za-z0-9_.-]+)/i
      ]
    ],
    gradle: [["function", /^\s*(?:task|register)\s*\(?\s*["']?(?<name>[A-Za-z0-9_.-]+)/]],
    maven: [["constant", /<artifactId>(?<name>[^<]+)<\/artifactId>/]],
    kubernetes: [["type", /^\s*kind\s*:\s*(?<name>[A-Za-z0-9_.-]+)/]],
    markdown: [["type", /^\s*#{1,6}\s+(?<name>.+?)\s*$/]],
    html: [["type", /<(?<name>[A-Za-z][\w:-]*)\b/]],
    xml: [["type", /<(?<name>[A-Za-z][\w:-]*)\b/]],
    css: [["type", /^\s*(?<name>[^@][^{]+)\s*\{/]],
    json: [["constant", /^\s*"(?<name>[^"]+)"\s*:/]],
    yaml: [["constant", /^\s*(?<name>[A-Za-z0-9_.-]+)\s*:/]],
    toml: [["constant", /^\s*(?:\[(?<name>[^\]]+)\]|(?<name2>[A-Za-z0-9_.-]+)\s*=)/]]
  };
  return [
    ...shared,
    ...(byId[id] ?? [
      ["function", /\b(?:function|func|fn|def|sub|procedure)\s+(?<name>[A-Za-z_$][\w$-]*)/]
    ])
  ];
}

function typeRelationshipMatches(
  id: string,
  line: string,
  lineNo: number
): LanguageAnalysisResult["typeRelationships"] {
  ...
  const add = (
    ...
    for (const target of targets
      .split(/\s*,\s*/)
      .map((value) => value.replace(/^(?:public|private|protected|virtual)\s+/, "").trim())
      .filter(Boolean)) {
      const name = target.match(/[A-Za-z_$][\w$.:]*/)?.[0];
      if (name)
        out.push({ source, target: name.split(/::|\./).at(-1) ?? name, kind, line: lineNo });
    }
  };
  const standard = line.match(
    /\b(?:class|interface|struct|record|object|trait)\s+([A-Za-z_$][\w$]*)(?:\s+extends\s+([^\s{]+(?:\s*,\s*[^\s{]+)*))?(?:\s+implements\s+([^\{]+))?/
  );
  if (standard) {
    add(standard[1], standard[2], "extends");
    add(standard[1], standard[3], "implements");
  }
  if (id === "python") {
    const match = line.match(/^\s*class\s+([A-Za-z_]\w*)\s*\(([^)]+)\)/);
    if (match) add(match[1], match[2], "extends");
  }
  if (id === "cpp" || id === "c") {
    const match = line.match(/\bclass\s+([A-Za-z_]\w*)\s*:\s*([^\{]+)/);
    if (match) add(match[1], match[2], "extends");
  }
  if (id === "objective-c") {
    const match = line.match(/@interface\s+([A-Za-z_]\w*)\s*:\s*([A-Za-z_]\w*)/);
    if (match) add(match[1], match[2], "extends");
  }
  if (id === "csharp" || id === "kotlin") {
    const match = line.match(/\bclass\s+([A-Za-z_]\w*)[^:{]*:\s*([^\{]+)/);
    if (match) {
      const targets = match[2].split(",");
      add(match[1], targets.shift(), "extends");
      add(match[1], targets.join(","), "implements");
    }
  }
  if (id === "scala") {
    const match = line.match(/\bclass\s+([A-Za-z_]\w*)[^\{]*\bextends\s+([A-Za-z_]\w*)(.*)/);
    if (match) {
      add(match[1], match[2], "extends");
      add(
        match[1],
        [...match[3].matchAll(/\bwith\s+([A-Za-z_]\w*)/g)].map((item) => item[1]).join(","),
        "implements"
      );
    }
  }
  if (id === "ruby") {
    const match = line.match(/\bclass\s+([A-Za-z_]\w*)\s*<\s*([A-Za-z_:]\w*)/);
    if (match) add(match[1], match[2], "extends");
  }
  if (id === "rust") {
    const match = line.match(/\bimpl(?:<[^>]+>)?\s+([A-Za-z_]\w*)\s+for\s+([A-Za-z_]\w*)/);
    if (match) add(match[2], match[1], "implements");
  }
  if (id === "swift") {
    const match = line.match(/\b(?:class|struct|enum)\s+([A-Za-z_]\w*)\s*:\s*([^\{]+)/);
    if (match) add(match[1], match[2], "implements");
  }
  if (id === "haskell") {
    const match = line.match(/^\s*instance\s+([A-Z]\w*)\s+([A-Z]\w*)/);
    if (match) add(match[2], match[1], "implements");
  }
  if (id === "elixir") {
    const match = line.match(/^\s*defimpl\s+([A-Za-z_.]\w*),\s*for:\s*([A-Za-z_.]\w*)/);
    if (match) add(match[2], match[1], "implements");
  }
  return out;
}
...
function callMatches(_id: string, line: string): string[] {
...
function apiMatch(
  ...
  const direct = line.match(
    /\b(?:app|router|server|route)\s*(?:\.|->)\s*(get|post|put|patch|delete|options|head)\s*\(\s*["'`]([^"'`]+)/i
  );
  if (direct) return { method: direct[1].toUpperCase(), path: direct[2], filePath, line: lineNo };
  const annotation = line.match(
    /@(Get|Post|Put|Patch|Delete|RequestMapping)(?:Mapping)?\s*\(\s*["']([^"']+)/i
  );
  if (annotation)
    return {
      method: annotation[1].replace(/RequestMapping/i, "ANY").toUpperCase(),
      path: annotation[2],
      filePath,
      line: lineNo
    };
  const attribute = line.match(/\[Http(Get|Post|Put|Patch|Delete)\s*\(\s*["']([^"']+)/i);
  if (attribute)
...
function isTestLine(id: string, line: string): boolean {
  return (
    /\b(?:describe|it|test|testcase|assert|expect|@Test|\[Test\]|pytest|unittest|XCTAssert|RSpec)\b/i.test(
      line
    ) ||
    (id === "go" && /func\s+Test\w+/.test(line))
  );
}
function testName(line: string): string | undefined {
  return line
    .match(/(?:describe|it|test|func\s+Test\w*|def\s+test_\w+)\s*\(?\s*["']?([^"'({]+)/i)?.[1]
    ?.trim();
}
function exportStatus(id: string, line: string): CodeSymbol["exportStatus"] {
  if (/\b(export|public|pub)\b/.test(line)) return "exported";
  if (id === "python" && !/^\s*_/.test(line)) return "unknown";
  return "local";
}
...
function normalizeTarget(filePath: string, target: string): string {
  if (target.startsWith("."))
    return path.posix.normalize(path.posix.join(path.posix.dirname(filePath), target));
  return target;
}
