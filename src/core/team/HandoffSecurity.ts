...
import { HandoffPackageSchema, HandoffValidationResultSchema } from "../../shared/contracts/team";
  ...
  validate(
    ...
    if (sizeBytes > limits.maxPackageBytes) {
      diagnostics.push(
        error(
          "package-too-large",
          `Package size ${sizeBytes} exceeds the ${limits.maxPackageBytes}-byte limit.`,
        ),
      );
    }
    for (const [code, pattern] of SECRET_PATTERNS) {
      if (pattern.test(raw))
        diagnostics.push(
          error(`secret-${code}`, "Secret-like material was detected. Remove it before transfer."),
        );
    }
    ...
    if (!parsed.success) {
      diagnostics.push(
        ...parsed.error.issues
          .slice(0, 100)
          .map((issue) => error("schema-invalid", issue.message, issue.path.join("."))),
      );
    ...
    if (packageData.attachments.length > limits.maxAttachments)
      diagnostics.push(
        error("too-many-attachments", "The package exceeds the configured attachment count."),
      );
    if (packageData.attachments.length)
      diagnostics.push(
        error(
          "attachment-body-unavailable",
          "Attachment metadata is present but this package format does not contain independently verified attachment bodies.",
        ),
      );
    const attachmentBytes = packageData.attachments.reduce(
      (total, item) => total + item.sizeBytes,
      0,
    );
    if (attachmentBytes > limits.maxAttachmentBytes)
      diagnostics.push(
        error(
          "attachments-too-large",
          "Attachment metadata exceeds the configured aggregate byte limit.",
        ),
      );
    for (const attachment of packageData.attachments) {
      if (attachment.archivePath && !safeRelativePath(attachment.archivePath))
        diagnostics.push(
          error(
            "unsafe-attachment-path",
            "Attachment archive path is unsafe.",
    ...
    for (const file of packageData.changedFiles) {
      if (!safeRelativePath(file.path))
        diagnostics.push(error("unsafe-file-path", "Changed file path is unsafe.", file.path));
    }
    const calculatedFingerprint = packageFingerprint(packageData);
    if (calculatedFingerprint !== packageData.fingerprint)
      diagnostics.push(
        error("fingerprint-mismatch", "Package content does not match its recorded fingerprint."),
      );
    if (packageData.expiresAt && Date.parse(packageData.expiresAt) < Date.now())
      diagnostics.push(error("package-expired", "The handoff package has expired."));
    return HandoffValidationResultSchema.parse({
...
export function decodeHandoffArtifact(bytes: Uint8Array, maxBytes: number): Uint8Array {
  if (bytes.byteLength > maxBytes)
    throw new Error("Handoff artifact exceeds the configured byte limit.");
  ...
  if (flags !== 0 || method !== 0)
    throw new Error("Only deterministic, unencrypted STORE ZIP handoff artifacts are supported.");
  const compressedSize = view.getUint32(18, true);
  const uncompressedSize = view.getUint32(22, true);
  const nameLength = view.getUint16(26, true);
  const extraLength = view.getUint16(28, true);
  if (compressedSize !== uncompressedSize || uncompressedSize > maxBytes)
    throw new Error("ZIP entry size is invalid or exceeds the configured limit.");
  const start = 30 + nameLength + extraLength;
  const end = start + uncompressedSize;
  if (start > bytes.byteLength || end > bytes.byteLength)
    throw new Error("Handoff ZIP entry is truncated.");
  const name = new TextDecoder("utf-8", { fatal: true }).decode(bytes.slice(30, 30 + nameLength));
  if (name !== "handoff.json")
    throw new Error("The ZIP must contain a single root handoff.json entry.");
  const payload = bytes.slice(start, end);
  if (zipCrc32(payload) !== view.getUint32(14, true))
    throw new Error("Handoff ZIP checksum does not match its JSON entry.");
  const endOffset = findEndOfCentralDirectory(bytes);
  if (endOffset < 0) throw new Error("Handoff ZIP central directory is missing.");
  const endView = new DataView(
    bytes.buffer,
    bytes.byteOffset + endOffset,
    bytes.byteLength - endOffset,
  );
  if (endView.getUint16(8, true) !== 1 || endView.getUint16(10, true) !== 1)
    throw new Error("The handoff ZIP must contain exactly one entry.");
  return payload;
}
