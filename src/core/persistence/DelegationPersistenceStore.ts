...
import {
  ...
  DelegationPersistentStateSchema,
  type DelegationPersistentState,
...
export class DelegationPersistenceStore {
  ...
  get snapshot(): DelegationPersistentState {
  ...
  async initialize(): Promise<DelegationPersistentState> {
    ...
    try {
      this.state = DelegationPersistentStateSchema.parse(
  ...
  async update(
    mutator: (current: DelegationPersistentState) => DelegationPersistentState,
  ): Promise<DelegationPersistentState> {
    let result: DelegationPersistentState | undefined;
    this.writeChain = this.writeChain
      .catch(() => undefined)
      .then(async () => {
        const next = DelegationPersistentStateSchema.parse({
