...
import { ORCHESTRATION_SCHEMA_VERSION, OrchestrationPersistentStateSchema, type OrchestrationPersistentState } from "../../shared/contracts/orchestration";
...
export class OrchestrationPersistenceStore {
  ...
  get snapshot(): OrchestrationPersistentState { return structuredClone(this.state); }
  ...
  async update(mutate: (state: OrchestrationPersistentState) => OrchestrationPersistentState): Promise<OrchestrationPersistentState> {
    let result: OrchestrationPersistentState | undefined;
    this.chain = this.chain.catch(() => undefined).then(async () => {
      const next = OrchestrationPersistentStateSchema.parse({ ...mutate(this.snapshot), revision: this.state.revision + 1, updatedAt: new Date().toISOString() });
  ...
  private persist(value: OrchestrationPersistentState): Promise<void> { return this.path ? this.writer.writeJson(this.path, value) : Promise.resolve(); }
}

function empty(): OrchestrationPersistentState { return { schemaVersion: ORCHESTRATION_SCHEMA_VERSION, revision: 0, instances: [], updatedAt: new Date().toISOString() }; }
