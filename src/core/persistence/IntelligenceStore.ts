...
const OPTIONAL_SEMANTIC_SHARDS = new Set<(typeof SHARDS)[number]>([
  "contributions",
  "indexes",
  "adapters",
]);
...
const defaultJsonWorker: IntelligenceJsonParser = {
  ...
  gzip: (value) =>
    new Promise((resolve, reject) =>
      gzipCallback(value, (error, output) => (error ? reject(error) : resolve(output))),
    ),
  parseGzipJson: (value) =>
    new Promise((resolve, reject) =>
      gunzipCallback(value, (error, output) => {
        if (error) reject(error);
        else {
          try {
            resolve(JSON.parse(output.toString("utf8")) as unknown);
          } catch (cause) {
...
export class IntelligenceStore implements IntelligenceSnapshotReader {
  ...
  constructor(
    ...
    this.currentPath = this.intelligenceRoot
      ? join(this.intelligenceRoot, "current.json")
      : undefined;
  ...
  getCpgManifest(): CpgGenerationManifest | undefined {
    return this.activeCpgManifest;
  }
  getAdapterState(): AdapterRegistryState | undefined {
    return this.activeAdapterState;
  }
  ...
  async getRetainedSnapshots(): Promise<IntelligenceSnapshot[]> {
    ...
    try {
      directories = (
        await readdir(join(this.intelligenceRoot, "generations"), { withFileTypes: true })
      )
        .filter((entry) => entry.isDirectory() && /^\d{6,}$/.test(entry.name))
        .map((entry) => entry.name)
        .sort((left, right) => right.localeCompare(left));
    } catch {
      return this.activeSnapshot ? [this.activeSnapshot] : [];
    }
    const snapshots: IntelligenceSnapshot[] = [];
    for (const directory of directories) {
      if (
        this.activeSnapshot &&
        generationDirectory(this.activeSnapshot.manifest.generation) === directory
      ) {
        snapshots.push(this.activeSnapshot);
        continue;
      }
      try {
        snapshots.push(await this.loadSnapshotOnly(directory));
      } catch {
        /* A damaged retained generation is not exposed. */
      }
    }
  ...
  async readCpgScope(scopeId: string): Promise<CpgScopeArtifact | undefined> {
    ...
    const root = join(
      ...
      "generations",
      generationDirectory(this.activeSnapshot.manifest.generation),
    );
    return this.cpgStore.readScope(root, descriptor, this.activeSnapshot.manifest.generation);
  }

  async readContributionPartition(
    fileId: string,
  ): Promise<IntelligenceContributionPartition | undefined> {
    if (
      !this.intelligenceRoot ||
      !this.activeSnapshot?.contributions?.some((item) => item.fileId === fileId)
    )
      return undefined;
    const root = join(
      this.intelligenceRoot,
    ...
    return {
      ...
      relationships: await parseArray(relationships, (value) =>
        RelationshipRecordSchema.parse(value),
      ),
      evidence: await parseArray(evidence, (value) => EvidenceRecordSchema.parse(value)),
      diagnostics: await parseArray(diagnostics, (value) =>
        IntelligenceDiagnosticSchema.parse(value),
      ),
    };
  }
  ...
  onDidHealthChange(listener: (health: IntelligencePersistenceHealth) => void): {
    dispose(): void;
  } {
  ...
  async initialize(): Promise<IntelligenceSnapshot | undefined> {
    ...
    try {
      ...
      this.loadError = persistenceError(
        "INTELLIGENCE_LOAD_FAILED",
        "Keystone could not load a valid repository intelligence generation.",
        cause,
        "Run a new repository scan. Corrupt state was not exposed to queries.",
      );
      await this.finishInitialization();
      return undefined;
    }
  ...
  async save(
    ...
    await previous;
    try {
      await this.publishGeneration(snapshot, beforeCommit, cpgDelta, adapterState);
    } finally {
      release();
    }
  }
  ...
  private async publishGeneration(
    ...
    if (!this.intelligenceRoot || !this.currentPath)
      throw persistenceError(
        "INTELLIGENCE_STORAGE_UNAVAILABLE",
        "Extension-managed workspace storage is unavailable.",
        undefined,
        "Open a saved local workspace before indexing.",
        false,
      );
    const validated = await validateSnapshotYielding(snapshot);
    if (
      this.activeSnapshot &&
      validated.manifest.generation <= this.activeSnapshot.manifest.generation
    ) {
      throw persistenceError(
        "INTELLIGENCE_STALE_GENERATION",
        "Keystone rejected a stale intelligence generation.",
        undefined,
    ...
    try {
      ...
      const values: Record<(typeof SHARDS)[number], unknown> = {
        ...
        adapters: adapterState ?? {
          ...
          updatedAt: validated.manifest.completedAt,
          capabilities: [],
          detections: [],
          coverage: [],
          metrics: [],
      ...
      for (const shard of SHARDS) {
        ...
        const serialized = SHARD_FILES[shard].endsWith(".gz")
          ? this.serializeCompressed(values[shard])
          : this.serialize(values[shard]);
      ...
      const previousGeneration = this.activeSnapshot
        ? join(generationsRoot, generationDirectory(this.activeSnapshot.manifest.generation))
        : undefined;
      const cpgManifest = cpgDelta
        ? await this.cpgStore.writeGeneration(
            pendingDirectory,
            previousGeneration,
            cpgDelta,
            beforeCommit,
      ...
      throw persistenceError(
        "INTELLIGENCE_GENERATION_PUBLISH_FAILED",
        "Keystone could not publish the immutable intelligence generation.",
        cause,
        "The previous generation remains active. Retry the repository update.",
      );
    }
  }
  ...
  private applyHealth(next: IntelligencePersistenceHealth): IntelligencePersistenceHealth {
    const changed =
      ...
      next.message !== this.health.message ||
      next.pendingGenerations !== this.health.pendingGenerations;
    ...
    if (wasHealthy && next.status !== "healthy")
      for (const listener of this.deletionListeners) listener();
    return { ...next };
  }
  ...
  private async loadGeneration(directory: string): Promise<IntelligenceSnapshot> {
    ...
    const [
      ...
      SHARDS.map(async (shard) => {
        try {
          return await this.readJson(join(generationRoot, SHARD_FILES[shard]));
        } catch (cause) {
          if (OPTIONAL_SEMANTIC_SHARDS.has(shard) && isMissingFile(cause)) return undefined;
          throw cause;
        }
      }),
    );
    const snapshot = await validateSnapshotYielding({
      manifest,
      repository,
      files,
      symbols,
  ...
  private async loadSnapshotOnly(directory: string): Promise<IntelligenceSnapshot> {
    ...
    const [
      ...
      this.readJson(join(root, SHARD_FILES.manifest)),
      this.readJson(join(root, SHARD_FILES.repository)),
      this.readJson(join(root, SHARD_FILES.files)),
      this.readJson(join(root, SHARD_FILES.symbols)),
      this.readJson(join(root, SHARD_FILES.relationships)),
      this.readJson(join(root, SHARD_FILES.evidence)),
      this.readJson(join(root, SHARD_FILES.diagnostics)),
      this.readJson(join(root, SHARD_FILES.contributions)).catch((cause: unknown) => {
        if (isMissingFile(cause)) return undefined;
        throw cause instanceof Error ? cause : new Error(String(cause));
      }),
      this.readJson(join(root, SHARD_FILES.indexes)).catch((cause: unknown) => {
        if (isMissingFile(cause)) return undefined;
        throw cause instanceof Error ? cause : new Error(String(cause));
      }),
    ]);
    return validateSnapshotYielding({
      manifest,
      repository,
      files,
      symbols,
  ...
  private async readJson(path: string): Promise<unknown> {
    if (path.endsWith(".gz")) {
      ...
      const decompressed = await new Promise<Buffer>((resolve, reject) =>
        gunzipCallback(value, (error, output) => (error ? reject(error) : resolve(output))),
      );
  ...
  private async loadLegacySnapshot(): Promise<IntelligenceSnapshot | undefined> {
    ...
    try {
      const snapshot = await validateSnapshotYielding(
        await this.readJson(join(this.intelligenceRoot, "active-snapshot.json")),
      );
  ...
  private async recoverLatestGeneration(): Promise<IntelligenceSnapshot | undefined> {
    ...
    for (const directory of directories) {
      try {
        ...
        await this.writer.writeJson(this.currentPath, {
          schemaVersion: 1,
  ...
  private async cleanupGenerations(active: string): Promise<void> {
    ...
    await Promise.all(
      directories
        .filter((item) => !retained.has(item))
        .map((item) => rm(join(root, item), { recursive: true, force: true })),
    );
  }
  ...
  private async finishInitialization(): Promise<void> {
    ...
    if (this.intelligenceRoot && this.activeSnapshot) {
      ...
      this.activeCpgManifest = await this.cpgStore.loadManifest(
        root,
        this.activeSnapshot.manifest.generation,
      );
    }
    await this.checkHealth();
  }
    ...
    try {
      ...
      await Promise.all(
        entries
          ...
          .map((entry) => rm(join(generationsRoot, entry.name), { recursive: true, force: true })),
      );
    } catch (cause) {
  ...
  private async inspectHealth(cpgShardLimit?: number): Promise<IntelligencePersistenceHealth> {
    if (!this.currentPath || !this.intelligenceRoot)
      return {
        status: "missing",
        message: "Extension-managed intelligence storage is unavailable.",
        pendingGenerations: 0,
      };
    const generationsRoot = join(this.intelligenceRoot, "generations");
    let pendingGenerations = 0;
    try {
      pendingGenerations = (await readdir(generationsRoot, { withFileTypes: true })).filter(
        (entry) => entry.isDirectory() && entry.name.endsWith(".pending"),
      ).length;
    } catch (cause) {
      if (!isMissingFile(cause))
        return { status: "damaged", message: safeMessage(cause), pendingGenerations };
    }
    ...
    try {
      ...
      return isMissingFile(cause)
        ? {
            status: "missing",
            message: "The active intelligence pointer is missing.",
            pendingGenerations,
          }
        : {
            status: "damaged",
            message: "The active intelligence pointer is invalid.",
            pendingGenerations,
          };
    }
    ...
    if (this.activeSnapshot && pointer.generation !== this.activeSnapshot.manifest.generation) {
      return {
        status: "damaged",
        message: "The active intelligence pointer changed outside the generation publisher.",
        pendingGenerations,
      };
    }
    const observedFingerprints =
      cpgShardLimit === undefined ? new Map<string, string>() : new Map(this.shardFingerprints);
    ...
    for (const shard of SHARDS) {
      ...
      try {
        ...
        return {
          status: "damaged",
          message: `The active generation is missing ${SHARD_FILES[shard]}.`,
          pendingGenerations,
        };
      }
    }
    if (this.activeCpgManifest) {
      ...
      const selectedScopes =
        cpgShardLimit === undefined || scopeShards.length <= cpgShardLimit
          ? scopeShards
          : Array.from(
              { length: cpgShardLimit },
              (_, index) => scopeShards[(this.cpgHealthCursor + index) % scopeShards.length]!,
            ).filter(Boolean);
      if (cpgShardLimit !== undefined && scopeShards.length > 0)
        this.cpgHealthCursor = (this.cpgHealthCursor + selectedScopes.length) % scopeShards.length;
      const relatives = [
        join("cpg", "manifest.json"),
        ...["scope-by-symbol", "calls", "reads", "writes", "data-flow"].map((file) =>
          join("cpg", "indexes", `${file}.json.gz`),
        ),
        ...selectedScopes,
      ];
      for (let index = 0; index < relatives.length; index += 64) {
        ...
        const inspected = await Promise.all(
          batch.map(async (relative) => {
            ...
            try {
              return { relative, path, value: await stat(path) };
            } catch {
              return { relative, path, value: undefined };
            }
          }),
        );
        for (const item of inspected) {
          if (!item.value)
            return {
              status: "damaged",
              message: `The active generation is missing ${item.relative}.`,
              pendingGenerations,
            };
          const fingerprint = `${item.value.size}:${item.value.mtimeMs}`;
          observedFingerprints.set(item.path, fingerprint);
          const previous = this.shardFingerprints.get(item.path);
          if (previous !== undefined && previous !== fingerprint) contentChanged = true;
        }
      }
    }
    if (contentChanged) {
      try {
        await this.loadGeneration(pointer.directory);
      } catch {
        return {
          status: "damaged",
          message:
            "An active intelligence shard changed or became corrupt outside the generation publisher.",
          pendingGenerations,
        };
      }
    }
  ...
  private async *serialize(value: unknown): AsyncGenerator<string> {
    ...
    for (let index = 0; index < value.length; index += PERSISTENCE_ARRAY_CHUNK_SIZE) {
      if (index > 0) yield ",";
      const serialized = await this.parser.stringifyJson(
        value.slice(index, index + PERSISTENCE_ARRAY_CHUNK_SIZE),
      );
  ...
  private async *serializeCompressed(value: unknown): AsyncGenerator<Uint8Array> {
    if (!this.parser.gzip)
      throw persistenceError(
        "INTELLIGENCE_COMPRESSION_UNAVAILABLE",
        "A background compression worker is required for intelligence shards.",
        undefined,
        "Restart Keystone to restore the intelligence worker pool.",
      );
    for await (const chunk of this.serialize(value))
      yield await this.parser.gzip(new TextEncoder().encode(chunk));
  }

  private async reusePreviousShard(
    ...
    try {
      ...
      const [previousHash, pendingHash] = await Promise.all([
        this.parser.sha256(previous),
        this.parser.sha256(pending),
      ]);
  ...
  private async writeContributionShards(
    ...
    const diagnosticsById = new Map(
      snapshot.diagnostics.flatMap((item) => (item.id ? [[item.id, item] as const] : [])),
    );
    const previous = new Map(
      (this.activeSnapshot?.contributions ?? []).map((item) => [item.fileId, item]),
    );
    for (const contribution of snapshot.contributions) {
      const key = partitionKey(contribution.fileId);
      const values = {
        entities: contribution.entityIds.flatMap((id) => {
          const item = entitiesById.get(id);
          return item ? [item] : [];
        }),
        relationships: contribution.relationshipIds.flatMap((id) => {
          const item = relationshipsById.get(id);
          return item ? [item] : [];
        }),
        evidence: contribution.evidenceIds.flatMap((id) => {
          const item = evidenceById.get(id);
          return item ? [item] : [];
        }),
        diagnostics: contribution.diagnosticIds.flatMap((id) => {
      ...
      for (const [family, value] of Object.entries(values)) {
        ...
        if (
          prior &&
          sameContribution(prior, contribution) &&
          (await this.linkPreviousPartition(relative, target))
        )
          continue;
...
async function validateSnapshotYielding(raw: unknown): Promise<IntelligenceSnapshot> {
  ...
  const relationships = await parseArray(candidate.relationships, (value) =>
    RelationshipRecordSchema.parse(value),
  );
  const evidence = await parseArray(candidate.evidence, (value) =>
    EvidenceRecordSchema.parse(value),
  );
  const diagnostics = await parseArray(candidate.diagnostics, (value) =>
    IntelligenceDiagnosticSchema.parse(value),
  );
  const contributions =
    candidate.contributions === undefined
      ? undefined
      : await parseArray(candidate.contributions, (value) => FileContributionSchema.parse(value));
  const indexes =
    candidate.indexes === undefined
      ? undefined
      : IntelligenceIndexesSchema.parse(candidate.indexes);
  ...
  for (const subject of [
    ...
    for (const evidenceId of subject.evidenceIds) {
      ...
      if (record.subjectId !== subject.id)
        throw new Error(`Evidence record ${evidenceId} does not support ${subject.id}.`);
    }
  }
...
async function parseArray<T>(raw: unknown, parse: (value: unknown) => T): Promise<T[]> {
  if (!Array.isArray(raw))
    throw new Error("Intelligence snapshot record collection is not an array.");
  const output: T[] = [];
...
function sameContribution(
  left: NonNullable<IntelligenceSnapshot["contributions"]>[number],
  right: NonNullable<IntelligenceSnapshot["contributions"]>[number],
): boolean {
  return (
...
function persistenceError(
  ...
  return new KeystoneError({
    ...
    ...(cause !== undefined
      ? {
          technicalDetails: cause instanceof Error ? cause.message : "Unknown persistence failure.",
          cause,
        }
      : {}),
