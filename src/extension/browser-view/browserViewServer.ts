...
interface BrowserSession { readonly id: string; readonly origin: string; readonly expiresAt: number; }
interface BootstrapToken { readonly expiresAt: number; }
...
export async function startBrowserViewServer(options: {
  ...
  const subscription = options.store.subscribe(state => broadcast({ type: 'APPLICATION_STATE', state }));
    ...
    try {
      ...
      if (request.method === 'GET' && requestUrl.pathname === '/' && bootstrap) {
        ...
        if (!entry || entry.expiresAt <= Date.now()) {
          ...
          json(response, 401, { error: 'The Browser View link is invalid or expired. Open it again from Keystone.' });
          return;
        }
        bootstrapTokens.delete(bootstrap);
        ...
        response.setHeader('Set-Cookie', `${SESSION_COOKIE}=${session.id}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${SESSION_TTL_SECONDS}${isSecureRequest(request) ? '; Secure' : ''}`);
        response.end();
        return;
      }
      ...
      if (!session) {
        json(response, 401, { error: 'Unauthorized. Open Browser View from the active Keystone extension.' });
        return;
      }
      ...
      if (requestUrl.pathname === '/command' && request.method === 'POST') {
        ...
        try { envelope = JSON.parse(body) as unknown; }
        catch { json(response, 400, { error: 'Browser View command JSON is malformed.' }); return; }
        const parsed = parseCommandEnvelope(envelope);
        if (!parsed) { json(response, 400, { error: 'Browser View command is not recognized.' }); return; }
        const currentVersion = options.store.snapshot().version;
        if (parsed.expectedStateVersion !== currentVersion) {
          json(response, 409, { error: 'The Browser View state is stale. Wait for synchronization and retry.', stateVersion: currentVersion });
          return;
        }
  ...
  if (!address || typeof address === 'string') throw new Error('Could not start Keystone Browser View.');
...
const WEBVIEW_COMMAND_TYPES = new Set<string>([
  'WEBVIEW_READY', 'INDEX_REPO', 'LOAD_INTELLIGENCE', 'LOAD_RESTORED_TASK_HANDOFF', 'CLEAR_CONTEXT_CACHE',
  'ENHANCE_INTENT', 'LOAD_ENHANCEMENT_SESSIONS', 'DELETE_ENHANCEMENT_SESSION', 'RETRIEVE_CONTEXT_ORIGINAL',
  'RECORD_CONTEXT_FEEDBACK', 'CANCEL_INGESTION', 'CANCEL_ANALYSIS', 'ANALYZE_INTENT', 'APPROVE_INTENT_RESEARCH', 'RUN_VALIDATION',
  'COMPLETE_TASK', 'ANALYZE_MODERNIZATION', 'ACCEPT_MODERNIZATION', 'APPROVE_DELEGATION', 'COPY_COPILOT_PROMPT',
  'COPY_PR_MARKDOWN', 'SAVE_SETTINGS', 'OPEN_BROWSER_VIEW', 'CREATE_TASK_HANDOFF', 'RESTORE_TASK_HANDOFF',
  'CREATE_SDLC_PLAN', 'SDLC_TRANSITION', 'APPROVE_SPECIFICATION', 'QUERY_INTELLIGENCE', 'EXPLORE_INTELLIGENCE',
  'LOAD_INTELLIGENCE_GRAPH', 'LOAD_CPG_VIEW', 'OPEN_SOURCE_LOCATION', 'RESOLVE_SDLC_FINDING', 'RECORD_DECISION',
  'CONFIGURE_VALUEEDGE', 'IMPORT_VALUEEDGE_FEATURE', 'PUBLISH_VALUEEDGE_STORIES',
]);

function parseCommandEnvelope(value: unknown): { message: WebviewToExtensionMessage; expectedStateVersion: number } | undefined {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return undefined;
  const record = value as Record<string, unknown>;
  if (!Number.isSafeInteger(record.expectedStateVersion) || Number(record.expectedStateVersion) < 1) return undefined;
  const message = record.message;
  if (!message || typeof message !== 'object' || Array.isArray(message)) return undefined;
  const type = (message as Record<string, unknown>).type;
  if (typeof type !== 'string' || !WEBVIEW_COMMAND_TYPES.has(type)) return undefined;
  return { message: message as WebviewToExtensionMessage, expectedStateVersion: Number(record.expectedStateVersion) };
}

function authenticate(request: IncomingMessage, sessions: ReadonlyMap<string, BrowserSession>): BrowserSession | undefined {
...
function isJson(request: IncomingMessage): boolean {
  return String(request.headers['content-type'] ?? '').toLowerCase().split(';')[0]?.trim() === 'application/json';
}

function pruneExpired(bootstrapTokens: Map<string, BootstrapToken>, sessions: Map<string, BrowserSession>): void {
  const now = Date.now();
  for (const [token, entry] of bootstrapTokens) if (entry.expiresAt <= now) bootstrapTokens.delete(token);
  for (const [id, session] of sessions) if (session.expiresAt <= now) sessions.delete(id);
}

function setSecurityHeaders(response: ServerResponse): void {
  response.setHeader('Content-Security-Policy', "default-src 'self'; connect-src 'self'; img-src 'self' data:; style-src 'self'; script-src 'self'; frame-ancestors 'none'; base-uri 'none'; form-action 'none'; object-src 'none'");
...
function readBody(request: IncomingMessage, max: number): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    request.on('end', () => { if (!settled) resolve(value); });
    request.on('error', reject);
  });
}

function serveAsset(root: string, pathname: string, response: ServerResponse, headOnly = false): void {
