...
export class ConfiguredAgentLoader {
  ...
  async load(): Promise<CopilotAgentDescriptor[]> {
    ...
    const profiles = this.registry.discovery.parseConfigured(
      configured.get<unknown[]>("profiles", []),
      "workspace-configured",
    );
    const aliases = this.registry.discovery.parseConfigured(
      configured.get<unknown[]>("aliases", []),
      "user-alias",
    );
    ...
    for (const root of this.workspace.getRoots().slice(0, 20)) {
      try {
        ...
        const value: unknown = JSON.parse(
          (await this.workspace.readTextFile(uri)).slice(0, 200_000),
        );
        const entries = Array.isArray(value)
          ...
          : value && typeof value === "object" && "profiles" in value
            ...
            : [];
        repository.push(
          ...this.registry.discovery.parseConfigured(entries, "repository-configured"),
        );
      } catch {
...
function parseRules(values: unknown[]): AgentSelectionRule[] {
  return values.slice(0, 100).flatMap((value) => {
    ...
    const category = DelegationTaskCategorySchema.safeParse(item.category);
    const capability = AgentCapabilitySchema.safeParse(item.requiredCapability);
    return [
      {
        id: item.id.slice(0, 200),
        agentId: item.agentId.slice(0, 200),
        ...(category.success ? { category: category.data } : {}),
        ...(capability.success ? { requiredCapability: capability.data } : {}),
      },
    ];
  });
}
