...
export interface RepositoryMonitor {
  onDidChange(listener: (change: RepositoryChange) => void): { dispose(): void };
  onDidGitChange(listener: (event: GitReconciliationEvent) => void): { dispose(): void };
...
export class VsCodeRepositoryMonitor implements RepositoryMonitor {
  ...
  async start(): Promise<void> {
    ...
    this.started = true;
    for (const root of this.workspace.getRoots()) this.gitState.set(root.uri, await this.git.getMetadata(root.uri));
    ...
    this.disposables.push(
      ...
      vscode.workspace.onDidSaveTextDocument((document) => this.emitUri(document.uri, "modified", "active-editor")),
      ...
      vscode.workspace.onDidChangeWorkspaceFolders(() => { for (const listener of this.rootListeners) listener(); }),
  ...
  private emitUri(uri: vscode.Uri, kind: RepositoryChange["kind"], reason: RepositoryChange["reason"]): void {
    const change = this.toChange(uri, kind, reason);
    if (!change) return;
    for (const listener of this.changeListeners) listener(change);
  }

  private toChange(uri: vscode.Uri, kind: RepositoryChange["kind"], reason: RepositoryChange["reason"]): RepositoryChange | undefined {
    ...
    return { kind, rootUri: root.uri, uri: uri.toString(), relativePath, reason };
  }
  ...
  private scheduleGitCheck(): void {
    ...
    this.gitCheckTimer = setTimeout(() => {
      ...
      if (this.started && revision === this.gitCheckRevision) {
        void this.checkGit().catch((cause: unknown) => this.logger.warning("intelligence.git.monitor", "Git state monitoring failed; the next repository event will retry reconciliation.", cause));
      }
    }, 300);
  }
  ...
  private async checkGit(): Promise<void> {
    ...
    for (const root of this.workspace.getRoots()) {
      ...
      if (previous?.branch === current.branch && previous?.headCommit === current.headCommit && previous?.dirtyFingerprint === current.dirtyFingerprint) continue;
      if (!this.workspace.getIndexingConfiguration().onBranchChange && previous?.branch !== current.branch) continue;
      const changes: RepositoryChange[] = [];
      let fullRebuild: boolean;
      if (previous) {
        try {
          const values = await this.git.getReconciliationChanges(root.uri, previous.headCommit, current.headCommit);
          ...
          fullRebuild = true;
          this.logger.warning("intelligence.git.reconcile", "Git diff reconciliation failed; a complete repair was scheduled.", cause);
        }
      } else {
        fullRebuild = true;
      }
...
function deduplicateChanges(changes: readonly RepositoryChange[]): RepositoryChange[] {
  ...
  for (const change of changes) values.set(`${change.rootUri}\u001f${change.relativePath}\u001f${change.kind}`, change);
  return [...values.values()].sort((left, right) => left.rootUri.localeCompare(right.rootUri) || left.relativePath.localeCompare(right.relativePath));
}

function findRoot(roots: readonly WorkspaceRootReference[], uri: vscode.Uri): WorkspaceRootReference | undefined {
  return roots
    .filter((root) => {
      const rootUri = vscode.Uri.parse(root.uri);
      return rootUri.scheme === uri.scheme && rootUri.authority === uri.authority && (uri.path === rootUri.path || uri.path.startsWith(`${rootUri.path.replace(/\/$/, "")}/`));
    })
