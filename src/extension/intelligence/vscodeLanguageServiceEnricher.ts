...
import type { SemanticEnrichmentProvider, SemanticEnrichmentRequest, SemanticEnrichmentResult } from '@core/intelligence/languages/semanticEnrichment';
...
export class VscodeLanguageServiceEnricher implements SemanticEnrichmentProvider {
  async enrich(request: SemanticEnrichmentRequest): Promise<SemanticEnrichmentResult | undefined> {
    ...
    const rawSymbols = await vscode.commands.executeCommand<unknown[]>('vscode.executeDocumentSymbolProvider', uri) ?? [];
    ...
    if (!flattened.length) {
      return {
        ...
        capabilities: { documentSymbols: false, definitions: false, references: false, implementations: false, callHierarchy: false },
        symbols: [], warnings: ['No document-symbol provider returned semantic symbols for this file.']
      };
    }
    ...
    for (const item of flattened) {
      request.signal?.throwIfAborted();
      const position = new vscode.Position(Math.max(item.line - 1, 0), Math.max(item.column - 1, 0));
      try {
        const result = await vscode.commands.executeCommand<unknown[]>('vscode.executeDefinitionProvider', uri, position) ?? [];
        definitions ||= result.length > 0;
      } catch (error) { warnings.push(`Definition provider failed at ${item.line}: ${message(error)}`); }
      try {
        const result = await vscode.commands.executeCommand<unknown[]>('vscode.executeReferenceProvider', uri, position) ?? [];
        references ||= result.length > 0;
        referenceCount += result.length;
      } catch (error) { warnings.push(`Reference provider failed at ${item.line}: ${message(error)}`); }
      try {
        const result = await vscode.commands.executeCommand<unknown[]>('vscode.executeImplementationProvider', uri, position) ?? [];
        implementations ||= result.length > 0;
      } catch (error) { warnings.push(`Implementation provider failed at ${item.line}: ${message(error)}`); }
      if (item.symbol.kind === 'function' || item.symbol.kind === 'method') {
        try {
          const prepared = await vscode.commands.executeCommand<unknown[]>('vscode.prepareCallHierarchy', uri, position) ?? [];
          for (const hierarchyItem of prepared) {
            const outgoing = await vscode.commands.executeCommand<any[]>('vscode.provideOutgoingCalls', hierarchyItem) ?? [];
            if (outgoing.length) callHierarchy = true;
            for (const outgoingCall of outgoing) {
              const target = outgoingCall?.to?.name;
              if (typeof target === 'string' && target) calls.push({ filePath: request.relativePath, caller: item.symbol.name, callee: target, line: item.line, evidence: semanticEvidence(request.relativePath, item.line, document.languageId) });
            }
          }
        } catch (error) { warnings.push(`Call hierarchy provider failed at ${item.line}: ${message(error)}`); }
      }
    ...
    return {
      provider: 'vscode-language-service',
      providerLanguageId: document.languageId,
      capabilities: { documentSymbols: true, definitions, references, implementations, callHierarchy },
...
interface FlattenedSymbol { readonly symbol: CodeSymbol; readonly line: number; readonly column: number; }

export function flattenSymbols(values: readonly unknown[], relativePath: string, providerLanguageId = 'unknown'): FlattenedSymbol[] {
...
function semanticEvidence(filePath: string, line: number, languageId: string): EvidenceMetadata {
  return { source: 'language-service', confidence: 0.96, evidencePath: filePath, evidenceLine: line, extractorVersion: `vscode-language-service:${languageId}` };
}
