  ...
  if (!overview) {
    return (
      <section className="page intelligence-page">
        ...
        <p>Loading local repository intelligence…</p>
      </section>
    );
  }

  const canStart =
    overview.status === "not-indexed" ||
    overview.status === "failed" ||
    overview.status === "ready" ||
    overview.status === "partial";
  const loadDiagnostics = (cursor?: string): void => {
    void bridge
      .request("intelligence/diagnostics", {
        limit: 50,
        ...(cursor ? { cursor } : {}),
      })
      .then((value) =>
  ...
  return (
    <section className="page intelligence-page">
      <div className="intelligence-heading">
        ...
        <span className={`intelligence-status ${overview.status}`}>
          {overview.runtime.phase.replace("-", " ")}
        </span>
      </div>
      ...
      <div className="intelligence-actions">
        {canStart && (
          <button className="primary-button" onClick={onStart}>
            {overview.generation > 0 ? "Rescan repository" : "Scan repository"}
          </button>
        )}
        {overview.pendingUpdate && overview.runtime.phase !== "paused" && (
          <button className="ghost-button" onClick={onPause}>
            Pause ingestion
          </button>
        )}
        {overview.runtime.phase === "paused" && (
      ...
      <article className="runtime-summary" aria-label="Continuous ingestion runtime">
        <div>
          <small>WORKERS</small>
          <strong>
            {overview.runtime.activeWorkers} / {overview.runtime.workerCapacity}
          </strong>
        </div>
        <div>
          <small>QUEUED JOBS</small>
          <strong>{overview.runtime.queueDepth}</strong>
        </div>
        <div>
          <small>PENDING FILES</small>
          <strong>{overview.runtime.pendingFiles}</strong>
        </div>
        <div>
        ...
        <div>
          ...
          <strong>{overview.runtime.completedJobs}</strong>
        </div>
        <div>
          <small>FAILED JOBS</small>
          <strong>{overview.runtime.failedJobs}</strong>
        </div>
        <div>
          <small>STALE DISCARDS</small>
          <strong>{overview.runtime.staleResultsDiscarded}</strong>
        </div>
        <div>
      ...
      {overview.runtime.currentFiles.length > 0 && (
        <p className="processing-files">
          <strong>Processing</strong> {overview.runtime.currentFiles.join(", ")}
        </p>
      )}
      ...
      {overview.repository && (
        <article className="repository-summary">
          <div>
            ...
            <strong>{overview.repository.displayName}</strong>
          </div>
          <div>
            <small>GENERATION</small>
            <strong>{overview.generation}</strong>
          </div>
          <div>
            <small>ROOTS</small>
            <strong>{overview.repository.workspaceRoots.length}</strong>
          </div>
          <div>
            <small>BRANCH</small>
            <strong>{overview.repository.branch ?? "Unknown"}</strong>
          </div>
          <div>
            <small>HEAD</small>
            <strong title={overview.repository.headCommit}>
              {overview.repository.headCommit?.slice(0, 12) ?? "Unknown"}
            </strong>
          </div>
          <div>
      ...
      {overview.cpg && (
        ...
        >
          ...
          <button
            onClick={() =>
              setInsight(
                cpgInsight(
                  "Built / reused",
                  overview.cpg!.scopesBuilt + overview.cpg!.scopesReused,
                ),
              )
            }
          >
            <small>BUILT / REUSED</small>
            <strong>
              {overview.cpg.scopesBuilt} / {overview.cpg.scopesReused}
            </strong>
          </button>
          ...
          >
            ...
            <strong>{Math.round(overview.cpg.buildTimeMs)} ms</strong>
          </button>
          <button
            onClick={() =>
        ...
        <section className="diagnostic-list" aria-label="Intelligence diagnostics">
          ...
          {diagnostics?.nextCursor && (
            <button
              ...
              onClick={() => loadDiagnostics(diagnostics.nextCursor)}
            >
              Load 50 more
            </button>
          )}
          {diagnostics && (
            <p>
              Showing {diagnostics.items.length.toLocaleString()} of{" "}
              {diagnostics.total.toLocaleString()} diagnostics from generation{" "}
...
interface Insight {
  ...
  calculation: string;
  action?: string;
  browse?: { query?: string; entityTypes?: string[]; languages?: string[] };
  settings?: boolean;
  rescan?: boolean;
}
function InsightPanel({
  ...
  const browse = (): void => {
    window.dispatchEvent(
      new CustomEvent("keystone:intelligence-browse", {
        detail: insight.browse,
      }),
    );
  };
  return (
    <section className="insight-panel" aria-label={`${insight.title} details`}>
      <header>
        <div>
          <small>Intelligence detail</small>
          <h2>
            {insight.title} · {insight.value.toLocaleString()}
          </h2>
        </div>
...
function metricInsight(label: string, value: number): Insight {
  ...
  return {
    title: label,
    value,
    meaning: meanings[label] ?? "Current-generation intelligence count.",
    calculation: diagnostic
      ? "Counted from current-generation diagnostics; unresolved targets are never fabricated."
      : "Counted from the last atomically promoted local generation in .keystone/.",
    action:
      diagnostic && value
        ? "Expand the Diagnostics section to inspect exact locations and prepare a fix workflow where appropriate."
        : label === "Excluded"
          ? "No action is required when exclusions match policy. Unexpected exclusions should be reviewed in settings and rescanned."
          : undefined,
    ...(types[label]
      ? { browse: { query: "", entityTypes: types[label] } }
      : label === "Symbols"
        ? { browse: { query: "" } }
        : {}),
    ...(label === "Excluded" ? { settings: true, rescan: true } : {}),
  };
}
...
function breakdownInsight(group: string, key: string, count: number): Insight {
  ...
  if (group === "Relationship types")
    return {
      title: displayKey(key),
      value: count,
      meaning: `Evidence-backed relationships of ontology type ${key}.`,
      calculation:
        "Counted from the persisted relationship-type index; inspect an entity to see edge evidence.",
      browse: { query: "" },
    };
  return {
    title: displayKey(key),
    value: count,
    meaning: `Relationships in the ${displayKey(key)} confidence band.`,
    calculation:
      "Exact is 1.0, high is at least 0.85, medium is at least 0.6, and lower values are candidates. Confidence never replaces evidence.",
    browse: { query: "" },
  };
}
...
function statusDescription(overview: IntelligenceOverviewModel): string {
  if (overview.runtime.phase === "paused")
    return "Continuous ingestion is paused. The last complete generation remains queryable.";
  if (overview.runtime.phase === "recovering")
  ...
  if (overview.status === "scanning")
    return overview.generation > 0
      ...
      : "Building the first local generation in background workers.";
  if (overview.status === "failed")
    return "The snapshot could not be loaded or built. Review diagnostics and retry.";
  if (overview.status === "partial")
    return "The snapshot is usable, with unsupported or failed files reported below.";
