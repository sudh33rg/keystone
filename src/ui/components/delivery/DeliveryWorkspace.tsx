...
export function DeliveryWorkspace({ bridge }: { bridge: HostBridge }): React.JSX.Element {
  ...
  const [capabilities, setCapabilities] = useState<GitCapabilities>();
  const [repository, setRepository] = useState<GitRepositoryState>();
  const [readiness, setReadiness] = useState<DeliveryReadiness>();
  const [changeSet, setChangeSet] = useState<DeliveryChangeSet>();
  const [plan, setPlan] = useState<CommitPlan>();
  const [provider, setProvider] = useState<PullRequestProviderCapability>();
