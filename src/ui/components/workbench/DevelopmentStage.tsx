...
import { DevelopmentContextPackage } from "./DevelopmentContextPackage";
...
export function DevelopmentStage({ bridge, workflowId, onWorkflowChange }: { bridge: HostBridge; workflowId: string; onWorkflowChange?: (workflow: CanonicalWorkflow) => void }): React.JSX.Element {
  ...
  const accept = useCallback((next: DevelopmentAggregate): void => {
    ...
    if (!loaded.current || next.result) {
      if (!loaded.current) setPromptNotes(next.contextPackage?.items.find((item) => item.id === "development:user-notes")?.content ?? "");
  ...
  const packagedNotes = state.contextPackage?.items.find((item) => item.id === "development:user-notes")?.content ?? "";
  const contextNotesChanged = Boolean(state.contextPackage && promptNotes.trim() !== packagedNotes);
  return <section className="development-workspace" aria-label="Development workspace">
    ...
    <section className="development-section" aria-labelledby="development-scope"><header><div><span className="eyebrow">2</span><h2 id="development-scope">Source Scope</h2></div><span>{state.scopeItems.length} selected</span></header>
      ...
      {(state.intelligenceSelections?.length ?? 0) > 0 && <div className="intelligence-selection-list"><strong>Bounded Intelligence context</strong>{state.intelligenceSelections.map((selection) => <article key={selection.id}><span>{selection.label}</span><small>{selection.entityIds.length} entities · {selection.edgeIds.length} relationships · {selection.evidenceIds.length} evidence records · revision {selection.intelligenceRevision}</small></article>)}</div>}
    </section>

    <ExecutionConfiguration bridge={bridge} workflowId={workflowId} workItemId={state.workItem.id} onProfileChange={() => void perform("configuration-sync", () => bridge.request("development.load", { correlationId: crypto.randomUUID(), workflowId }))} />

    <DevelopmentContextPackage bridge={bridge} state={state} notes={promptNotes} onChange={(next, message) => { accept(next); setNotice(message); }} onError={(cause) => setError({ message: cause instanceof Error ? cause.message : "Keystone could not update context.", detail: cause instanceof Error ? cause.stack ?? cause.message : String(cause) })} />

    <section className="development-section" aria-labelledby="development-prompt"><header><div><span className="eyebrow">5</span><h2 id="development-prompt">Prompt Preparation</h2></div><span>{state.promptPreparation?.status ?? "not prepared"}</span></header>
      ...
      {contextNotesChanged && <span className="field-error">User notes changed after this context revision. Regenerate and approve context before preparing the prompt.</span>}
      <button className="primary-button" disabled={state.contextPackage?.metadata.status !== "approved" || contextNotesChanged || dirtyObjective || Boolean(busy)} onClick={() => void perform("prepare", () => bridge.request("development.preparePrompt", { correlationId: crypto.randomUUID(), ...ids }), "Development prompt prepared from the approved context revision.")}>Prepare Prompt</button>
    ...
    <section className="development-section" aria-labelledby="development-result"><header><div><span className="eyebrow">6</span><h2 id="development-result">Development Result</h2></div><span>{state.result?.reviewStatus ?? "not recorded"}</span></header>
    ...
    <section className="development-section" aria-labelledby="development-changes"><header><div><span className="eyebrow">7</span><h2 id="development-changes">Changed Files</h2></div><span>{state.changeDetection.available ? `${state.changeDetection.changes.length} detected` : "unavailable"}</span></header>
    ...
    <section className="development-section completion-section" aria-labelledby="development-completion"><header><div><span className="eyebrow">8</span><h2 id="development-completion">Completion</h2></div><span>{state.workItem.status === "completed" ? "completed" : state.completion.allowed ? "ready" : "gated"}</span></header>
