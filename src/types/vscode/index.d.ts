declare module "vscode" {
  ...
  export class Uri { readonly scheme: string; readonly fsPath: string; static file(path:string): Uri; static parse(value:string): Uri; static joinPath(base:Uri,...paths:string[]):Uri; toString(skipEncoding?:boolean):string; }
  export interface Memento { get<T>(key:string):T|undefined; get<T>(key:string,defaultValue:T):T; update(key:string,value:unknown):Thenable<void>; keys():readonly string[]; }
  ...
  export interface ExtensionContext { extensionUri:Uri; subscriptions:Disposable[]; workspaceState:Memento; globalState:Memento; secrets:SecretStorage; storageUri?:Uri; globalStorageUri:Uri; extensionPath:string; }
  export interface StatusBarItem extends Disposable { name?:string; text:string; tooltip?:string; command?:string; show():void; hide():void; }
  export interface LogOutputChannel extends Disposable { info(message:string,...args:unknown[]):void; warn(message:string,...args:unknown[]):void; error(message:string,...args:unknown[]):void; debug(message:string,...args:unknown[]):void; appendLine(message:string):void; show():void; }
  ...
  export interface WebviewPanel extends Disposable { webview:Webview; reveal(column?:ViewColumn,preserveFocus?:boolean):void; onDidDispose(listener:()=>any):Disposable; }
  ...
  export interface WorkspaceConfiguration { get<T>(section:string):T|undefined; get<T>(section:string,defaultValue:T):T; update(section:string,value:unknown,target?:ConfigurationTarget|boolean):Thenable<void>; }
  export namespace workspace {
    ...
    function onDidChangeWorkspaceFolders(listener:(event:{added:readonly WorkspaceFolder[];removed:readonly WorkspaceFolder[]})=>any):Disposable;
