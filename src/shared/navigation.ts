import { AppRouteSchema, WorkbenchStageSchema, type AppRoute, type NavigationSection, type WorkbenchStage } from "./contracts/domain";
...
export interface WorkbenchRoute { kind: "new" | "workflow"; workflowId?: string; stage?: WorkbenchStage; }
...
export function parseWorkbenchRoute(route: AppRoute): WorkbenchRoute | undefined {
  ...
  const match = /^\/workbench\/([0-9a-f-]+)\/(define|plan|build|validate|review|complete)$/.exec(route);
...
export function compatibilityRoute(section: NavigationSection, workflowId?: string): AppRoute {
  ...
  switch (section) {
    ...
    case "validation": return selected === "new" ? "/workbench/new" : workbenchRoute(selected, "validate");
