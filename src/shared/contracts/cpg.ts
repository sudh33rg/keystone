...
export const CpgDiagnosticSchema = z
  .object({
    id: z.string().min(1),
...
export const CpgScopeSummarySchema = z
  .object({
...
export const CpgScopeDescriptorSchema = z
  .object({
    id: z.string().min(1),
    fileId: z.string().min(1),
...
export const CpgScopeArtifactSchema = z
  .object({
  ...
  .superRefine((artifact, context) => {
    ...
    if (!ids.has(artifact.entryNodeId))
      context.addIssue({
        code: "custom",
        message: "CPG entry node is missing.",
        path: ["entryNodeId"],
      });
    if (!ids.has(artifact.exitNodeId))
      context.addIssue({
        code: "custom",
        message: "CPG exit node is missing.",
        path: ["exitNodeId"],
      });
    artifact.edges.forEach((edge, index) => {
      if (!ids.has(edge.sourceId) || !ids.has(edge.targetId))
        context.addIssue({
          code: "custom",
          message: "CPG edge endpoint is missing.",
          path: ["edges", index],
        });
    });
  });
...
export const CpgGenerationManifestSchema = z
  .object({
    schemaVersion: z.literal(CPG_SCHEMA_VERSION),
    ...
    indexes: z
      .object({
        ...
        writes: z.record(z.string(), z.number().int().nonnegative()),
        dataFlow: z.record(z.string(), z.number().int().nonnegative()),
      })
      .strict(),
    metrics: z
      .object({
...
export const CpgOverlaysSchema = z
  .array(z.enum(["ast", "evaluation", "control-flow", "data-flow", "calls"]))
  .max(5);
export const CpgScopeQuerySchema = z
  .object({
    semanticSymbolId: z.string().min(1),
...
export const CpgSliceQuerySchema = z
  .object({
    semanticSymbolId: z.string().min(1),
    nodeId: z.string().min(1).optional(),
  ...
  .strict()
  .refine((value) => Boolean(value.nodeId || value.location), {
    message: "A CPG node or source location is required.",
  });
export type CpgSliceQuery = z.input<typeof CpgSliceQuerySchema>;

export const CpgSourceFragmentSchema = z
  .object({
    nodeId: z.string(),
    range: SourceRangeSchema,
  ...
  .strict();
export const CpgQueryResultSchema = z
  .object({
...
export const CpgSliceResultSchema = z
  .object({
    generation: z.number().int().nonnegative(),
    scope: CpgScopeDescriptorSchema,
    direction: z.enum(["backward", "forward"]),
