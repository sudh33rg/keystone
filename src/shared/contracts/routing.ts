...
export const ExecutionRoutingRequestSchema = z
  .object({
    operation: ExecutionOperationSchema,
    copilotAgentId: z.string().min(1).max(300).optional(),
    copilotAvailable: z.boolean().default(false),
  })
  .strict();
export const ExecutionRoutingDecisionSchema = z
  .object({
    operation: ExecutionOperationSchema,
    route: ExecutionRouteSchema,
    copilotAgentId: z.string().max(300).optional(),
    reasons: z.array(z.string().max(2_000)).min(1).max(20),
