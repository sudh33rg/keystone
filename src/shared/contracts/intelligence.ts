...
export const IntelligenceRuntimePhaseSchema = z.enum(["ready", "stale", "reconciling", "rebuilding", "recovering", "failed", "paused"]);
...
export const WorkspaceRootRecordSchema = z.object({
  id: z.string().min(1),
  name: z.string().min(1),
...
export const FileRecordSchema = z.object({
  ...
  parseStatus: z.enum(["not-applicable", "parsed", "partial", "failed", "unsupported"]).optional(),
...
export const SymbolRecordSchema = z.object({
  ...
  parameters: z.array(z.object({ name: z.string(), type: z.string().optional(), optional: z.boolean().optional(), rest: z.boolean().optional() }).strict()).optional(),
  ...
  properties: z.record(z.string(), z.union([z.string(), z.number(), z.boolean(), z.array(z.string())])).optional(),
  codeAnalysis: z.object({
    scopeId: z.string().min(1), structuralHash: z.string().min(1), providerId: z.string().min(1), providerVersion: z.string().min(1),
    calculationMethod: z.string().min(1), confidence: z.number().min(0).max(1), evidenceIds: z.array(z.string().min(1)).min(1),
    branches: z.number().int().nonnegative(), calls: z.number().int().nonnegative(), reads: z.number().int().nonnegative(), writes: z.number().int().nonnegative(), unresolvedCalls: z.number().int().nonnegative()
  }).strict().optional(),
...
export const RelationshipRecordSchema = z.object({
  ...
  resolution: z.enum(["exact", "compiler", "framework", "syntactic", "convention", "candidate", "external", "unresolved"]).optional(),
  properties: z.record(z.string(), z.union([z.string(), z.number(), z.boolean(), z.array(z.string())])).optional(),
  evidenceIds: z.array(z.string().min(1)).min(1),
...
export const EvidenceRecordSchema = z.object({
  ...
  sourceKind: z.enum(["workspace-inventory", "source-file", "language-provider", "typescript-compiler", "framework-rule", "manifest", "configuration", "adapter", "documentation", "schema", "database", "ci", "infrastructure"]),
  workspaceRootId: z.string().min(1),
...
export const IntelligenceIndexesSchema = z.object({
...
export const IntelligenceSnapshotSchema = z.object({
  ...
  const requireEvidence = (subject: { id: string; evidenceIds: string[] }, path: (string | number)[]): void => {
    for (const id of subject.evidenceIds) {
      const evidence = evidenceById.get(id);
      if (!evidence) context.addIssue({ code: "custom", message: `Missing evidence ${id}`, path });
      else if (evidence.subjectId !== subject.id) context.addIssue({ code: "custom", message: `Evidence ${id} supports another subject`, path });
    }
  };
  requireEvidence(snapshot.repository, ["repository", "evidenceIds"]);
  snapshot.repository.workspaceRoots.forEach((root, index) => requireEvidence(root, ["repository", "workspaceRoots", index, "evidenceIds"]));
  snapshot.files.forEach((file, index) => requireEvidence(file, ["files", index, "evidenceIds"]));
  snapshot.symbols.forEach((symbol, index) => requireEvidence(symbol, ["symbols", index, "evidenceIds"]));
  snapshot.relationships.forEach((relationship, index) => requireEvidence(relationship, ["relationships", index, "evidenceIds"]));
  const entityIds = new Set([snapshot.repository.id, ...snapshot.files.map((item) => item.id), ...snapshot.symbols.map((item) => item.id)]);
  snapshot.relationships.forEach((relationship, index) => {
    if (!entityIds.has(relationship.sourceId)) context.addIssue({ code: "custom", message: "Unresolved relationship source", path: ["relationships", index, "sourceId"] });
    if (!entityIds.has(relationship.targetId)) context.addIssue({ code: "custom", message: "Unresolved relationship target", path: ["relationships", index, "targetId"] });
  });
  const fileIds = new Set(snapshot.files.map((item) => item.id));
  snapshot.contributions?.forEach((contribution, index) => {
    if (!fileIds.has(contribution.fileId)) context.addIssue({ code: "custom", message: "Contribution owner file is missing", path: ["contributions", index, "fileId"] });
  });
});
export type IntelligenceSnapshot = z.infer<typeof IntelligenceSnapshotSchema>;

const BoundedCountSchema = z.object({ key: z.string(), count: z.number().int().nonnegative() }).strict();

export const IntelligenceRuntimeOverviewSchema = z.object({
...
export const IntelligenceOverviewSchema = z.object({
  ...
  technologyCoverage: z.array(z.object({
    technologyId: z.string(), adapterId: z.string(), capabilityLevel: z.enum(["deep", "semantic", "structural", "metadata-only", "unsupported"]),
    filesDiscovered: z.number().int().nonnegative(), filesParsed: z.number().int().nonnegative(), filesFailed: z.number().int().nonnegative(), freshness: z.string()
  }).strict()).max(20).optional(),
  cpg: z.object({
    scopes: z.number().int().nonnegative(), scopesBuilt: z.number().int().nonnegative(), scopesReused: z.number().int().nonnegative(),
    buildTimeMs: z.number().nonnegative(), shardBytes: z.number().int().nonnegative(), analysisFailures: z.number().int().nonnegative(), approximateResults: z.number().int().nonnegative()
  }).strict().optional(),
...
export const emptyIntelligenceOverview = (status: IntelligenceStatus, pendingUpdate = false): IntelligenceOverview => ({
  status,
  pendingUpdate,
  generation: 0,
  runtime: { phase: "ready", queueDepth: 0, activeWorkers: 0, workerCapacity: 0, pendingFiles: 0, completedJobs: 0, failedJobs: 0, staleResultsDiscarded: 0, workerRestarts: 0, throughputFilesPerSecond: 0, currentFiles: [], health: "healthy" },
  counts: { files: 0, symbols: 0, relationships: 0, evidence: 0, packages: 0, tests: 0, routes: 0, externalDependencies: 0, parseFailures: 0, unresolvedReferences: 0, excluded: 0, sensitive: 0, diagnostics: 0 },
  languages: [],
  categories: [],
  symbolTypes: [],
  relationshipTypes: [],
...
export const IntelligenceSearchRequestSchema = z.object({
...
export const IntelligenceSearchItemSchema = z.object({
  id: z.string().min(1),
...
export const IntelligenceRelationshipViewSchema = z.object({
  id: z.string(),
  type: z.string(),
...
export const IntelligenceEntityDetailsSchema = z.object({
  generation: z.number().int().nonnegative(),
...
export const IntelligenceNeighborhoodRequestSchema = z.object({
...
export const IntelligenceNeighborhoodSchema = z.object({
  generation: z.number().int().nonnegative(),
