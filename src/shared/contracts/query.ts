...
export const QueryFiltersSchema = z
  .object({
    ...
    relationshipTypes: z.array(z.string().min(1)).max(50).optional(),
    modules: z.array(z.string().min(1)).max(30).optional(),
    packages: z.array(z.string().min(1)).max(30).optional(),
    languages: z.array(z.string().min(1)).max(30).optional(),
    confidenceAtLeast: z.number().min(0).max(1).default(0),
    capabilityLevels: z
      .array(z.enum(["deep", "semantic", "structural", "metadata-only", "unsupported"]))
      .max(5)
      .optional(),
    derivations: z.array(z.string().min(1)).max(20).optional(),
...
export const TraversalSpecSchema = z
  .object({
    direction: QueryDirectionSchema.default("both"),
    maxDepth: z.number().int().min(1).max(20).default(3),
    stopEntityTypes: z.array(z.string().min(1)).max(30).optional(),
    stopRelationshipTypes: z.array(z.string().min(1)).max(30).optional(),
    pathMode: z
      .enum(["shortest", "shortest-typed", "all-bounded", "highest-confidence", "lowest-risk"])
      .default("shortest"),
  })
  .strict();

export const IncludeSpecSchema = z
  .object({
    ...
    cpg: z.boolean().default(false),
  })
  .strict();
export const RankingSpecSchema = z
  .object({
    strategy: z
      .enum([
        "relevance",
  ...
  .strict();
export const QueryLimitsSchema = z
  .object({
    results: z.number().int().min(1).max(100).default(25),
    nodes: z.number().int().min(1).max(500).default(100),
    edges: z.number().int().min(1).max(1500).default(300),
    paths: z.number().int().min(1).max(20).default(5),
    depth: z.number().int().min(1).max(20).default(3),
    evidence: z.number().int().min(0).max(100).default(30),
    timeBudgetMs: z.number().int().min(10).max(5000).default(1000),
...
export const IntelligenceQuerySchema = z
  .object({
    ...
    seeds: z.array(EntitySelectorSchema).max(10).optional(),
    filters: QueryFiltersSchema.default({ confidenceAtLeast: 0 }),
    traversal: TraversalSpecSchema.default({
      direction: "both",
      maxDepth: 3,
      pathMode: "shortest",
    }),
    include: IncludeSpecSchema.default({
      source: false,
      evidence: true,
      relationships: true,
      diagnostics: true,
    ...
    ranking: RankingSpecSchema.default({ strategy: "relevance", descending: true }),
    limits: QueryLimitsSchema.default({
      results: 25,
      nodes: 100,
      edges: 300,
      paths: 5,
...
export const ResolvedEntitySchema = z
  .object({
    ...
    selected: ResolutionCandidateSchema.optional(),
    candidates: z.array(ResolutionCandidateSchema).max(20),
    ambiguous: z.boolean(),
...
export const QueryPathStepSchema = z
  .object({
    entityId: z.string(),
    entityName: z.string(),
    entityType: z.string(),
    ...
    traversalDirection: QueryDirectionSchema.exclude(["both"]).optional(),
    confidence: z.number().min(0).max(1),
    classification: QueryResultItemSchema.shape.classification,
...
export const QueryFlowMetadataSchema = z
  .object({
    templateId: z.enum([
      "http-persistence",
      "event",
...
export const QueryPathSchema = z
  .object({
    steps: z.array(QueryPathStepSchema).max(50),
    confidence: z.number().min(0).max(1),
    risk: z.number().nonnegative(),
    unsupportedBoundaries: z.array(z.string()).max(20),
    truncated: z.boolean(),
    flow: QueryFlowMetadataSchema.optional(),
  })
  .strict();
...
export const QueryDataSchema = z
  .object({
    kind: z.string().min(1),
    items: z.array(QueryResultItemSchema).max(100).default([]),
    nodes: z.array(QueryResultItemSchema).max(500).default([]),
    relationships: z.array(RelationshipRecordSchema).max(1500).default([]),
    paths: z.array(QueryPathSchema).max(20).default([]),
    sections: z.record(z.string(), z.array(QueryResultItemSchema).max(100)).default({}),
...
export const QueryExplanationSchema = z
  .object({
    parsedAs: z.string(),
    parserRule: z.string(),
    indexesUsed: z.array(z.string()).max(30),
    relationshipFamilies: z.array(z.string()).max(50),
    confidenceThreshold: z.number().min(0).max(1),
    rankingRules: z.array(z.string()).max(30),
    truncationReasons: z.array(z.string()).max(20),
    capabilityBoundaries: z.array(z.string()).max(50),
...
export const EvidenceSummarySchema = EvidenceRecordSchema.pick({
  id: true,
  subjectId: true,
  relativePath: true,
  range: true,
...
export const IntelligenceQueryResultSchema = z
  .object({
    ...
    resolvedSeeds: z.array(ResolvedEntitySchema).max(10),
    plan: QueryPlanSchema,
    data: QueryDataSchema,
    evidence: z.array(EvidenceSummarySchema).max(100),
    diagnostics: z.array(QueryDiagnosticSchema).max(100),
    explanation: QueryExplanationSchema,
    truncated: z.boolean(),
    continuationToken: z.string().optional(),
    cacheState: z.enum(["hit", "miss", "bypassed", "invalidated"]),
  })
  .strict();
export type IntelligenceQueryResult = z.infer<typeof IntelligenceQueryResultSchema>;

export const QueryCompileRequestSchema = z
  .object({
    text: z.string().min(1).max(1000),
    generation: z.number().int().positive().optional(),
    branch: z.string().optional(),
    currentFile: z.string().max(1024).optional(),
    limits: QueryLimitsSchema.partial().optional(),
  })
  .strict();
export const UnifiedQueryRequestSchema = z
  .object({
    text: z.string().min(1).max(1000).optional(),
    query: IntelligenceQuerySchema.optional(),
    currentFile: z.string().max(1024).optional(),
  })
  .strict()
  .refine((value) => Boolean(value.text || value.query), {
    message: "A structured query or deterministic query text is required.",
  });
...
export const QueryCompilationSchema = z
  .object({
    input: z.string(),
    parsed: z.boolean(),
    rule: z.string(),
    query: IntelligenceQuerySchema.optional(),
    diagnostics: z.array(QueryDiagnosticSchema).max(20),
    suggestedTemplates: z.array(z.string()).max(20),
  })
  .strict();
export type QueryCompilation = z.infer<typeof QueryCompilationSchema>;
export const QuerySuggestionRequestSchema = z
  .object({
...
export const QuerySuggestionSchema = z
  .object({
    value: z.string(),
    label: z.string(),
    kind: z.enum(["entity", "template", "relationship", "operation"]),
    detail: z.string(),
    entityId: z.string().optional(),
  })
  .strict();
export const QuerySuggestionsResultSchema = z
  .object({
    generation: z.number().int().nonnegative(),
    items: z.array(QuerySuggestionSchema).max(20),
  })
  .strict();
export const QueryTemplatesResultSchema = z
  .object({
    templates: z
      .array(
        z
...
export const QueryLifecycleEventSchema = z
  .object({
    queryId: z.string().uuid(),
    operation: QueryOperationSchema.optional(),
