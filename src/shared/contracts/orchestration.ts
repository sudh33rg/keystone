...
const AuditSchema = z
  .object({
    ...
    actor: z.enum(["user", "keystone", "recovery"]),
...
const GateSchema = z
  .object({
    ...
    status: z.enum(["pending", "approved", "rejected", "changes-requested", "overridden"]),
    taskId: z.string().uuid().optional(),
    requestedAction: z.string().max(2000),
    evidence: z.array(z.string().max(1000)).max(50),
...
const FindingSchema = z
  .object({
    ...
    blocking: z.boolean(),
    status: z.enum(["open", "accepted", "resolved", "rejected"]),
  })
  .strict();
...
export const WorkflowDefinitionSchema = z
  .object({
    ...
    stages: z
      ...
      .min(1)
      .max(20),
    requiredGates: z.array(GateSchema.shape.kind).max(20),
  })
  .strict();
/**
...
export const WorkflowPolicySchema = z
  .object({
    ...
    requiredApprovals: z.array(GateSchema.shape.kind).max(20),
    maximumRetries: z.number().int().min(0).max(10),
    parallelReadOperations: z.number().int().min(1).max(8),
    serializeWrites: z.literal(true),
    allowedCopilotCapabilities: z.array(z.string().max(100)).max(30),
    contextApproval: z.boolean(),
    validationRequired: z.boolean(),
    qaRequired: z.boolean(),
    securityTriggers: z.array(z.string().max(100)).max(30),
    performanceTriggers: z.array(z.string().max(100)).max(30),
    documentationRequired: z.boolean(),
    completionApproval: z.boolean(),
    deliveryBoundary: z.literal("delivery-ready"),
    stalenessToleranceGenerations: z.number().int().min(0).max(10),
    maxTasks: z.number().int().min(1).max(500),
    maxAudit: z.number().int().min(100).max(5000),
  })
  .strict();
 ...
 * This schema is deprecated in favor of the canonical workflow model.
 */
export const WorkflowTaskStateSchema = z
  .object({
    ...
    readinessBlockers: z.array(DiagnosticSchema).max(100),
    warnings: z.array(DiagnosticSchema).max(100),
    activeExecutionId: z.string().uuid().optional(),
    ...
    repositoryBaselineFingerprint: z.string().max(500).optional(),
      ...
      .array(
        z
          .object({
            ...
            optional: z.boolean(),
            omitted: z.boolean(),
            omissionReason: z.string().max(1000).optional(),
            taskIds: z.array(z.string().uuid()).max(500),
            gates: z.array(GateSchema.shape.kind).max(20),
          })
          .strict(),
      )
      .max(20),
    schedule: z.array(z.string().uuid()).max(500),
    conflictFingerprint: z.string().max(500),
    createdAt: z.string().datetime(),
...
export const WorkflowInstanceSchema = z
  .object({
    schemaVersion: z.literal(ORCHESTRATION_SCHEMA_VERSION),
    id: z.string().uuid(),
    ...
    staleness: z.array(StalenessRecordSchema).max(200).default([]),
    definitionId: WorkflowDefinitionIdSchema,
    policyProfileId: z.string().max(100),
    status: WorkflowStatusSchema,
    currentStage: WorkflowStageSchema.optional(),
    taskStates: z.array(WorkflowTaskStateSchema).max(500),
    approvalGates: z.array(GateSchema).max(1000),
    activeExecutionIds: z.array(z.string().uuid()).max(100),
    validationRunIds: z.array(z.string().uuid()).max(500),
    ...
    metrics: z
      .object({
        ...
        waitingForUserMs: z.number().nonnegative(),
 ...
 * This schema is deprecated in favor of the canonical workflow model.
 */
export const OrchestrationPersistentStateSchema = z
  .object({
    schemaVersion: z.literal(ORCHESTRATION_SCHEMA_VERSION),
    revision: z.number().int().nonnegative(),
    instances: z.array(WorkflowInstanceSchema).max(100),
    updatedAt: z.string().datetime(),
  })
  .strict();
 ...
 * This schema is deprecated in favor of the canonical workflow model.
 */
export const OrchestrationCreatePayloadSchema = z
  .object({
    workflowId: z.string().uuid(),
  ...
  .strict();
export const OrchestrationIdPayloadSchema = z
  .object({ orchestrationId: z.string().uuid() })
  .strict();
/**
 * @deprecated Use WorkflowDecisionPayloadSchema from '@keystone/shared/contracts/workflow' instead.
 * This schema is deprecated in favor of the canonical workflow model.
 */
export const OrchestrationDecisionPayloadSchema = OrchestrationIdPayloadSchema.extend({
  gateId: z.string().uuid(),
  decision: z.enum(["approved", "rejected", "changes-requested", "overridden"]),
  reason: z.string().min(1).max(2000),
 ...
 * This schema is deprecated in favor of the canonical workflow model.
 */
export const OrchestrationTaskPayloadSchema = OrchestrationIdPayloadSchema.extend({
  taskId: z.string().uuid(),
}).strict();
/**
 * @deprecated Use the new workflow model from '@keystone/shared/contracts/workflow'.
 * This schema is deprecated in favor of the canonical workflow model.
 */
export const OrchestrationTaskActionPayloadSchema = OrchestrationTaskPayloadSchema.extend({
  reason: z.string().min(1).max(2000),
 ...
 * This schema is deprecated in favor of the canonical workflow model.
 */
export const OrchestrationReviewPlanSchema = z
  .object({
    category: z.enum(["qa", "security", "performance", "documentation", "validation"]),
    triggered: z.boolean(),
    reasons: z.array(z.string().max(2000)).max(50),
    taskIds: z.array(z.string().uuid()).max(500),
    requiredChecks: z.array(z.string().max(500)).max(100),
 ...
 * This schema is deprecated in favor of the canonical workflow model.
 */
export const OrchestrationFindingPayloadSchema = OrchestrationIdPayloadSchema.extend({
 ...
 * This schema is deprecated in favor of the canonical workflow model.
 */
export const OrchestrationRecoverPayloadSchema = OrchestrationIdPayloadSchema.extend({
  repositoryId: z.string().max(1000),
  branch: z.string().max(500),
