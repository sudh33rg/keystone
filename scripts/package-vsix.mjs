...
if(!fs.existsSync(extensionEntry)) throw new Error('Build output is missing. Run npm run build first.');
const stage=fs.mkdtempSync(path.join(os.tmpdir(),'keystone-vsix-'));
try {
  const ext=path.join(stage,'extension'); fs.mkdirSync(ext,{recursive:true});
  for(const name of ['package.json','README.md']) fs.copyFileSync(path.join(root,name),path.join(ext,name));
...
function copy(src,dst){const stat=fs.statSync(src);if(stat.isDirectory()){fs.mkdirSync(dst,{recursive:true});for(const entry of fs.readdirSync(src))copy(path.join(src,entry),path.join(dst,entry));}else{fs.mkdirSync(path.dirname(dst),{recursive:true});fs.copyFileSync(src,dst);}}
