...
try {
  ...
  const run = async (mode) => {
    ...
    await new Promise((resolve, reject) => {
      ...
      child.once("exit", (code) =>
        code === 0
          ? resolve()
          : reject(new Error(`Production ${mode} verifier exited with ${code}.`))
      );
    });
  ...
  assert(
    index.status === "ready",
    "Built production CockpitService did not complete persisted indexing."
  );
  assert(
    index.okfValid === true,
    "Built production CockpitService did not promote a validated OKF snapshot."
  );
  assert(
    query.queryResults > 0 && query.queryEvidenceResults > 0,
    "Persisted production snapshot returned no provenance-backed query result."
  );
  assert(
    /^okf-/.test(analyze.intentRetrievalMode),
    "Production intent retrieval did not use authoritative OKF first."
  );
  assert(
    analyze.readOnlyGitEvidence === true,
    "Production intent analysis did not attach read-only Git evidence."
  );
  ...
  await fsp.writeFile(
    path.join(root, "dist", "evidence", "production-cockpit.json"),
...
async function removeTreeConcurrent(target) {
  ...
  for (let start = 0; start < files.length; start += 256) {
    await Promise.all(
      files.slice(start, start + 256).map((file) => fsp.unlink(file).catch(() => undefined))
    );
  }
...
async function copyActualProject(sourceRoot, targetRoot) {
  ...
  for (const directory of directories) {
    ...
    if (fs.existsSync(source))
      await fsp.cp(source, path.join(targetRoot, directory), { recursive: true });
  }
...
async function sourceFingerprint(sourceRoot) {
  ...
  for (const directory of ["src", "tests", "scripts"])
    await hashTree(path.join(sourceRoot, directory), sourceRoot, hash);
  for (const file of [
    "package.json",
    "package-lock.json",
    "tsconfig.json",
...
async function hashTree(directory, sourceRoot, hash) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) await hashTree(full, sourceRoot, hash);
      ...
      hash
        .update(path.relative(sourceRoot, full).replaceAll(path.sep, "/"))
