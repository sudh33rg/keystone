...
function minimalIntelligence() {
  ...
  return {
    ...
    files: [
      ...
      { path: "c.ts", language: "typescript", sizeBytes: 10, lineCount: 10, contentHash: "h3", structuralHash: "s3", isTest: false, isGenerated: false, evidence: { source: "repo-intelligence", evidencePath: "c.ts" } }
    ],
    symbols: [
      { name: "doWork", kind: "function", filePath: "b.ts", line: 2, exportStatus: "exported", evidence: { source: "repo-intelligence", evidencePath: "b.ts" } },
      { name: "doWork", kind: "function", filePath: "c.ts", line: 2, exportStatus: "exported", evidence: { source: "repo-intelligence", evidencePath: "c.ts" } },
      { name: "run", kind: "function", filePath: "a.ts", line: 1, exportStatus: "local", evidence: { source: "repo-intelligence", evidencePath: "a.ts" } }
    ],
    dependencies: [
      { from: "a.ts", to: "b.ts", kind: "import", evidence: { source: "repo-intelligence", evidencePath: "a.ts" } },
      { from: "a.ts", to: "c.ts", kind: "import", evidence: { source: "repo-intelligence", evidencePath: "a.ts" } }
    ],
    tests: [],
    apis: [],
    services: [],
    ...
    calls: [
      { filePath: "a.ts", caller: "run", callee: "helper.doWork", line: 3, evidence: { source: "repo-intelligence", evidencePath: "a.ts" } }
    ],
    ownershipHints: [],
    frameworkHints: [],
...
function main() {
  ...
  const unresolved = snapshot.observations.filter((o) => o.predicate === "keystone:unresolvedCallee");

  assert.ok(calls.length >= 1, "expected at least one 'calls' relationship");
  const resolved = calls.find((r) => nameOf(r.sourceId) === "run" && nameOf(r.targetId) === "doWork");
  assert.ok(resolved, "helper.doWork must resolve to a 'calls' edge from run -> doWork via import scope");
  assert.equal(unresolved.filter((o) => o.value === "helper.doWork").length, 0, "helper.doWork must NOT be logged as unresolved");

  console.log("PASS call-resolution: base.method 'helper.doWork' resolved via import scope (no global fallback, no unresolvedCallee)");
}
