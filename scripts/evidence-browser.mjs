...
const { ApplicationStore } = require(
  path.join(root, "dist/app/core/application/applicationStore.js")
);
const { CockpitService } = require(
  path.join(root, "dist/app/core/integration/webview/cockpitService.js")
);
const { startBrowserViewServer } = require(
  path.join(root, "dist/app/extension/browser-view/browserViewServer.js")
);
...
await fsp.cp(path.join(root, "tests", "fixtures", "extension-workspace"), evidenceRoot, {
  recursive: true
});
...
await fsp.writeFile(
  path.join(evidenceRoot, ".github", "agents", "order-review.agent.md"),
  "# Order Review Agent\nReview order changes against repository evidence.\n"
);
await fsp.writeFile(
  path.join(evidenceRoot, ".github", "skills", "order-safety", "SKILL.md"),
  "# Order safety\n- Preserve calculation behavior\n- Run impacted tests\n"
);
await fsp.writeFile(
  path.join(evidenceRoot, ".github", "instructions", "orders.instructions.md"),
...
if (indexed.status !== "ready")
  throw new Error("Live evidence repository did not complete persisted indexing.");
...
if (!initialQuery.items.length)
  throw new Error("Live evidence query returned no persisted OKF evidence.");
...
const store = new ApplicationStore({
  ...
  notification: {
    level: "info",
    message: `${initialQuery.answer} (${initialQuery.items.length} evidence-backed result(s))`
  }
});
...
const dispatch = async (message) => {
  ...
  store.update({
    notification: {
      level: "info",
      message: `${message.type} accepted by the live evidence Browser View. Use the installed VS Code extension for IDE-only actions.`
    }
  });
};

handle = await startBrowserViewServer({
  mediaRoot: path.join(root, "dist/media"),
  store,
  dispatch
});
await fsp.mkdir(path.join(root, "docs", "evidence"), { recursive: true });
await fsp.writeFile(
  path.join(root, "docs", "evidence", "live-browser-state.json"),
  `${JSON.stringify(
    {
      ...
      indexed: {
        ...
        files: indexed.intelligence?.fileCount,
        okf: indexed.intelligence?.okf
      },
      query: initialQuery,
      task,
      plan
...
function createPlan(sdlcEngine, text, result, architecture) {
  return sdlcEngine.createPlan(text, {
    ...
    evidence: (result.evidence ?? []).map((item) => ({
      ...item,
      summary:
        item.summary ?? item.reason ?? item.label ?? "Repository evidence selected by Keystone."
    })),
    ...
    constraints: [
      ...
      "Git and remote merge-request access remain read-only."
    ]
  });
}
