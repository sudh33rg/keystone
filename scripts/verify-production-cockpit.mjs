...
if (!workspaceRoot) throw new Error('Usage: node scripts/verify-production-cockpit.mjs <workspace-root> <index|query|analyze>');
const { CockpitService } = require(path.join(process.cwd(), 'dist/app/core/integration/webview/cockpitService.js'));
...
let result;
if (mode === 'index') {
  const state = await service.index((message, progress, stage) => { if (process.env.KEYSTONE_VERIFY_PROGRESS === '1') console.error(`[production-index] ${Date.now() - started}ms ${progress}% ${stage}: ${message}`); });
  result = {
    mode,
    status: state.status,
  ...
  const query = await service.queryIntelligence('What tests are impacted by changing the OKF query engine?');
  result = {
    mode,
  ...
  const task = await service.analyze('Improve the OKF query engine while preserving impacted tests and read-only Git review.', { currentFile: 'src/core/intelligence/okf/queryEngine.ts' });
  result = {
    mode,
