...
type Asymmetric = {
  __keystoneMatcher: "objectContaining" | "arrayContaining" | "any";
  value: unknown;
};

function isAsymmetric(value: unknown): value is Asymmetric {
  return Boolean(value && typeof value === "object" && "__keystoneMatcher" in value);
}
function containsAsymmetric(value: unknown): boolean {
  if (isAsymmetric(value)) return true;
  if (Array.isArray(value)) return value.some(containsAsymmetric);
  if (value && typeof value === "object")
    return Object.values(value as Record<string, unknown>).some(containsAsymmetric);
  return false;
}

function deepSubset(actual: unknown, expected: unknown): boolean {
  if (isAsymmetric(expected)) {
    if (expected.__keystoneMatcher === "objectContaining")
      return deepSubset(actual, expected.value);
    if (expected.__keystoneMatcher === "arrayContaining")
      return (
        Array.isArray(actual) &&
        Array.isArray(expected.value) &&
        expected.value.every((item) => actual.some((candidate) => deepSubset(candidate, item)))
      );
    if (expected.__keystoneMatcher === "any") {
      const ctor = expected.value as Function;
      return ctor === String
        ...
        : ctor === Number
          ...
          : ctor === Boolean
            ...
            : typeof ctor === "function" &&
              actual instanceof (ctor as new (...args: never[]) => unknown);
    }
  }
  ...
  if (Array.isArray(expected))
    return (
      Array.isArray(actual) &&
      expected.every((value, index) => deepSubset((actual as unknown[])[index], value))
    );
...
function matcher(actual: unknown, negate = false) {
  ...
  const api = {
    get not() {
      return matcher(actual, !negate);
    },
    toBe(expected: unknown) {
      check(
        Object.is(actual, expected),
        `Expected ${String(actual)} ${negate ? "not " : ""}to be ${String(expected)}`
      );
    },
    toEqual(expected: unknown) {
      let ok = containsAsymmetric(expected) ? deepSubset(actual, expected) : true;
      ...
      check(ok, `Expected values ${negate ? "not " : ""}to be deeply equal`);
    },
    toMatchObject(expected: unknown) {
      check(deepSubset(actual, expected), `Expected value ${negate ? "not " : ""}to match object`);
    },
      ...
      check(
        typeof actual === "string"
          ...
          : Array.isArray(actual)
            ...
            : false,
        `Expected value ${negate ? "not " : ""}to contain ${String(expected)}`
      );
    },
    toContainEqual(expected: unknown) {
      check(
        Array.isArray(actual) &&
          actual.some((value) =>
            containsAsymmetric(expected)
              ? deepSubset(value, expected)
              : (() => {
                  try {
        ...
        `Expected array ${negate ? "not " : ""}to contain equal value`
      );
    },
    ...
    toMatch(expected: RegExp | string) {
      check(
        ...
        `Expected ${String(actual)} to match ${String(expected)}`
      );
    },
    toHaveBeenCalledOnce() {
      check(
        typeof actual === "function" &&
          Array.isArray((actual as MockFunction).mock?.calls) &&
          (actual as MockFunction).mock.calls.length === 1,
    ...
    toThrow(expected?: RegExp | string) {
      ...
      check(
        threw && matches,
        `Expected function to throw${expected ? ` ${String(expected)}` : ""}`
      );
    }
  };
  return api;
...
function promiseMatcher(actual: Promise<unknown>, mode: "resolves" | "rejects") {
  const create =
    ...
    async (): Promise<void> => {
      ...
      const target =
        name === "toMatch" && mode === "rejects" && value instanceof Error ? value.message : value;
...
export function expect(actual: unknown) {
  ...
  return base as ReturnType<typeof matcher> & {
    resolves: ReturnType<typeof promiseMatcher>;
    rejects: ReturnType<typeof promiseMatcher>;
  };
}
...
expect.objectContaining = (value: unknown): Asymmetric => ({
  __keystoneMatcher: "objectContaining",
  value
});
expect.arrayContaining = (value: unknown[]): Asymmetric => ({
  __keystoneMatcher: "arrayContaining",
  value
});
