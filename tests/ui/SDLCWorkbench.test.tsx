...
describe("SDLCWorkbench", () => {
  it("creates a single workflow draft and navigates directly to Define", async () => {
    ...
    const request = vi.fn((type: string) =>
      Promise.resolve(
        type === "workbench/getCreateContext"
          ? {
              schemaVersion: 1,
              repository: {
                available: true,
              ...
              workflowDefinitions: [
                ...
                },
                {
                  ...
                  label: "Modernization",
                  description: "Modernization work",
                },
              ],
            }
          : { id: workflowId },
    ...
    fireEvent.change(screen.getByLabelText("Work intent"), {
      target: { value: "Modernize the repository adapter" },
    });
    fireEvent.change(screen.getByLabelText("Repository scope"), { target: { value: "paths" } });
    fireEvent.change(screen.getByLabelText("Repository paths"), {
      target: { value: "src/adapters, tests/adapters.test.ts" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Start workflow" }));

    await waitFor(() =>
      expect(request).toHaveBeenCalledWith("workbench/createWorkflow", {
        workType: "modernization",
        intent: "Modernize the repository adapter",
        repositoryScope: { kind: "paths", paths: ["src/adapters", "tests/adapters.test.ts"] },
        constraints: [],
        expectedRepositoryId: "repository:fixture",
  ...
  it("does not permit a repository workflow without a ready trusted repository", async () => {
    const request = vi.fn().mockResolvedValue({
      schemaVersion: 1,
      repository: { available: false, trusted: false },
      intelligence: { status: "unavailable", generation: 0, message: "Unavailable" },
      copilot: { available: false, directInvocation: false, summary: "Unavailable" },
      workflowDefinitions: [],
    });
    render(
    ...
    expect(screen.getByRole("button", { name: "Start workflow" }).hasAttribute("disabled")).toBe(
      true,
    );
