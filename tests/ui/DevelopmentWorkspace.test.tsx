...
describe("DevelopmentWorkspace", () => {
  it("captures intent through the typed workflow request and renders repository resolution", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Development intent"), {
      target: { value: "Implement safe delegation" },
    });
    ...
    expect(
      screen.getByText(
        (_content, element) =>
          element?.tagName === "P" &&
          element.textContent === "ResolvedSymbol — Exact intelligence result.",
      ),
    ).toBeTruthy();
    expect(request).toHaveBeenCalledWith("workflow/capture", {
      text: "Implement safe delegation",
      mode: "guided",
    });
  });
  ...
  it("shows unavailable capability state and never renders a fabricated default agent", async () => {
    ...
    const request = vi.fn((type: string) => {
      ...
      if (type === "copilot/capabilities") return Promise.resolve(capabilities(false));
      if (type === "copilot/agents") return Promise.resolve([]);
      if (type === "context/get" || type === "delegation/getPrompt")
        return Promise.resolve(undefined);
      return Promise.resolve(undefined);
    });
...
function workflowFixture(withTask: boolean): DevelopmentWorkflowSnapshot {
  ...
  const specId = "10000000-0000-4000-8000-000000000002";
  ...
  return {
    ...
    intent: {
      ...
      workflowId: id,
      revision: 1,
      originalText: "Implement safe delegation",
      normalizedObjective: "Implement safe delegation",
      mode: "guided",
      category: "feature",
      expectedOutcome: "Safe delegation",
      risk: "medium",
      constraints: [],
      ambiguities: [],
      requiredDecisions: [],
      affectedEntities: [
        {
          ...
          reason: "Exact intelligence result.",
          confidence: 1,
    ...
    specification: {
      ...
      baseCommit: "head",
      intelligenceGeneration: 4,
      objective: "Implement safe delegation",
      scope: {
        included: ["delegation"],
        excluded: [],
      ...
      requirements: [{ id: "REQ-1", description: "Safe delegation" }],
      constraints: [],
      acceptanceCriteria: [
        {
          id: "AC-1",
          ...
          coveringTaskIds: withTask ? [taskId] : [],
        },
      ],
      testStrategy: {
        existingTests: [],
