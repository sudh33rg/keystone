...
describe("HostBridge", () => {
  ...
  it("preserves typed universal-adapter coverage responses", async () => {
    const postMessage = vi.fn();
    const bridge = new HostBridge({ postMessage, getState: () => undefined, setState: vi.fn() });
    const coverage = { generation: 2, total: 1, items: [{ technologyId: "typescript", adapterId: "keystone.typescript", adapterVersion: "1.0.0", capabilityLevel: "semantic", filesDiscovered: 1, filesParsed: 1, filesFailed: 0, filesMetadataOnly: 0, entitiesExtracted: 3, relationshipsResolved: 2, unresolvedReferences: 0, unsupportedConstructs: 0, lastSuccessfulUpdate: "2026-07-15T00:00:00.000Z", freshness: "current" }], detections: [] };
    ...
    window.dispatchEvent(new MessageEvent("message", { data: hostMessage("response/success", { requestId: coverageRequest.requestId, data: coverage }) }));

    const diagnostics = { generation: 2, total: 1, items: [{ code: "partial-support", severity: "info", message: "Structural extraction only.", adapterId: "keystone.adapter.contract", limitation: true }] };
    ...
    window.dispatchEvent(new MessageEvent("message", { data: hostMessage("response/success", { requestId: diagnosticsRequest.requestId, data: diagnostics }) }));
