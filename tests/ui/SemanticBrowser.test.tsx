...
describe("SemanticBrowser", () => {
  it("uses bounded typed search, inspector, source, and neighborhood requests", async () => {
    const request = vi.fn((type: string, _payload?: unknown, _options?: unknown) => {
      ...
      if (type === "intelligence/entity")
        return Promise.resolve({
          ...
          entity: {
            ...
            confidence: 1,
            generation: 3,
            sourceRange: { startLine: 4, startColumn: 2, endLine: 6, endColumn: 3 },
          },
          incoming: [],
          outgoing: [
            {
              ...
              confidence: 1,
              derivation: "resolved",
              evidenceIds: ["evidence:calls"],
            },
          ],
          evidence: [
            {
              id: "evidence:run",
              subjectId: "entity:run",
              ...
              generation: 3,
              confidence: 1,
              statement: "Compiler declaration.",
            },
          ],
          diagnostics: [],
          truncated: false,
        });
      if (type === "intelligence/neighborhood")
        return Promise.resolve({
          generation: 3,
          nodes: [
            {
      ...
      if (type === "intelligence/technologies")
        return Promise.resolve({
          generation: 3,
          total: 1,
          items: [
            {
              technologyId: "openapi",
      ...
      if (type === "intelligence/adapter-diagnostics")
        return Promise.resolve({
          generation: 3,
          total: 1,
          items: [
            {
              ...
              severity: "info",
              message: "Only structural contract parsing is available.",
              adapterId: "keystone.adapter.contract",
              technologyId: "openapi",
              limitation: true,
            },
    ...
    fireEvent.change(screen.getByLabelText("Search semantic intelligence"), {
      target: { value: "run" },
    });
    ...
    expect(request).toHaveBeenCalledWith("intelligence/neighborhood", {
      ids: ["entity:run"],
      direction: "both",
      maxDepth: 1,
      maxNodes: 30,
    ...
    expect(request).toHaveBeenCalledWith("intelligence/source/open", {
      relativePath: "src/service.ts",
...
function cpgResult() {
  ...
  const node = (id: string, kind: "ENTRY" | "EXIT" | "PARAMETER", code?: string) => ({
    id,
    fileId: "file:service",
    scopeId: "cpg-scope:run",
    semanticSymbolId: "entity:run",
    kind,
    ...(code ? { code, range } : {}),
    evidenceIds: ["evidence:run"],
    parserVersion: "1",
    generation: 3,
    ...(kind === "PARAMETER" ? { properties: { variable: "input", read: true } } : {}),
  });
  const scope = {
    id: "cpg-scope:run",
    fileId: "file:service",
    ...
    structuralHash: "sha256:scope",
    providerId: "keystone.typescript-cpg",
    providerVersion: "1",
    schemaVersion: 1 as const,
    analysisLevel: "basic" as const,
    generation: 3,
    nodeCount: 3,
    edgeCount: 2,
    summary: {
    ...
    shard: "cpg/scopes/run.json.gz",
  };
  const edge = (id: string, sourceId: string, targetId: string) => ({
    id,
    sourceId,
    targetId,
    type: "CFG_NEXT" as const,
    derivation: "calculated" as const,
    confidence: 1,
    evidenceIds: ["evidence:run"],
    fileId: "file:service",
    scopeId: "cpg-scope:run",
  ...
  return {
    ...
    nodes: [
      ...
      node("node:param", "PARAMETER", "input"),
      node("node:exit", "EXIT"),
    ],
    edges: [
      edge("edge:one", "node:entry", "node:param"),
      edge("edge:two", "node:param", "node:exit"),
    ],
    diagnostics: [],
    truncated: false,
  };
