...
describe("ExecutionValidationWorkspace", () => {
  it("shows the honest prerequisite when no execution session exists", async () => { const request = vi.fn(() => Promise.resolve([])); render(<ExecutionValidationWorkspace bridge={{ request } as unknown as HostBridge}/>); expect(await screen.findByText("No validation session for this workflow")).toBeTruthy(); expect(request).toHaveBeenCalledWith("execution/list", {}); });

  it("does not expose another workflow's execution session", async () => {
    const workflowId = crypto.randomUUID();
    const request = vi.fn(() => Promise.resolve([{ id: crypto.randomUUID(), workflowId: crypto.randomUUID(), taskId: crypto.randomUUID() }]));
    render(<ExecutionValidationWorkspace bridge={{ request } as unknown as HostBridge} workflowId={workflowId} onReturnToBuild={vi.fn()}/>);
