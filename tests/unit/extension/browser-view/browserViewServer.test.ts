...
import { startBrowserViewServer, type BrowserViewHandle } from '../../../../src/extension/browser-view/browserViewServer';
...
function assets(): string {
  ...
  roots.push(root);
  fs.writeFileSync(path.join(root, 'index.html'), '<!doctype html><title>Keystone</title><script src="/webview.js"></script>');
  fs.writeFileSync(path.join(root, 'webview.js'), 'console.log("keystone")');
  return root;
}

async function bootstrap(handle: BrowserViewHandle): Promise<{ origin: string; cookie: string; url: string; setCookie: string }> {
...
async function command(session: { origin: string; cookie: string }, message: unknown, expectedStateVersion: number, origin = session.origin): Promise<Response> {
...
describe('Browser View server', () => {
  it('uses a one-time bootstrap and an HttpOnly same-origin session', async () => {
    ...
    const handle = await startBrowserViewServer({ mediaRoot: assets(), store, dispatch: message => { dispatched.push(message); } });
    handles.push(handle);
    ...
    expect(String((await fetch(`${session.origin}/`, { headers: { cookie: session.cookie } })).headers.get('content-security-policy'))).toMatch(/frame-ancestors 'none'/);
    ...
    const crossOrigin = await command(session, { type: 'LOAD_INTELLIGENCE' }, snapshot.version, 'https://attacker.invalid');
    ...
    const accepted = await command(session, { type: 'LOAD_INTELLIGENCE' }, store.snapshot().version);
  ...
  it('sends current and subsequent shared state to reconnecting clients', async () => {
    ...
    const handle = await startBrowserViewServer({ mediaRoot: assets(), store, dispatch: () => undefined });
    handles.push(handle);
    const session = await bootstrap(handle);

    for (let connection = 0; connection < 2; connection += 1) {
      const controller = new AbortController();
      const response = await fetch(`${session.origin}/events`, { headers: { cookie: session.cookie }, signal: controller.signal });
