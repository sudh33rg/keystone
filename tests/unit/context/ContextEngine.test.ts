...
import {
  ...
  ContextCompressor,
  ContextRanker,
} from "../../../src/core/context/ContextEngine";
import { ContextItemSchema } from "../../../src/shared/contracts/delegation";

function candidate(
  id: string,
  content: string,
  options: { required?: boolean; pinned?: boolean; priority?: number } = {},
) {
...
describe("deterministic context ranking and compression", () => {
  it("keeps mandatory task context ahead of optional candidates", () => {
    const ranked = new ContextRanker().rank([
      candidate("optional", "source", { priority: 900 }),
      candidate("required", "objective", { required: true }),
    ]);
  ...
  it("protects required items and removes optional items first at the budget", () => {
    ...
    const result = new ContextCompressor().compress(
      [
        candidate("required", "required", { required: true }),
        candidate("optional", "x".repeat(2_100)),
      ],
      budget,
    );
