...
import {
  ...
  CopilotAgentDiscoveryService,
} from "../../../src/core/copilot/AgentRegistry";
import {
  CapabilityDrivenCopilotAdapter,
  CopilotCapabilityDetector,
  type CopilotEnvironment,
} from "../../../src/core/copilot/CopilotAdapter";
import {
  DelegationFallbackService,
  DelegationPromptBuilder,
  TaskEligibilityService,
} from "../../../src/core/copilot/DelegationService";
import {
...
import { ContextPackageSchema } from "../../../src/shared/contracts/contextPackage";
    ...
    evidence: [
      {
        kind: "runtime",
        source: "fixture",
        statement: "Returned by a supported discovery contract.",
      },
    ],
    ...overrides,
  });
...
function task() {
  return DevelopmentTaskSchema.parse({
    ...
    objective: "Add bounded delegation",
    ...
    expectedEntityIds: ["symbol:feature"],
    validationSteps: [{ command: "npm test" }],
    assignedAgentId: "agent.implementation",
    requiredCapabilities: ["implementation", "testing"],
    staleReasons: [],
    baseEntityFingerprints: { "file:src/feature.ts": "sha256:base" },
    createdAt: NOW,
    updatedAt: NOW,
  });
}
...
function specification() {
  return DevelopmentSpecificationSchema.parse({
    ...
    scope: {
      ...
      excluded: ["validation"],
      expectedFiles: ["src/feature.ts"],
      entityIds: ["symbol:feature"],
    },
    requirements: [{ id: "REQ-1", description: "Delegate only approved work." }],
    constraints: ["Do not modify unrelated files."],
    acceptanceCriteria: [
      {
        id: "AC-1",
        description: "Approval gates execution.",
        required: true,
        requirementIds: ["REQ-1"],
        validationMethod: "npm test",
        expectedEvidence: "passing output",
        coveringTaskIds: [IDS.task],
      },
    ],
    testStrategy: {
      existingTests: [],
      requiredTests: ["approval gate"],
...
function context(reviewed = true) {
  return ContextPackageSchema.parse({
    schemaVersion: 3,
    id: IDS.context,
    contentFingerprint: "sha256:context",
    workflowId: IDS.workflow,
    stageId: "development",
    executionProfileId: "agent.implementation",
    sourceSnapshot: {
      intelligenceRevision: "7",
      specificationRevision: "1",
      instructionRevisionHash: "sha256:instruction",
      createdAt: NOW,
    },
    budget: {
    ...
    metadata: {
      status: reviewed ? "approved" : "ready",
      createdAt: NOW,
      updatedAt: NOW,
...
function workflow() {
  ...
  const intent = DevelopmentIntentSchema.parse({
    ...
    workflowId: IDS.workflow,
    revision: 1,
    originalText: "Add bounded delegation",
    normalizedObjective: "Add bounded delegation",
    mode: "spec-driven",
    category: "feature",
    expectedOutcome: "Bounded delegation",
    risk: "medium",
    constraints: [],
    ambiguities: [],
    requiredDecisions: [],
...
describe("capability-driven Copilot integration", () => {
  ...
  it("does not infer direct invocation or discovery from chat commands", async () => {
    const result = await new CopilotCapabilityDetector(
      environment({
        listExtensions: () =>
          Promise.resolve([{ id: "github.copilot-chat", version: "1.2.3", active: true }]),
        listCommands: () =>
          Promise.resolve(["workbench.action.chat.open", "workbench.action.chat.openEditSession"]),
      }),
    ).detect();
  ...
  it("discovers agents only through an explicitly supported runtime method", async () => {
    ...
    const adapter = new CapabilityDrivenCopilotAdapter(
      environment({
        integrationMethods: () => Promise.resolve(["agent-discovery-v1"]),
        discoverAgents,
      }),
    );
    expect(await adapter.discoverAgents()).toMatchObject([
      { id: discovered.id, availability: "available", source: "copilot-discovered" },
    ]);
...
describe("agent profiles, recommendation, and selection", () => {
  it("keeps configured availability unknown and inherits alias evidence only from its explicit target", () => {
    ...
    const configured = agent({
      ...
      evidence: [{ kind: "configuration", source: "settings", statement: "Inert metadata." }],
    });
    const alias = agent({
      id: "alias",
      displayName: "Alias",
      ...
      capabilities: [],
      taskCategories: [],
      aliasFor: discovered.id,
      evidence: [{ kind: "alias", source: "settings", statement: "Explicit alias." }],
    });
    ...
    expect(merged.find((item) => item.id === "alias")).toMatchObject({
      availability: "available",
      capabilities: ["implementation", "testing"],
    });
  });
  ...
  it("ranks deterministically and exposes every match and penalty", () => {
    const result = new AgentRecommendationService().recommend(task(), [
      agent(),
      agent({
        id: "agent.docs",
        displayName: "Docs",
    ...
    expect(result.candidates[0]).toMatchObject({
      agent: { id: "agent.implementation" },
      matchingCapabilities: ["implementation", "testing"],
      missingCapabilities: [],
    });
  ...
  it("requires confirmation and rejects unavailable selections", async () => {
    ...
    await expect(selection.select(IDS.task, agent(), false)).rejects.toThrow(
      "explicit user confirmation",
    );
    await expect(
      selection.select(IDS.task, agent({ availability: "unavailable" }), true),
...
describe("prompt, fallback, and eligibility gates", () => {
      ...
      fallback.mode(
        {
          ...base,
          chatAvailable: true,
          directInvocationAvailable: true,
          promptInsertionAvailable: true,
        },
        agent(),
      ),
    ).toBe("direct");
  ...
  it("blocks unreviewed context and an unapproved exact prompt", () => {
    const result = new TaskEligibilityService().check({
      workflow: workflow(),
      task: task(),
    ...
    expect(result.reasons.map((item) => item.code)).toEqual(
      expect.arrayContaining(["context-unreviewed", "delegation-unapproved"]),
    );
  });
});
