...
describe("repository intelligence heuristics", () => {
  ...
  it("retains specific security and performance evidence", () => {
    const source =
      "const credential = readSecret(); const rows = database.query({ pagination: true });";
    expect(detectSecuritySensitiveArea("src/auth.ts", source)).toEqual(
      expect.arrayContaining(["auth", "credential", "secret"])
    );
    expect(detectPerformanceSensitivePath("src/store.ts", source)).toEqual(
      expect.arrayContaining(["database", "query", "pagination"])
    );
  });

  it("does not emit control-flow keywords as method symbols", () => {
    const symbols = analyzeLanguageFile(
      "src/example.ts",
