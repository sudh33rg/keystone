...
describe('CockpitService persisted intelligence path', () => {
  it('indexes through the production service, promotes valid OKF, queries relationships, and uses OKF for intent context', async () => {
    ...
    await fs.writeFile(path.join(root, 'test', 'api.config.test.ts'), "import express from 'express';\nconst app=express();\napp.get('/health',()=>true);\n", 'utf8');
    await fs.writeFile(path.join(root, 'vite.config.ts'), "import { orderTotal } from './src/orders';\nexport default { orderTotal };\n", 'utf8');
    ...
    await fs.writeFile(path.join(root, '.github', 'agents', 'order-review.agent.md'), '# Order Review Agent\nReview order changes against repository evidence.\n', 'utf8');
    await fs.writeFile(path.join(root, '.github', 'skills', 'order-safety', 'SKILL.md'), '# Order safety\n- Preserve calculation behavior\n- Run impacted tests\n', 'utf8');
    await fs.writeFile(path.join(root, '.github', 'instructions', 'orders.instructions.md'), '# Order instructions\n- Do not change public order contracts without evidence\n', 'utf8');
    try {
      ...
      const task = await service.analyze('Change orderTotal and identify impacted tests.', { currentFile: 'src/orders.ts' });
      ...
      expect(task.copilotCustomizations?.agents.some(agent => agent.name === 'order-review')).toBe(true);
      expect(task.copilotCustomizations?.skills.some(skill => skill.name === 'order-safety')).toBe(true);
      expect(task.copilotCustomizations?.instructions.some(item => item.path.endsWith('orders.instructions.md'))).toBe(true);
