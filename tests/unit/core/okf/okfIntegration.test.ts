...
function fixture(): string {
  ...
  roots.push(root);
  for (const directory of ['src', 'tests', 'db', 'config']) fs.mkdirSync(path.join(root, directory), { recursive: true });
  fs.writeFileSync(path.join(root, 'package.json'), JSON.stringify({ dependencies: { express: '^5.0.0' }, scripts: { test: 'vitest' } }));
  fs.writeFileSync(path.join(root, 'README.md'), '# Fixture\nDocuments the user service.');
  fs.writeFileSync(path.join(root, 'config', 'app.yaml'), 'service: user\ntimeout: 30');
  fs.writeFileSync(path.join(root, 'db', 'schema.sql'), 'CREATE TABLE users (id INTEGER, password TEXT);');
  fs.writeFileSync(path.join(root, 'src', 'base.ts'), 'export class Base {}\nexport interface Contract {}\nexport function helper(){ return true; }');
  ...
  fs.writeFileSync(path.join(root, 'tests', 'userService.test.ts'), "import { Child } from '../src/userService';\ndescribe('Child', () => it('runs', () => new Child().run()));");
  return root;
}
...
describe('authoritative OKF intelligence', () => {
  it('produces every profile family, validates it, links projections, and retains deletion lifecycle', async () => {
    ...
    expect(new Set(first!.units.filter(unit => unit.lifecycle === 'active').map(unit => unit.kind)).size).toBe(KEYSTONE_OKF_PROFILE.knowledgeKinds.length);
    expect([...new Set(first!.units.filter(unit => unit.lifecycle === 'active').map(unit => unit.kind))].sort()).toEqual([...KEYSTONE_OKF_PROFILE.knowledgeKinds].sort());
    expect([...new Set(first!.relationships.filter(edge => edge.lifecycle === 'active').map(edge => edge.kind))].sort()).toEqual([...KEYSTONE_OKF_PROFILE.relationshipKinds].sort());
    expect(first!.evidence.every(item => item.source.workspaceRelativePath && item.extractor && item.extractionRunId)).toBe(true);
    ...
    const graph = JSON.parse(fs.readFileSync(path.join(okfRoot, 'projections', 'graph.json'), 'utf8')) as { nodes: Array<{ id:string; okfId:string }>; edges: Array<{ id:string; okfId:string }> };
    expect(graph.nodes.every(node => node.id === node.okfId)).toBe(true);
    expect(graph.edges.every(edge => edge.id === edge.okfId)).toBe(true);
    const search = fs.readFileSync(path.join(okfRoot, 'projections', 'search.jsonl'), 'utf8').trim().split(/\r?\n/).map(line => JSON.parse(line) as { okfId:string; evidenceIds:string[] });
    expect(search.length).toBeGreaterThan(0);
    expect(search.every(item => item.okfId && item.evidenceIds.length > 0)).toBe(true);
    const bindings = fs.readFileSync(path.join(okfRoot, 'projections', 'cpg-bindings.jsonl'), 'utf8').trim().split(/\r?\n/).map(line => JSON.parse(line) as { okfId:string; path:string });
    ...
    expect(cpg!.edges.some(edge => Boolean(edge.okfSourceId) || Boolean(edge.okfTargetId))).toBe(true);
    ...
    expect(portable.concepts).toBe(first!.units.filter(unit => unit.lifecycle === 'active').length);
    const portableManifest = JSON.parse(fs.readFileSync(path.join(portableRoot, '.keystone-bundle.json'), 'utf8')) as { format: string; version: string; concepts: number; extractionRunId: string };
    expect(portableManifest).toMatchObject({ format: 'OKF', version: '0.2', concepts: portable.concepts, extractionRunId: first!.manifest.extractionRunId });
    const rootIndex = fs.readFileSync(path.join(portableRoot, 'index.md'), 'utf8');
    expect(rootIndex.startsWith('---\nokf_version: \"0.2\"\n---')).toBe(true);
    const conceptFiles = walk(portableRoot).filter(file => file.endsWith('.md') && !file.endsWith('index.md') && !file.endsWith('log.md'));
    expect(conceptFiles.length).toBe(portable.concepts);
    expect(conceptFiles.every(file => {
      const text = fs.readFileSync(file, 'utf8');
      return text.startsWith('---\ntype: ') && text.includes('\ngenerated:\n  by: \"keystone/1.0.0\"') && text.includes('\nverified:\n  - by: \"process:keystone-okf-validator\"') && text.includes('\nstatus: stable');
    })).toBe(true);
    expect(conceptFiles.some(file => {
      const text = fs.readFileSync(file, 'utf8');
      return text.includes('\nsources:\n') && /\[\^source-[^\]]+\]:/m.test(text);
    })).toBe(true);

    const readmeId = first!.units.find(unit => unit.kind === 'documentation' && unit.properties.path === 'README.md')!.id;
...
function walk(root: string): string[] { return fs.readdirSync(root, { withFileTypes: true }).flatMap(entry => entry.isDirectory() ? walk(path.join(root, entry.name)) : [path.join(root, entry.name)]); }
