...
describe('CpgShardStore', () => {
  it('writes compressed shards atomically, reuses unchanged content, and removes stale shards', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'keystone-cpg-shards-')); roots.push(root);
    ...
    expect(await first.finalize()).toEqual(expect.objectContaining({ written: 2, reused: 0, deleted: 0 }));
    ...
    expect(fs.readdirSync(path.join(root, '.keystone', 'intelligence', 'cpg')).filter(file => file.endsWith('.tmp'))).toEqual([]);
  });
});
