...
describe("buildTypeScriptCpg", () => {
  it("builds stable AST and evaluation-order layers with honest capabilities", () => {
    ...
    expect(
      first.edges.some((edge) => edge.kind === "dfg" && edge.metadata.variable === "result")
    ).toBe(true);
    expect(first.nodes.every((node) => node.location.startLine >= 1)).toBe(true);
  });

  it("adds control dependence for conditional branches", () => {
    const graph = buildTypeScriptCpg({
      sourcePath: "src/branch.ts",
      content: "function choose(ok: boolean) { if (ok) { return 1; } else { return 2; } }"
    });
    const branches = graph.edges
      .filter((edge) => edge.kind === "cdg")
      .map((edge) => edge.metadata.branch);
  ...
  it("selects JavaScript language and JSX parsing from the extension", () => {
    const graph = buildTypeScriptCpg({
      sourcePath: "src/view.jsx",
      content: "const View = () => <div />;"
    });
    expect(graph.language).toBe("javascript");
    expect(
      graph.nodes.some(
        (node) => node.syntaxKind === "JsxElement" || node.syntaxKind === "JsxSelfClosingElement"
      )
    ).toBe(true);
  });
});
