...
function completeReadyStory(engine: SDLCEngine, plan: SDLCPlan, type: SDLCStory['type']): SDLCPlan {
  ...
  if (['development', 'new-test-creation'].includes(type)) {
    plan = engine.prepareDelegation(plan, current.id, { agent: 'GitHub Copilot', skills: ['implementation'], instructions: ['Follow the approved specification'], prompt: `Complete ${type}`, contextPackId: `context-${type}` });
    plan = engine.approveDelegation(plan, current.id);
    plan = engine.completeDelegation(plan, current.id, [`Copilot delegation for ${type} returned for review.`]);
  } else {
    plan = engine.transition(plan, current.id, 'awaiting-validation');
  }
  current = story(plan, type);
  if (!['research', 'design', 'documentation', 'completion'].includes(type)) {
    plan = engine.recordValidation(plan, current.id, { status: 'passed', commands: [`verify:${type}`], evidence: [`Passing validation evidence for ${type}.`] });
  }
  current = story(plan, type);
  return engine.transition(plan, current.id, 'completed', { evidence: [`Completion evidence for ${type}.`], satisfiedCriteria: current.acceptanceCriteria });
}
...
describe('SDLCEngine', () => {
  ...
  it('creates presentable R&D documentation and small user/quality stories before execution', () => {
    ...
    let plan = engine.createPlan('Add audit history to order updates.', {
      relevantFiles: ['src/orders/service.ts'], relevantSymbols: ['updateOrder'], relatedTests: ['tests/orders.test.ts'], missingTests: ['audit regression test'],
      qaChecklist: ['Audit entries are persisted and tested.'], securityRisk: 'medium', performanceRisk: 'low', architecture: 'layered service',
    ...
    expect(plan.backlogStories.filter(item => item.kind === 'user-story').length).toBeGreaterThan(0);
    expect(plan.backlogStories.filter(item => item.kind === 'quality-story').length).toBeGreaterThan(0);
  ...
  it('executes the complete 16-story intent-led SDLC with approvals, delegation, validation, findings, and read-only PR review', () => {
    ...
    const order: SDLCStory['type'][] = [
      'design', 'development', 'existing-test-analysis', 'test-impact-analysis', 'new-test-creation',
      'failed-test-investigation', 'flaky-test-analysis', 'security-review', 'performance-review', 'modernization-review',
    ...
    for (const type of order) {
      ...
      if (type === 'security-review') {
        ...
        plan = engine.recordFinding(plan, ready.id, { kind: 'security', severity: 'high', summary: 'Authorization edge requires review.', status: 'open', evidence: ['okf:evidence:security'] });
        ...
        plan = engine.recordValidation(plan, ready.id, { status: 'passed', commands: ['security-check'], evidence: ['Security check passed.'] });
        const updated = story(plan, type);
        plan = engine.transition(plan, ready.id, 'completed', { evidence: ['Security review complete.'], satisfiedCriteria: updated.acceptanceCriteria });
      } else {
    ...
    expect(story(plan, 'pr-review').evidence.join(' ')).toMatch(/read-only|Completion evidence|validation/i);
  });
  ...
  it('blocks completion when criteria, validation, blockers, or severe findings are unresolved', () => {
    ...
    expect(() => engine.transition(plan, research.id, 'completed', { evidence: ['Only evidence'] })).toThrow();
  });
});
