...
function response(body: unknown, status = 200, setCookie?: string): FetchLikeResponse {
  return { ok: status >= 200 && status < 300, status, statusText: status === 200 ? 'OK' : 'Error', headers: { get: name => name.toLowerCase() === 'set-cookie' ? setCookie ?? null : null, getSetCookie: () => setCookie ? [setCookie] : [] }, json: async () => body, text: async () => JSON.stringify(body) };
}
...
describe('ValueEdgeClient', () => {
  it('imports a feature and publishes approved draft user and quality stories without persisting a secret', async () => {
    ...
    const client = new ValueEdgeClient({ baseUrl:'https://valueedge.example', sharedSpaceId:'1', workspaceId:'2', clientId:'client' }, 'secret', async (input, init) => {
      ...
      if (url.includes('/features/42')) return response({data:[{id:'42',name:'Audit feature',description:'Add auditing'}]});
    ...
    const result=await client.publishBacklogStories('42',[
      {id:'u',kind:'user-story',title:'User',description:'Do it',acceptanceCriteria:['Works'],linkedSdlcStoryTypes:['development'],status:'approved'},
      {id:'q',kind:'quality-story',title:'Quality',description:'Test it',acceptanceCriteria:['Passes'],linkedSdlcStoryTypes:['new-test-creation'],status:'approved'},
      {id:'d',kind:'user-story',title:'Draft',description:'No',acceptanceCriteria:[],linkedSdlcStoryTypes:[],status:'draft'},
    ]);
    ...
    expect(calls.filter(call=>call.url.includes('/workspaces/')).some(call=>String(call.init?.body).includes('secret'))).toBe(false);
  });
});
