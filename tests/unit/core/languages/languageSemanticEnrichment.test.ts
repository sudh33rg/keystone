      ...
      symbols: [
        {
          ...
          evidence: {
            ...
            confidence: 0.99,
            evidencePath: request.relativePath,
            evidenceLine: 1,
            extractorVersion: "test-language-service"
          }
        }
      ],
      calls: [
...
describe("language semantic enrichment", () => {
  it("merges installed language-service semantics without replacing deterministic evidence", async () => {
    ...
    expect(
      result.symbols.some(
        (symbol) => symbol.name === "SemanticRun" && symbol.evidence?.source === "language-service"
      )
    ).toBe(true);
  ...
  it("indexes unknown text extensions through the universal frontend", async () => {
    ...
    await fs.writeFile(
      path.join(root, "workflow.customlang"),
      "function execute() {\n  result = source\n}\n",
      "utf8"
    );
    ...
    expect(result.files.find((file) => file.path === "workflow.customlang")?.language).toBe(
      "unknown"
    );
    expect(
      result.symbols.some(
        (symbol) => symbol.filePath === "workflow.customlang" && symbol.name === "execute"
      )
    ).toBe(true);
    expect(result.languageSupport?.find((item) => item.id === "unknown")?.baseline).toBe(
      "universal-text"
    );
  });
  it("reuses unchanged semantic intelligence and enriches only changed files incrementally", async () => {
    ...
    const countingProvider: SemanticEnrichmentProvider = {
      async enrich(request) {
        calls += 1;
        return {
          provider: "counting-language-service",
          providerLanguageId: request.languageId,
          capabilities: {
            documentSymbols: true,
            definitions: true,
          ...
          symbols: [
            {
              ...
              evidence: {
                ...
                evidencePath: request.relativePath,
                evidenceLine: 1,
                extractorVersion: "counting-language-service"
              }
            }
          ],
          calls: [],
    ...
    const first = await indexRepository(root, {
      persist: true,
      semanticEnricher: countingProvider
    });
    expect(calls).toBe(1);
    ...
    await fs.writeFile(source, "def run():\n    value = helper()\n    return value\n", "utf8");
    const third = await indexRepository(root, {
      persist: true,
      semanticEnricher: countingProvider
    });
    expect(calls).toBe(2);
