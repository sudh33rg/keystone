...
} from "@core/intelligence/languages/languageRegistry";
...
const samples: Record<string, { path: string; content: string }> = {
  ...
  php: {
    path: "src/sample.php",
    content:
      "<?php require 'helper.php'; class Child extends Base implements Contract { public function run(){ $value = helper(); } }"
  },
  ruby: {
    path: "src/sample.rb",
  ...
  graphql: {
    path: "schema/sample.graphql",
    content: "type Query { sample: Sample }\ntype Sample { id: ID! }"
  },
  protobuf: {
    path: "schema/sample.proto",
  ...
  terraform: {
    path: "infra/main.tf",
    content: 'resource "aws_s3_bucket" "sample" {\n  bucket = var.name\n}'
  },
  ...
  it("indexes every registered category plus an unknown text language through OKF and CPG end to end", async () => {
    ...
    const activeFilePaths = new Set(
      okf!.units
        .filter(
          (unit) =>
            ["file", "test", "documentation", "configuration"].includes(unit.kind) &&
            unit.lifecycle === "active"
        )
        .map((unit) => String(unit.properties.path))
    );
