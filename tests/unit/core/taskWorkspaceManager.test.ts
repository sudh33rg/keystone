...
const seed = {
  intent: "Add audit telemetry",
  intentType: "feature",
  route: "hybrid",
  relevantFiles: ["src/a.ts"],
  relevantSymbols: ["run"],
  tests: ["src/a.test.ts"],
  qaChecks: ["Run unit tests"],
  securityRisk: "high",
  performanceRisk: "medium",
  modernizationNotes: ["Preserve behavior"],
  copilotPrompt: "Implement with context"
};
...
describe("TaskWorkspaceManager", () => {
  it("creates incrementing task folders, updates progress, and archives before deletion", async () => {
    ...
    await expect(
      fs.access(path.join(first.absolutePath, "specification.md"))
    ).resolves.toBeUndefined();
    const updated = await manager.update(first, "approved", { percent: 30, current: "Delegated" });
    expect(
      JSON.parse(await fs.readFile(path.join(first.absolutePath, "progress.json"), "utf8")).percent
    ).toBe(30);
    await manager.complete(updated);
    await expect(fs.access(first.absolutePath)).rejects.toMatchObject({ code: "ENOENT" });
    expect(
      await fs.readFile(path.join(root, ".keystone", "tasks", "completed.jsonl"), "utf8")
    ).toContain(first.id);
    await manager.complete(second);
    expect((await manager.create({ ...seed, intent: "Third task" })).name).toMatch(
      /^0003_third-task/
    );
  });
  ...
  it("copies task artifacts for handoff and reconstructs received packages", async () => {
    ...
    const imported = await new TaskWorkspaceManager(target).importHandoffPackage({
      taskId: "shared-task",
      packageId: "pkg",
      task: { intent: seed.intent },
      specification: {},
      plan: {},
      progress: {},
      context: {},
      continuation: { suggestedFirstPrompt: seed.copilotPrompt }
    });
  ...
  it("recovers the latest task, binds delegation, preserves status metadata, and archives cancellation truthfully", async () => {
    ...
    expect(
      JSON.parse(await fs.readFile(path.join(first.absolutePath, "status.json"), "utf8")).createdAt
    ).toBeTruthy();
  ...
  it("materializes an accepted modernization plan into the same numbered lifecycle", async () => {
    ...
    const ref = await manager.createModernization({
      ...
      targetArchitecture: {
        id: "target-1",
        name: "Modern Modular Platform",
        style: "modular-monolith",
        principles: ["Preserve contracts"],
        technologyPreferences: ["Node.js"]
      },
      capabilities: [],
      gaps: [],
      risks: [],
      ...
      decision: {
        ...
        source: "keystone-recommendation",
        targetArchitecture: {
          id: "target-1",
          name: "Modern Modular Platform",
          style: "modular-monolith",
        ...
        technologies: { runtime: "Node.js 24" },
        notes: []
      },
      phases: [
        {
          id: "phase-1",
          name: "Baseline and upgrade",
          strategy: "incremental-upgrade",
          order: 1,
          goals: ["Upgrade runtime"],
          scope: ["src"],
          prerequisites: [],
          risks: [],
          transformations: [],
      ...
      specifications: [
        {
          ...
          scope: ["src"],
          technologyDecisions: ["Node.js 24"],
          functionalRequirements: ["Preserve API behavior"],
          nonFunctionalRequirements: ["No latency regression"],
    ...
    expect(
      JSON.parse(await fs.readFile(path.join(ref.absolutePath, "progress.json"), "utf8"))
    ).toMatchObject({ status: "approved", activePhase: "phase-1" });
    expect(await fs.readFile(path.join(ref.absolutePath, "specification.md"), "utf8")).toContain(
      "Node.js 24"
    );
    expect(
      JSON.parse(await fs.readFile(path.join(ref.absolutePath, "agents.json"), "utf8")).map(
        (agent: { id: string }) => agent.id
      )
    ).toEqual(expect.arrayContaining(["security", "performance", "qa"]));
  });
  ...
  it("round-trips an encrypted handoff into an independent workspace without sharing repository state", async () => {
    const sourceRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), "keystone-handoff-source-instance-")
    );
    const targetRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), "keystone-handoff-target-instance-")
    );
    ...
    const encrypted = await encryptHandoffPackage(
      ...
      "independent workspace passphrase"
    );
    expect(encrypted).not.toContain(packageValue.task.originalUserRequest);
    const decrypted = JSON.parse(
      await decryptHandoffPackage(encrypted, "independent workspace passphrase")
    );
    ...
    expect(await targetManager.delegationPrompt(restored)).toBe(
      packageValue.continuation.suggestedFirstPrompt
    );
    expect(
      JSON.parse(await fs.readFile(path.join(restored.absolutePath, "context.json"), "utf8"))
    ).toMatchObject({ repositoryReference: packageValue.repositoryReference });
    expect(
      await fs.readFile(path.join(restored.absolutePath, "instructions.md"), "utf8")
    ).toContain(packageValue.continuation.manualRepositorySyncReminder);
  });
  ...
  it("materializes a verified handoff as an idempotent active task workspace", async () => {
    ...
    expect(await fs.readFile(path.join(restored.absolutePath, "delegation.md"), "utf8")).toContain(
      "Continue from verified state"
    );
    expect(
      JSON.parse(await fs.readFile(path.join(restored.absolutePath, "task.json"), "utf8"))
    ).toMatchObject({ kind: "restored-handoff", sourcePackageId: packageValue.packageId });
  });
});
...
const handoffInput = {
  ...
  taskId: "shared-task",
  createdBy: "dev",
  repositoryReference: { repositoryName: "repo", expectedBranch: "feature" },
  task: {
    originalUserRequest: "Continue shared task",
    normalizedProblemStatement: "Continue shared task",
    businessGoal: "Preserve continuity",
    technicalGoal: "Finish implementation",
    scope: ["src/a.ts"],
    nonGoals: [],
    constraints: [],
    assumptions: [],
  ...
  specification: {
    approvedBehavior: ["Preserve behavior"],
  ...
  plan: {
    ...
    currentTask: "Finish implementation",
    completedTasks: ["Analyze"],
    pendingTasks: ["Implement"],
    blockedTasks: [],
    deferredTasks: []
  },
  progress: {
    progressPercentage: 40,
    completedWorkSummary: ["Analyzed"],
  ...
  context: {
    architectureSummary: "modular",
    relevantModules: [],
    relevantFiles: ["src/a.ts"],
    relevantSymbols: ["run"],
    dependencyRelationships: [],
    impactedComponents: [],
    compressedTaskContext: "context",
  ...
  quality: {
    ...
    knownRegressions: [],
    qualityChecksStillRequired: ["Run test"]
  },
  decisions: {
    acceptedDecisions: [],
  ...
  continuation: {
    exactNextRecommendedAction: "Implement",
    suggestedFirstPrompt: "Continue from verified state",
    expectedFilesToInspect: ["src/a.ts"],
    expectedTestsToRun: ["test"],
    environmentRequirements: [],
    setupReminders: [],
    restoreWarnings: [],
    manualRepositorySyncReminder: "Synchronize manually",
    definitionOfCompletion: ["Tests pass"]
  }
};
