...
function fixture(): string {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ scripts: { build: "tsc", test: "vitest" }, dependencies: { react: "^19" } })
  );
  fs.writeFileSync(
...
describe("repository intelligence pipeline", () => {
  it("executes all stages in canonical order and summarizes active families", async () => {
    const root = fixture();
    const events: string[] = [];
    const snapshot = await buildRepositoryIntelligence(root, {
      cognitive: true,
    ...
    expect(
      snapshot.stages.filter((stage) => stage.cognitivelyEnriched).map((stage) => stage.id)
    ).toEqual(["architecture", "impact", "context", "sdlc-workflow", "risk", "documentation"]);
    ...
    expect(fs.readdirSync(path.join(root, ".keystone", "intelligence", "stages"))).toHaveLength(
      INTELLIGENCE_STAGES.length
    );
    ...
    expect(
      snapshot.health.checks.find((check) => check.id === "fallback-semantic-diagnostics")
    ).toEqual(expect.objectContaining({ passed: true }));
    expect(snapshot.health.checks.find((check) => check.id === "runtime-evidence")).toEqual(
      expect.objectContaining({ passed: false })
    );
    expect(snapshot.incremental.action).toBe("full");
    const apiDependency = snapshot.intelligence.dependencies.find(
      (edge) =>
        edge.from === "src/api.ts" && edge.to === "src/repository.ts" && edge.kind === "local"
    );
    expect(apiDependency).toEqual(
      expect.objectContaining({ from: "src/api.ts", to: "src/repository.ts", kind: "local" })
    );
    expect(apiDependency?.evidence).toEqual(
      expect.objectContaining({
        source: "heuristic",
    ...
    expect(snapshot.intelligence.tests).toContainEqual(
      expect.objectContaining({
  ...
  it("updates canonical cognitive knowledge without accumulating stale run entities", async () => {
    ...
    const units = fs
      ...
      .split(/\r?\n/)
      .filter(Boolean)
      .map((line) => JSON.parse(line) as { id: string; canonicalKey: string; lifecycle: string });
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(root, ".keystone", "intelligence", "okf", "manifest.json"), "utf8")
    ) as { parentExtractionRunId?: string; validation: { valid: boolean } };
  ...
  it("honors cancellation before ordered stage execution", async () => {
    ...
    controller.abort();
    await expect(
      buildRepositoryIntelligence(root, { signal: controller.signal, cognitive: false })
    ).rejects.toBeInstanceOf(IntelligencePipelineCancelledError);
    expect(fs.existsSync(path.join(root, ".keystone", "intelligence", "snapshot.json"))).toBe(
      false
    );
  });
});
