...
import { OperationContextFactory, RepositoryStateService, StalenessService, StartupStateService } from "../../../src/core/integration/ProductIntegrationService";
...
describe("product integration contracts", () => {
  it("creates stable repository identity and explains each changed boundary", () => {
    ...
    const before = service.fromIntelligence(snapshot, { stagedPaths: ["src/a.ts"], workingTreePaths: ["src/b.ts"] });
    const after = service.fromIntelligence({ ...snapshot, manifest: { ...snapshot.manifest, generation: 8 }, repository: { ...snapshot.repository, branch: "feature", headCommit: "b".repeat(40) } }, { stagedPaths: ["src/c.ts"], workingTreePaths: ["src/d.ts"] });
    expect(after.rootPathIdentity).toBe(before.rootPathIdentity);
    expect(new StalenessService().compare(before, after, "workflow", crypto.randomUUID()).map((record) => record.reason)).toEqual([
      "branch-changed", "head-changed", "intelligence-generation-changed", "relevant-files-changed", "delivery-change-set-changed",
    ]);
  });
  ...
  it("reports staged startup and preserves degraded diagnostics", () => {
    ...
    expect(startup.transition("persistence-restoring", "running", "Restoring local state.").stage).toBe("persistence-restoring");
    const degraded = startup.diagnose({ category: "persistence", message: "One optional projection was unavailable.", dataPreserved: true, retrySafe: true, suggestedRecovery: "Retry projection generation." });
  ...
  it("migrates the legacy repository artifact path once", async () => {
    const root = await mkdtemp(join(tmpdir(), "keystone-team-migration-")); roots.push(root);
    const seed = new TeamWorkflowPersistenceStore(root); await seed.initialize();
    const path = join(root, "workflow", "team-state.json");
    const value = JSON.parse(await readFile(path, "utf8")) as { settings: { repositoryArtifactPath: string } };
