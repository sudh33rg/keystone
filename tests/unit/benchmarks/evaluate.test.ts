...
function makeSymbol(
  overrides: Partial<IntelligenceSnapshot["symbols"][0]> = {},
): IntelligenceSnapshot["symbols"][0] {
  return {
    id: "sym-1",
    name: "Foo",
...
function makeGroundTruth(
  overrides: Partial<IntelligenceGroundTruth> = {},
): IntelligenceGroundTruth {
  return {
    repositoryId: "test",
    entities: [],
    relationships: [],
...
describe("evaluateEntities", () => {
  it("matches entities by qualifiedName", () => {
    ...
    const snapshot = makeSnapshot({
      symbols: [makeSymbol({ qualifiedName: "pkg/src/Foo.ts:Foo" })],
    });
  ...
  it("matches entities by name+filePath fallback", () => {
    ...
    const snapshot = makeSnapshot({
      symbols: [makeSymbol({ name: "Foo", filePath: "src/Foo.ts", qualifiedName: "other:Foo" })],
    });
    const result = evaluateEntities(expected, snapshot);
    expect(result.matched.size).toBe(1);
  });
  ...
  it("correctly reports false positive entities", () => {
    const expected: ExpectedEntity[] = [];
    const snapshot = makeSnapshot({
      symbols: [makeSymbol({ qualifiedName: "pkg/Extra.ts:Extra" })],
    });
    const result = evaluateEntities(expected, snapshot);
...
describe("evaluateCapabilityMetrics", () => {
  ...
  it("computes call precision/recall correctly", () => {
    const snapshot = makeSnapshot({
      relationships: [makeRelationship({ type: "keystone.core.CALLS" })],
    });
    const metrics = evaluateCapabilityMetrics(snapshot);
  ...
  it("computes covers precision/recall correctly", () => {
    const snapshot = makeSnapshot({
      relationships: [makeRelationship({ type: "keystone.core.COVERS" })],
    });
    const metrics = evaluateCapabilityMetrics(snapshot);
...
describe("validateFalsePositives", () => {
  ...
  it("does not mistake an honestly classified candidate for an exact relationship", () => {
    const snapshot = makeSnapshot({
      relationships: [makeRelationship({ resolution: "candidate" })],
    });
    const report = validateFalsePositives(snapshot, makeGroundTruth());
  ...
  it("detects a candidate relationship carrying exact confidence", () => {
    const snapshot = makeSnapshot({
      relationships: [makeRelationship({ resolution: "candidate", confidence: "exact" })],
    });
    const report = validateFalsePositives(snapshot, makeGroundTruth());
...
describe("validateThresholds", () => {
  ...
  it("fails when entity precision is below threshold", () => {
    const report = passingReport();
    report.entityMetrics.precision = 0.8;
    expect(validateThresholds(report, DEFAULT_THRESHOLDS)).toBe(false);
  });

  it("fails when false positive exact calls exceed threshold", () => {
    const report = passingReport();
    report.falsePositives.fabricatedExactCalls = ["bad-call"];
    expect(validateThresholds(report, DEFAULT_THRESHOLDS)).toBe(false);
  });

  it("fails when a candidate is promoted to exact confidence", () => {
    const report = passingReport();
    report.falsePositives.candidateMarkedExact = ["bad-candidate"];
    expect(validateThresholds(report, DEFAULT_THRESHOLDS)).toBe(false);
  });
});

function passingReport(): IntelligenceEvaluationReport {
  const metrics = {
    truePositives: 1,
    falsePositives: 0,
    falseNegatives: 0,
  ...
  return {
    ...
    relationshipMetrics: {},
    capabilityMetrics: { imports: { ...metrics }, calls: { ...metrics }, covers: { ...metrics } },
