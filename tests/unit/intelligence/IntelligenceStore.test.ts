...
describe("IntelligenceStore", () => {
  ...
  it("atomically persists and reloads the complete snapshot", async () => {
    ...
    expect(JSON.parse(await readFile(join(root, "intelligence", "current.json"), "utf8"))).toMatchObject({ generation: 1, directory: "000001" });
    expect(JSON.parse(await readFile(join(root, "intelligence", "generations", "000001", "manifest.json"), "utf8"))).toMatchObject({ generation: 1 });
  });
  ...
  it("retains the last complete snapshot when publication becomes stale", async () => {
    ...
    await expect(store.save(intelligenceSnapshot(2), () => { throw new Error("stale scan"); })).rejects.toMatchObject({ code: "INTELLIGENCE_ATOMIC_WRITE_FAILED" });
  ...
  it("serializes overlapping immutable generation publications", async () => {
    const root = await mkdtemp(join(tmpdir(), "keystone-intelligence-")); directories.push(root); const store = new IntelligenceStore(root);
    ...
    expect((await readdir(join(root, "intelligence", "generations"))).sort()).toEqual(["000001", "000002"]);
    store.dispose();
  });
  ...
  it("retains immutable generations and recovers from a corrupt current pointer", async () => {
    ...
    expect(JSON.parse(await readFile(join(root, "intelligence", "current.json"), "utf8"))).toMatchObject({ generation: 2, directory: "000002" });
    recovered.dispose();
  });
  ...
  it("keeps the last generation readable and reports manual intelligence deletion", async () => {
    ...
    await Promise.race([
      deleted,
      new Promise<never>((_resolve, reject) => setTimeout(() => reject(new Error("Intelligence deletion was not observed.")), 2_000))
    ]);
  ...
  it("detects corruption of an active compressed shard", async () => {
    ...
    await store.initialize();
    await store.save(intelligenceSnapshot(1));
    await writeFile(join(root, "intelligence", "generations", "000001", "evidence.json.gz"), "corrupt", "utf8");
  ...
  it("cleans interrupted pending generations and hard-links unchanged shards", async () => {
    ...
    await store.save(intelligenceSnapshot(1));
    await store.save(intelligenceSnapshot(2));
    const first = await stat(join(root, "intelligence", "generations", "000001", "diagnostics.json.gz"));
    const second = await stat(join(root, "intelligence", "generations", "000002", "diagnostics.json.gz"));
    ...
    expect(await readdir(join(root, "intelligence", "generations"))).not.toContain("000003.pending");
    restored.dispose();
