...
describe("semantic IntelligenceQueryService", () => {
  it("returns real semantic overview counts and paginated search", async () => {
    ...
    expect(overview.relationshipTypes.some((item) => item.key === "keystone.core.CALLS")).toBe(
      true,
    );
    ...
    expect(
      (await query.search({ query: "/users", entityTypes: ["keystone.core.Route"], limit: 10 }))
        .items[0]?.name,
    ).toBe("/users");
    expect(
      (
        await query.search({
          query: "target",
          packageIds: ["entity:package-membership"],
          moduleIds: ["entity:module"],
          limit: 10,
        })
      ).items,
    ).toHaveLength(2);
    expect(
      (await query.search({ query: "target", packageIds: ["entity:other"], limit: 10 })).items,
    ).toHaveLength(0);
  });
  ...
  it("returns bounded entity evidence and neighborhoods", async () => {
    ...
    expect(details).toMatchObject({
      entity: { qualifiedName: "caller" },
      outgoing: [{ type: "keystone.core.CALLS", entityName: "target" }],
    });
    expect(details?.evidence[0]?.subjectId).toBe("entity:caller");
    const neighborhood = await query.neighborhood({
      ids: ["entity:caller"],
      direction: "outgoing",
      maxDepth: 2,
      maxNodes: 2,
...
function semanticSnapshot(): IntelligenceSnapshot {
  ...
  snapshot.diagnostics.push(
    ...
    },
    {
      ...
      severity: "warning",
      message: "Fixture unresolved call.",
      ownerFileId: file.id,
      relativePath: file.relativePath,
    },
  );
