...
import {
  CpgScopeArtifactSchema,
  type CpgScopeArtifact,
} from "../../../../src/shared/contracts/cpg";
...
describe("progressive TypeScript CPG", () => {
  const coverageCases: Array<[string, (scopes: CpgScopeArtifact[]) => boolean]> = [
    ...
    [
      ...
      (scopes) =>
        kinds(find(scopes, "flow")).includes("PARAMETER") &&
        edgeTypes(find(scopes, "flow")).includes("DEFINES"),
    ],
    ...
    [
      ...
      (scopes) => edgeTypes(find(scopes, "compute")).includes("ARGUMENT_TO_PARAMETER"),
    ],
    [
      "return-value flow",
      (scopes) => edgeTypes(find(scopes, "compute")).includes("RETURN_TO_CALL"),
    ],
    [
      "nested calls",
      (scopes) => find(scopes, "compute").nodes.filter((node) => node.kind === "CALL").length >= 3,
    ],
    ...
    [
      ...
      (scopes) =>
        edgeTypes(find(scopes, "control")).includes("CFG_TRUE") &&
        edgeTypes(find(scopes, "control")).includes("CFG_FALSE"),
    ],
    ["switch control flow", (scopes) => edgeTypes(find(scopes, "control")).includes("CFG_CASE")],
    [
      "for loop",
      (scopes) => find(scopes, "control").nodes.filter((node) => node.kind === "LOOP").length >= 1,
    ],
    [
      "while loop",
      (scopes) => find(scopes, "control").nodes.filter((node) => node.kind === "LOOP").length >= 2,
    ],
    ...
    [
      "try/catch/finally",
      (scopes) =>
        ["TRY", "CATCH", "FINALLY"].every((kind) => kinds(find(scopes, "control")).includes(kind)),
    ],
    [
      "explicit throw",
      (scopes) =>
        kinds(find(scopes, "control")).includes("THROW_STATEMENT") &&
        edgeTypes(find(scopes, "control")).includes("CFG_EXCEPTION"),
    ],
    [
      "short-circuit boolean expression",
      (scopes) => find(scopes, "flow").nodes.some((node) => node.properties?.shortCircuit === true),
    ],
    [
      "conditional expression",
      (scopes) => kinds(find(scopes, "compute")).includes("CONDITIONAL_EXPRESSION"),
    ],
    [
      "optional chaining",
      (scopes) =>
        find(scopes, "compute").nodes.some((node) => node.properties?.optionalChain === true),
    ],
    [
      "destructuring",
      (scopes) =>
        find(scopes, "flow").nodes.some(
          (node) => node.kind === "VARIABLE_DECLARATION" && node.code?.startsWith("{ value:"),
        ),
    ],
    [
      "basic object-property flow",
      (scopes) =>
        find(scopes, "flow").edges.some(
          (edge) => edge.type === "FLOWS_TO" && edge.confidence === 0.7,
        ),
    ],
    [
      "unresolved dynamic call",
      (scopes) => kinds(find(scopes, "compute")).includes("UNRESOLVED_TARGET"),
    ],
    [
      "external call representation",
      (scopes) => kinds(find(scopes, "compute")).includes("EXTERNAL_CALL"),
    ],
    [
      "exact local call binding",
      (scopes) => edgeTypes(find(scopes, "compute")).includes("CALLS_SYMBOL"),
    ],
    [
      "evaluation-order overlay",
      (scopes) => edgeTypes(find(scopes, "compute")).includes("EVAL_NEXT"),
    ],
    [
      "scope entry and exit",
      (scopes) => ["ENTRY", "EXIT"].every((kind) => kinds(find(scopes, "compute")).includes(kind)),
    ],
    ["structural scope return", (scopes) => kinds(find(scopes, "compute")).includes("RETURN")],
    [
      "receiver-to-call flow",
      (scopes) => edgeTypes(find(scopes, "compute")).includes("RECEIVER_TO_CALL"),
    ],
    [
      "CFG return termination",
      (scopes) => edgeTypes(find(scopes, "compute")).includes("CFG_RETURN"),
    ],
    [
      "evidence-backed nodes",
      (scopes) => find(scopes, "compute").nodes.every((node) => node.evidenceIds.length > 0),
    ],
    [
      "evidence-backed edges",
      (scopes) => find(scopes, "compute").edges.every((edge) => edge.evidenceIds.length > 0),
    ],
    [
      "stable edge IDs",
      (scopes) =>
        new Set(find(scopes, "compute").edges.map((edge) => edge.id)).size ===
        find(scopes, "compute").edges.length,
    ],
    [
      "module executable scope",
      (scopes) => scopes.some((scope) => scope.descriptor.kind === "module"),
    ],
    [
      "approximation diagnostic",
      (scopes) =>
        find(scopes, "flow").diagnostics.some((item) => item.code === "approximate-data-flow"),
    ],
    [
      "exception-model diagnostic",
      (scopes) =>
        find(scopes, "control").diagnostics.some(
          (item) => item.code === "incomplete-exception-model",
        ),
    ],
  ];
  const shared = build();
  it.each(coverageCases)("covers %s", async (_name, verify) => {
    expect(verify(await shared)).toBe(true);
  });
  ...
  it("builds normalized ASTs for functions, methods, arrows, parameters, calls, and returns", async () => {
    ...
    expect(scopes.map((scope) => scope.descriptor.kind)).toEqual(
      expect.arrayContaining(["function", "method", "constructor", "arrow"]),
    );
    const compute = find(scopes, "compute");
    expect(kinds(compute)).toEqual(
    ...
    expect(edgeTypes(compute)).toEqual(
      expect.arrayContaining(["AST_CHILD", "AST_PARENT", "BELONGS_TO_SCOPE"]),
    );
    expect(() => CpgScopeArtifactSchema.parse(compute)).not.toThrow();
    expect(
      compute.nodes.every(
        (node) => node.evidenceIds.length > 0 && node.parserVersion && node.generation === 1,
      ),
    ).toBe(true);
    expect(
      compute.edges.every((edge) => edge.evidenceIds.length > 0 && edge.sourceId !== edge.targetId),
    ).toBe(true);
  });
  ...
  it("models deterministic evaluation order, nested and awaited calls, optional access, and conditional expressions", async () => {
    ...
    const evaluated = compute.nodes
      .filter((node) => node.evaluationIndex !== undefined)
    ...
    expect(kinds(compute)).toEqual(
      expect.arrayContaining(["AWAIT", "CONDITIONAL_EXPRESSION", "MEMBER_ACCESS", "CALL"]),
    );
  ...
  it("computes parameter, variable, reassignment, property, reaching-definition, and return flow", async () => {
    ...
    expect(edgeTypes(flow)).toEqual(
      expect.arrayContaining(["DEFINES", "USES", "REACHING_DEFINITION", "FLOWS_TO"]),
    );
  ...
  it("builds branches, switch, for/while, break/continue, and try/catch/finally exception flow", async () => {
    ...
    expect(edgeTypes(control)).toEqual(
      expect.arrayContaining([
        "CFG_NEXT",
        "CFG_TRUE",
        "CFG_FALSE",
    ...
    expect(control.diagnostics.some((item) => item.code === "incomplete-exception-model")).toBe(
      true,
    );
  });
  ...
  it("binds exact local arguments, parameters, receivers, and returns without inventing dynamic targets", async () => {
    ...
    expect(
      compute.diagnostics.some(
        (item) =>
          item.code === "unresolved-call" &&
          item.message.includes("no local target was fabricated"),
      ),
    ).toBe(true);
  ...
  it("produces bounded backward and forward slices with truncation and unsupported boundaries", async () => {
    ...
    const backward = slicer.slice(flow, {
      semanticSymbolId: flow.descriptor.semanticSymbolId,
      nodeId: read.id,
      direction: "backward",
    ...
    const forward = slicer.slice(flow, {
      semanticSymbolId: flow.descriptor.semanticSymbolId,
      nodeId: flow.nodes.find((node) => node.kind === "PARAMETER")!.id,
      direction: "forward",
      includeConditions: true,
      maxNodes: 2,
      maxDepth: 8,
  ...
  it("keeps stable IDs, reuses unchanged scopes, replaces a changed scope, and removes a deleted scope", async () => {
    ...
    expect(repeated.cpg?.scopes.map((item) => item.descriptor.id)).toEqual(
      first.cpg?.scopes.map((item) => item.descriptor.id),
    );
    ...
    const changed = {
      ...
      content: input.content.replace(
        ...
        "return total + state.value + 1;",
      ),
      contentHash: "sha256:changed",
    };
    const updated = await parser.extract(request(3, false, [changed]));
    expect(updated.cpg?.scopes.find((item) => item.descriptor.name.endsWith("flow"))?.reused).toBe(
      false,
    );
    expect(
      updated.cpg?.scopes.find((item) => item.descriptor.name.endsWith("control"))?.reused,
    ).toBe(true);
    const deleted = await parser.extract({
      ...request(4, false, []),
      removedPaths: [input.relativePath],
    });
...
async function build(): Promise<CpgScopeArtifact[]> {
  const result = await new TypeScriptJavaScriptParser().extract(request(1, true, fixture()));
  return result.cpg?.scopes ?? [];
}
function find(scopes: CpgScopeArtifact[], suffix: string): CpgScopeArtifact {
  const scope = scopes.find((item) => item.descriptor.name.endsWith(suffix));
  if (!scope)
    throw new Error(
      `Missing ${suffix} scope: ${scopes.map((item) => item.descriptor.name).join(", ")}`,
    );
  return scope;
}
function kinds(scope: CpgScopeArtifact): string[] {
  return scope.nodes.map((node) => node.kind);
}
function edgeTypes(scope: CpgScopeArtifact): string[] {
  return scope.edges.map((edge) => edge.type);
}

function fixture(): SemanticSourceFileInput[] {
  return [
    file(
      "src/cpg.ts",
      `
...
function file(relativePath: string, content: string): SemanticSourceFileInput {
  return {
