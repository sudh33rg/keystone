...
import type {
  GitReconciliationEvent,
  RepositoryMonitor,
} from "../../../../src/extension/intelligence/VsCodeRepositoryMonitor";
  ...
  it("coalesces file changes and schedules Git and storage repairs", async () => {
    ...
    const index = {
      ...
      onDidChange: (listener: () => void) => {
        ...
        return { dispose: () => indexListeners.delete(listener) };
      },
    ...
    const changed: RepositoryChange = {
      kind: "modified",
      rootUri: "file:///repo",
...
function createStore(): { value: object; emitDelete(): void } {
  let healthListener:
    ...
    | undefined;
  let health: { status: "healthy" | "damaged"; pendingGenerations: number } = {
    status: "healthy",
  ...
  return {
    value: {
      ...
      onDidHealthChange: (listener: typeof healthListener) => {
...
function createMonitor(): {
  value: RepositoryMonitor;
  emitChange(change: RepositoryChange): void;
  emitGit(event: GitReconciliationEvent): void;
} {
