...
describe("RepositoryIndexService", () => {
  ...
  it("publishes only observed containment and declaration relationships", async () => {
    ...
    const files: WorkspaceFileReference[] = paths.map((relativePath) => ({
      root,
      relativePath,
      uri: `${root.uri}/${relativePath}`,
    }));
    const readFile = vi.fn((uri: string) =>
      Promise.resolve(
        new TextEncoder().encode(
          uri.includes("test") ? "test('works', () => true)" : "export function run() {}",
        ),
      ),
    );
    const workspace: WorkspaceAdapter = {
      ...
      fileReference: (selectedRoot, relativePath) => ({
        root: selectedRoot,
        relativePath,
        uri: `${selectedRoot.uri}/${relativePath}`,
      }),
      statFile: () =>
        Promise.resolve({ byteSize: 24, modifiedAt: "2026-07-15T00:00:00.000Z", type: "file" }),
      readFile,
    ...
    const language: LanguageServiceAdapter = {
      extractSymbols: (uri) =>
        Promise.resolve({
          ...
          symbols: [
            {
              name: uri.includes("test") ? "works" : "run",
              qualifiedName: uri.includes("test") ? "works" : "run",
    ...
    expect(
      snapshot?.files.find((file) => file.relativePath === "tests/index.test.ts")?.classification,
    ).toMatchObject({ category: "test", included: true, generated: false });
    expect(
      snapshot?.files.find((file) => file.relativePath === "src/excluded.ts")?.classification,
    ).toMatchObject({ included: false, analysisLevel: "excluded", ruleId: "exclude.user" });
    expect(
      snapshot?.files.find((file) => file.relativePath === ".env")?.contentHash,
    ).toBeUndefined();
    expect(readFile).not.toHaveBeenCalledWith("file:///fixture/.env");
    expect(
      snapshot?.relationships.every((relationship) =>
        ["keystone.core.CONTAINS", "keystone.core.DECLARES"].includes(relationship.type),
      ),
    ).toBe(true);
    expect(
      snapshot?.relationships.every((relationship) => relationship.evidenceIds.length > 0),
    ).toBe(true);
    expect(
      snapshot?.relationships.filter(
        (relationship) => relationship.type === "keystone.core.DECLARES",
      ),
    ).toHaveLength(2);
    ...
    const rescanned = new Promise<void>((resolve) => {
      const subscription = service.onDidChange((state) => {
        if (!state.pendingUpdate && state.scanRevision === 2) {
          subscription.dispose();
          resolve();
        }
      });
    ...
    expect(store.getSnapshot()?.files.some((item) => item.relativePath === "src/excluded.ts")).toBe(
      false,
    );
    expect(
      store
  ...
  it("discards a superseded scan before atomic publication", async () => {
    ...
    const service = new RepositoryIndexService(
      ...
      new DefaultIgnorePolicy(),
      store,
      { info: vi.fn(), error: vi.fn() } as unknown as KeystoneLogger,
    );
    await service.initialize();
    service.start();
    await firstStarted;
  ...
  it("rejects a file result when its content hash changes before merge", async () => {
    ...
    const workspace = {
      ...
      readFile: () =>
        Promise.resolve(
          new TextEncoder().encode(++read === 1 ? "export const a = 1" : "export const b = 2"),
        ),
    ...
    const service = new RepositoryIndexService(
      workspace,
      { getMetadata: () => Promise.resolve({ branch: "main" }) } as never,
      language,
      new DefaultIgnorePolicy(),
      new IntelligenceStore(storageRoot),
      { info: vi.fn(), error: vi.fn() } as never,
    );
    await service.initialize();

    await expect(service.rebuild("file")).rejects.toMatchObject({
      code: "INTELLIGENCE_SOURCE_STALE",
    });
  });
});
