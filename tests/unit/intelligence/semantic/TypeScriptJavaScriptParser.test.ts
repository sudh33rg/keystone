...
describe("TypeScriptJavaScriptParser", () => {
  it("extracts evidence-backed compiler, framework, test, package, and configuration intelligence", async () => {
    ...
    expect(types(result.entities)).toEqual(
    ...
    expect(relationshipTypes(result.relationships)).toEqual(
      expect.arrayContaining([
    ...
    expect(
      result.relationships.some(
        (item) => item.type === "keystone.core.IMPORTS" && item.properties?.commonJs === true,
      ),
    ).toBe(true);
    expect(
      result.relationships.some(
        (item) => item.type === "keystone.core.IMPORTS" && item.properties?.dynamic === true,
      ),
    ).toBe(true);
    expect(
      result.relationships.some(
        (item) =>
          item.type === "keystone.core.IMPORTS" && item.properties?.moduleSpecifier === "./base",
      ),
    ).toBe(true);
    expect(
      result.relationships.some(
        (item) => item.type === "keystone.core.ALIASES" && item.properties?.alias === "base",
      ),
    ).toBe(true);
    expect(
      result.relationships.some(
    ...
    expect(
      result.relationships.some(
        (item) =>
          item.type === "keystone.core.REGISTERS_HANDLER" && item.properties?.registration === "on",
      ),
    ).toBe(true);
    expect(
      result.relationships.some(
        (item) =>
          item.type === "keystone.core.TESTS" &&
          item.confidence === 1 &&
          item.properties?.evidenceLevel === 1,
      ),
    ).toBe(true);
    expect(
      result.relationships.some(
    ...
    expect(
      result.entities.some(
        (item) => item.type === "keystone.core.ConfigurationKey" && item.name === "API_KEY",
      ),
    ).toBe(true);
    expect(
      result.entities
        .filter((item) => item.type === "keystone.core.Route")
        .every((item) => item.name.startsWith("/") || item.name === "*"),
    ).toBe(true);
    expect(JSON.stringify(result)).not.toContain("super-secret-value");
    expect(result.diagnostics.some((item) => item.code === "unresolved-call")).toBe(true);
    expect(
      result.relationships.every(
        (item) => item.evidenceIds.length > 0 && item.sourceId !== "" && item.targetId !== "",
      ),
    ).toBe(true);
    expect(
      result.evidence.every(
        (item) => item.ownerFileId && item.contentHash && item.extractorVersion,
      ),
    ).toBe(true);
    expect(result.contributions.every((item) => item.parserVersion && item.sourceHash)).toBe(true);
    expect(
      result.fileUpdates.some(
        (item) => item.id === "file:src/legacy-component.jsx" && item.parseStatus === "parsed",
      ),
    ).toBe(true);
    expect([...new Set(result.cpg?.scopes.map((item) => item.descriptor.fileId))]).toEqual(
      expect.arrayContaining([
        "file:src/base.ts",
        "file:src/consumer.ts",
        "file:src/components.tsx",
        "file:src/legacy.js",
        "file:src/legacy-component.jsx",
      ]),
    );
    ...
    const invokeCpg = result.cpg?.scopes.find(
      (item) =>
        item.descriptor.semanticSymbolId ===
          result.entities.find((entity) => entity.name === "invoke")?.id &&
        item.descriptor.kind === "function",
    );
    ...
    const invokeTargets = result.relationships
      .filter((item) => item.sourceId === invokeId && item.type === "keystone.core.CALLS")
      .flatMap((item) => {
        const target = result.entities.find((entity) => entity.id === item.targetId);
    ...
    expect(
      result.adapterState?.coverage.find((item) => item.technologyId === "typescript"),
    ).toMatchObject({ capabilityLevel: "semantic" });
    expect(
      result.adapterState?.coverage.find((item) => item.technologyId === "typescript")
        ?.entitiesExtracted,
    ).toBeGreaterThan(0);
  ...
  it("keeps identities deterministic and removes deleted or changed-signature contributions", async () => {
    ...
    expect(repeated.relationships.map((item) => item.id)).toEqual(
    ...
    const changed = {
      ...base,
      content: base.content.replace("helper(value: string)", "helper(value: number)"),
      contentHash: "sha256:base-changed",
    };
    const updated = await parser.extract(request(3, false, [changed]));
    const firstHelper = first.entities.find(
      (item) => item.name === "helper" && item.type === "keystone.core.Function",
    )!;
    const nextHelper = updated.entities.find(
      (item) => item.name === "helper" && item.type === "keystone.core.Function",
    )!;
  ...
  it("keeps universal adapter relationships and evidence valid in a mixed semantic result", async () => {
    const files = [
      file("src/index.ts", "typescript", "source", "export const initial = 1;"),
      file("docs/README.md", "markdown", "documentation", "# Fixture\n## Usage"),
      file("api/openapi.yaml", "yaml", "schema", "openapi: 3.0.0\npaths:\n  /fixture:\n    get:"),
      file(
        "db/schema.sql",
        "sql",
        "schema",
        "CREATE TABLE fixture_items (id INTEGER PRIMARY KEY, name TEXT);",
      ),
      file(
        "db/migrations/002.sql",
        "sql",
        "migration",
        "ALTER TABLE fixture_items ADD COLUMN active INTEGER;",
      ),
    ];
    ...
    expect(
      [...result.entities, ...result.relationships].flatMap((item) =>
        item.evidenceIds.filter((id) => evidence.get(id)?.subjectId !== item.id),
      ),
    ).toEqual([]);
    expect(result.adapterState?.coverage.map((item) => item.technologyId)).toEqual(
      expect.arrayContaining(["openapi", "sql", "sql-migration", "documentation"]),
    );
  });
  ...
  it("does not report external fluent APIs or configuration getters as routes", async () => {
    ...
    expect(
      result.diagnostics.filter(
        (item) => item.code === "unresolved-call" && /z\.|\.strict/.test(item.message),
      ),
    ).toEqual([]);
  });
});
...
`,
    ...
    file(
      ...
      "javascript",
      "source",
      `const base = require("./base"); module.exports = function legacy() { return base.helper("legacy"); };`,
    ),
    file(
      "src/legacy-component.jsx",
      "javascriptreact",
      "source",
      `import { Child } from "./components"; export function LegacyComponent() { return <Child label="legacy"/>; }`,
    ),
    file(
      "tests/base.test.ts",
      "typescript",
...
function file(
  relativePath: string,
  language: string,
  category: SemanticSourceFileInput["category"],
  content: string,
): SemanticSourceFileInput {
  return {
...
function types(items: Array<{ type: string }>): string[] {
  return items.map((item) => item.type);
}
function relationshipTypes(items: Array<{ type: string }>): string[] {
  return items.map((item) => item.type);
}
