...
describe("SemanticDeltaBuilder", () => {
  it("updates changed and dependent files while retaining unrelated contributions", () => {
    ...
    const unrelated = { ...original, id: "file:unrelated", relativePath: "src/unrelated.ts", contentHash: "sha256:unrelated", evidenceIds: ["evidence:unrelated"] };
    snapshot.files.push(unrelated);
    snapshot.evidence.push({ ...snapshot.evidence.find((item) => item.subjectId === original.id)!, id: "evidence:unrelated", subjectId: unrelated.id, relativePath: unrelated.relativePath, contentHash: unrelated.contentHash });
    snapshot.symbols.push(entity("entity:old", original.id, 1), entity("entity:unrelated", unrelated.id, 1));
    snapshot.contributions = [contribution(original.id, original.contentHash!, ["entity:old"], 1), contribution(unrelated.id, unrelated.contentHash, ["entity:unrelated"], 1)];
    const result: SemanticProjectResult = {
      ...
      sourceHashes: { [original.id]: "sha256:changed", [unrelated.id]: unrelated.contentHash }, fileUpdates: [],
      entities: [entity("entity:new", original.id, 2), entity("entity:unrelated", unrelated.id, 2)], relationships: [], evidence: [], diagnostics: [],
      contributions: [contribution(original.id, "sha256:changed", ["entity:new"], 2), contribution(unrelated.id, unrelated.contentHash, ["entity:unrelated"], 2)]
    };
...
function contribution(fileId: string, sourceHash: string, entityIds: string[], generation: number, parserVersion = "1") { return { fileId, sourceHash, parserId: "keystone.typescript", parserVersion, entityIds, relationshipIds: [], evidenceIds: [], diagnosticIds: [], dependencyFileIds: [], generation }; }
