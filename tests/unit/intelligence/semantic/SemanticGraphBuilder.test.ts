...
describe("SemanticGraphBuilder", () => {
  it("indexes JavaScript prototype-shaped names without corrupting the index", () => {
    const symbols = ["constructor", "__proto__", "toString"].map((name, index) => ({
      id: `entity:${index}`, repositoryId: "repository:fixture", fileId: "file:fixture", ownerFileId: "file:fixture",
      type: "keystone.core.Function", name, qualifiedName: name, language: "javascript",
      range: { startLine: index, startColumn: 0, endLine: index, endColumn: name.length },
      evidenceIds: [`evidence:${index}`], confidence: 1, generation: 1
    }));
    const snapshot: Pick<IntelligenceSnapshot, "files" | "symbols" | "relationships"> = { files: [], symbols, relationships: [] };
