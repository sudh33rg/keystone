...
interface TechnologyResult {
  generation: number;
  total: number;
  items: Array<{ technologyId: string; capabilityLevel: string }>;
}
interface UnifiedQueryResult {
  ...
  explanation: { rankingRules: string[] };
  truncated: boolean;
}
...
export async function run(): Promise<void> {
  ...
  assert.ok(
    extension,
    "Keystone extension should be discoverable in the Extension Development Host",
  );
  ...
  assert.ok(
    commands.includes("keystone.intelligence.search"),
    "Keystone semantic search command should be registered",
  );
  assert.ok(
    commands.includes("keystone.intelligence.cpg"),
    "Keystone CPG query command should be registered",
  );
  assert.ok(
    commands.includes("keystone.intelligence.technologies"),
    "Keystone technology coverage command should be registered",
  );
  assert.ok(
    commands.includes("keystone.intelligence.query"),
    "Keystone deterministic query command should be registered",
  );
  for (const command of [
    "keystone.startWorkflow",
    "keystone.resumeWorkflow",
    "keystone.openCurrentTask",
    "keystone.askRepository",
    "keystone.openEntity",
    "keystone.showUsages",
    "keystone.showFlow",
  ...
  await vscode.commands.executeCommand("keystone.open");
  await vscode.commands.executeCommand("keystone.open");
  const panelMetrics = await vscode.commands.executeCommand<{
    panelOpen: boolean;
  ...
  assert.ok(
    overview.runtime.workerCapacity >= 2,
    "Continuous ingestion should keep multiple persistent workers",
  );
  assert.ok(
    overview.categories.some((item) => item.key === "test" && item.count >= 1),
    "Test files should remain indexed as tests",
  );
  const search = await vscode.commands.executeCommand<SearchResult>(
  ...
  assert.ok(
    unifiedSearch?.data.items.some((item) => item.id === initialEntity.id),
    "Unified deterministic search should resolve the fixture entity",
  );
  assert.ok(
    (unifiedSearch?.evidence.length ?? 0) > 0 &&
      (unifiedSearch?.explanation.rankingRules.length ?? 0) > 0,
    "Unified queries should expose evidence and ranking explanation",
  );
  const functionSearch = await vscode.commands.executeCommand<SearchResult>(
    "keystone.intelligence.search",
  ...
  const cpg = await vscode.commands.executeCommand<{
    ...
    semanticSymbolId: analyze.id,
    overlays: ["control-flow", "data-flow", "calls"],
    maxNodes: 100,
    includeSource: true,
  });
  assert.ok(
    cpg?.nodes.some((node) => node.kind === "ENTRY") &&
      cpg.nodes.some((node) => node.kind === "EXIT"),
    "The persisted scope CPG should expose entry and exit nodes",
  );
  assert.ok(
    cpg?.edges.some((edge) => edge.type === "CFG_TRUE"),
    "The persisted scope CPG should expose the fixture branch",
  );
  const unifiedCpg = await vscode.commands.executeCommand<UnifiedQueryResult>(
    "keystone.intelligence.query",
    {
      query: {
        operation: "CPG_SCOPE",
        seeds: [{ id: analyze.id, kind: "stable-id" }],
        include: {
          source: true,
          evidence: true,
          relationships: true,
  ...
  assert.ok(
    (unifiedCpg?.data.cpg?.nodes.length ?? 0) > 0,
    "Unified CPG queries should reuse the persisted bounded CPG facade",
  );
  const technologies = await vscode.commands.executeCommand<TechnologyResult>(
  ...
  assert.ok(
    technologies?.items.some((item) => item.technologyId === "sql"),
    "Worker-produced SQL coverage should persist and be queryable",
  );
  ...
  await vscode.workspace.fs.writeFile(
    source,
    new TextEncoder().encode("export const modified = 2;\n"),
  );
  const modified = await waitForOverview(
    (value) => value.generation > initialGeneration && !value.pendingUpdate,
  );

  const createdTest = vscode.Uri.joinPath(root, "tests", "created.test.ts");
  await vscode.workspace.fs.writeFile(
    createdTest,
    new TextEncoder().encode("export const createdTest = true;\n"),
  );
  const created = await waitForOverview(
    (value) =>
      value.generation > modified.generation &&
  ...
  for (const value of [1, 2, 3]) {
    await vscode.workspace.fs.writeFile(
      source,
      new TextEncoder().encode(`export const burst = ${value};\n`),
    );
    await new Promise<void>((resolve) => setTimeout(resolve, 15));
  }
  const burst = await waitForOverview(
    (value) => value.generation > beforeBurst && !value.pendingUpdate,
  );
  assert.equal(
    burst.generation,
    beforeBurst + 1,
    "Repeated file changes should coalesce into one generation",
  );

  const renamedTest = vscode.Uri.joinPath(root, "tests", "renamed.test.ts");
  await vscode.workspace.fs.rename(createdTest, renamedTest);
  const renamed = await waitForOverview(
    (value) =>
      value.generation > burst.generation &&
      value.counts.files === burst.counts.files &&
      !value.pendingUpdate,
  );
  await vscode.workspace.fs.delete(renamedTest);
  const deleted = await waitForOverview(
    (value) =>
      value.generation > renamed.generation &&
      value.counts.files < renamed.counts.files &&
      !value.pendingUpdate,
  );
  ...
  await vscode.workspace.fs.writeFile(
    source,
    new TextEncoder().encode("export const branchValue = 4;\n"),
  );
  await execGit("git", ["add", "."], { cwd: root.fsPath });
  await execGit("git", ["commit", "-m", "feature change"], {
    cwd: root.fsPath,
  });
  const branch = await waitForOverview(
    (value) =>
      value.generation > deleted.generation &&
      value.repository?.branch === "feature" &&
      !value.pendingUpdate,
  );
  ...
  assert.ok(
    pulled.generation > branch.generation,
    "A same-branch HEAD advance should reconcile like a pull",
  );
}

async function waitForOverview(
  predicate: (overview: Overview) => boolean,
  timeoutMs = 30_000,
): Promise<Overview> {
  const started = Date.now();
  let lastOverview: Overview | undefined;
  while (Date.now() - started < timeoutMs) {
    const overview = await vscode.commands.executeCommand<Overview>(
  ...
  throw new Error(
    `Timed out waiting for continuous intelligence state. Last overview: ${JSON.stringify(lastOverview)}`,
  );
}
