...
**Documented Type**:
```typescript
export type ContextDeliveryMode = 
  | 'full' 
  | 'summary' 
  | 'references-only' 
  | 'adaptive-segments';  // ← DOCUMENTED BUT NOT IMPLEMENTED
```
...
|---------|------------|-------------|--------|
| File Hash Caching | ✅ | ⚠️ Partial | Minor Gap |
| Extraction Result Caching | ✅ | ⚠️ Partial | Minor Gap |
| Projection Caching | ✅ | ⚠️ Partial | Minor Gap |
| Query Result Caching (TTL) | ✅ | ❌ | **Critical Gap** |
| Context Compression Caching | ✅ | ❌ | **Critical Gap** |
| Continuation Packets | ✅ | ❌ | **Critical Gap** |
| adaptive-segments Delivery Mode | ✅ (type) | ❌ | **Critical Gap** |
...
| Priority | Gap | Effort | Risk | Dependencies |
|----------|-----|--------|------|--------------|
| P0 | Continuation Packets | Medium | High | ContextPacket type update |
| P0 | Context Compression Caching | Medium | Medium | Cache infrastructure |
| P0 | Query Result Caching | Low | Low | QueryEngine modification |
| P0 | adaptive-segments Delivery | Low | Low | intentContextBuilder only |
| P1 | File Hash Cache Persistence | Low | Low | repoIndexer + storage |
| P1 | Extraction Result Cache Persistence | Medium | Low | repoIndexer + storage |
| P1 | Projection Cache In-Memory | Low | Low | OkfStore + derivedGraph |
