...
export class CpgQueryService {
  ...
  async scope(raw: CpgScopeQuery, signal?: AbortSignal): Promise<CpgQueryResult | undefined> {
    ...
    const descriptor = manifest?.scopes
      .filter((scope) => scopeIds.has(scope.id))
      .sort(
        (left, right) =>
          scopePriority(left.kind) - scopePriority(right.kind) ||
          scopeSize(left) - scopeSize(right),
      )[0];
    if (!descriptor || !this.store.readCpgScope) return undefined;
    throwIfAborted(signal);
    const artifact = await this.store.readCpgScope(descriptor.id);
    if (!artifact) return undefined;
    throwIfAborted(signal);
    const allowed = (edge: CpgEdge): boolean =>
      (query.overlays.includes("ast") &&
        (edge.type === "AST_CHILD" || edge.type === "AST_PARENT")) ||
      (query.overlays.includes("evaluation") &&
        (edge.type === "EVAL_NEXT" || edge.type === "EVAL_PREVIOUS")) ||
      (query.overlays.includes("control-flow") && isCfgEdge(edge.type)) ||
      (query.overlays.includes("data-flow") && isDataFlowEdge(edge.type)) ||
      (query.overlays.includes("calls") &&
        [
          "CALLS_SYMBOL",
          "INSTANTIATES_SYMBOL",
    ...
    const required = new Set([
      artifact.entryNodeId,
      artifact.exitNodeId,
      ...selectedEdges.flatMap((edge) => [edge.sourceId, edge.targetId]),
    ]);
    const candidates = artifact.nodes.filter(
      (node) => required.has(node.id) || (query.overlays.includes("ast") && isAstKind(node.kind)),
    );
    const nodes = candidates
      .slice(0, query.maxNodes)
      .map((node) => (query.includeSource ? node : { ...node, code: undefined }));
  ...
  controlFlow(
    ...
    maxNodes = 200,
    signal?: AbortSignal,
  ): Promise<CpgQueryResult | undefined> {
    return this.scope(
      { semanticSymbolId, overlays: ["control-flow"], maxNodes, includeSource: true },
      signal,
    );
  }
  dataFlow(
    ...
    return this.scope(
      { semanticSymbolId, overlays: ["data-flow", "calls"], maxNodes, includeSource: true },
      signal,
    );
  }
  calls(
    ...
    return this.scope(
      { semanticSymbolId, overlays: ["calls"], maxNodes, includeSource: true },
      signal,
    );
  }
  ...
  async slice(raw: CpgSliceQuery, signal?: AbortSignal): Promise<CpgSliceResult | undefined> {
    ...
    if (!artifact) return undefined;
    throwIfAborted(signal);
    const result = this.slicer.slice(artifact, query);
    this.sliceCache.set(key, result);
    if (this.sliceCache.size > 100)
      this.sliceCache.delete(this.sliceCache.keys().next().value as string);
    return result;
  }
  ...
  conditionsForNode(
    ...
    nodeId: string,
    signal?: AbortSignal,
  ): Promise<CpgSliceResult | undefined> {
    return this.slice(
      {
        semanticSymbolId,
        nodeId,
  ...
  metrics() {
    return {
      ...
      averageQueryLatencyMs: this.queryLatencies.length
        ? this.queryLatencies.reduce((sum, value) => sum + value, 0) / this.queryLatencies.length
        : 0,
      sliceCacheHits: this.sliceCacheHits,
    };
  }
  private recordLatency(value: number): void {
    this.queryLatencies.push(value);
    if (this.queryLatencies.length > 100) this.queryLatencies = this.queryLatencies.slice(-100);
  }
}
...
function scopeSize(scope: {
  ...
  return (
    (scope.range.endLine - scope.range.startLine) * 100000 +
    scope.range.endColumn -
    scope.range.startColumn
  );
}
function scopePriority(kind: string): number {
  return kind === "callback" || kind === "module" ? 1 : 0;
}
function throwIfAborted(signal?: AbortSignal): void {
  if (signal?.aborted) {
    const error = new Error("CPG query was cancelled.");
