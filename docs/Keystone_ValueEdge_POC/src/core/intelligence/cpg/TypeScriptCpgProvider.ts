...
export class TypeScriptCpgProvider implements CodeAnalysisProvider {
  ...
  slice(artifact: CpgScopeArtifact, query: CpgSliceQuery): CpgSliceResult {
    return this.slicer.slice(artifact, query);
  }
  ...
  private analyze(request: BuildAnalysisRequest): Promise<CpgDelta> {
    ...
    for (const input of request.files.filter((file) => this.supports(file.language))) {
      const source = request.program
        .getSourceFiles()
        .find((item) => item.fileName.replace(/\\/g, "/").endsWith(`/${input.relativePath}`));
      if (!source) continue;
      const entityIndex = new Map(
        request.entities
          .filter((entity) => entity.fileId === input.fileId)
          .map((entity) => [`${entity.name}:${entity.range.startLine}`, entity.id]),
      );
      const context: CpgBuildContext = {
      ...
      artifacts.push(...this.builder.buildFile(context, this.cache, false));
    }
    ...
    this.cache.clear();
    for (const artifact of artifacts) this.cache.set(artifact.descriptor.id, artifact);
    return Promise.resolve({
      semanticGeneration: request.generation,
      providerId: this.id,
      providerVersion: this.version,
      scopes: artifacts,
      removedScopeIds,
...
export class CpgDiagnosticsService {
  summarize(artifacts: readonly CpgScopeArtifact[]) {
    return {
      failures: artifacts.reduce(
        (count, artifact) =>
          count + artifact.diagnostics.filter((item) => item.severity === "error").length,
        0,
      ),
      approximate: artifacts.reduce(
        (count, artifact) => count + artifact.descriptor.summary.approximateFlows,
        0,
      ),
      diagnostics: artifacts.flatMap((artifact) => artifact.diagnostics).slice(0, 100),
    };
  }
}

export class CpgProjectionService {
  project(artifact: CpgScopeArtifact) {
    return {
      ...
      summary: artifact.descriptor.summary,
    };
  }
}
