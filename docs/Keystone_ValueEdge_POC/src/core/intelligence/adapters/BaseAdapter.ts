...
export abstract class DeterministicAdapter implements IntelligenceAdapter {
  ...
  analyze(input: AdapterInput): Promise<AdapterOutput> {
    ...
    const selected = new Set(
      input.context.detections
        .filter((item) => item.adapterId === this.id)
...
export class AdapterOutputBuilder {
  ...
  constructor(
    ...
    readonly files: readonly SemanticSourceFileInput[],
  ) {
  ...
  entity(
    ...
    const value = this.factory.entity(
      file,
      type,
      name,
      qualifiedName,
    ...
    this.entities.set(value.entity.id, value.entity);
    this.evidence.set(value.evidence.id, value.evidence);
    return value.entity;
  }
  ...
  relationship(
    ...
    range: SourceRange,
    options?: Parameters<AdapterEvidenceFactory["relationship"]>[5],
  ): IntelligenceRelationshipRecord {
    const value = this.factory.relationship(source, target, type, owner, range, options);
    this.relationships.set(value.relationship.id, value.relationship);
    for (const evidence of value.evidence) this.evidence.set(evidence.id, evidence);
  ...
  diagnostic(
    ...
    file?: SemanticSourceFileInput,
    range?: SourceRange,
    extras: Pick<AdapterDiagnostic, "technologyId" | "limitation" | "ambiguity"> = {},
  ): void {
  ...
  finish(executionTimeMs: number): AdapterOutput {
    return {
      ...
      sourceContentHashes: Object.fromEntries(
        this.files.map((file) => [file.fileId, file.contentHash]),
      ),
      jobRevision: this.input.context.jobRevision,
      generationCompatibility: this.input.context.generation,
      ...
      exclusions: [],
      invalidations: this.files.map((file) => file.fileId),
      indexUpdates: [],
      okfProjectionHints: [],
      metrics: {
        adapterId: this.adapterId,
        executionTimeMs,
        filesConsidered: this.files.length,
        filesParsed: Math.max(0, this.files.length - this.failedFiles),
        filesFailed: this.failedFiles,
        cacheReused: 0,
        entitiesExtracted: this.entities.size,
        relationshipsResolved: this.relationships.size,
        crossLinksResolved: this.crossLinks,
        unsupportedFiles: this.unsupported,
        memoryWarning: false,
      },
    };
  }
...
export function detection(
  adapterId: string,
  technologyId: string,
  level: AdapterDetection["capabilityLevel"],
  files: readonly SemanticSourceFileInput[],
  kind: AdapterDetection["evidence"][number]["kind"],
  statement: string,
  confidence = 1,
  ...
  return {
    ...
    evidence: files
      .slice(0, 20)
      .map((file) => ({ kind, relativePath: file.relativePath, statement })),
    conflicts: [],
    unsupportedFeatures,
...
export function matches(path: string, expressions: readonly RegExp[]): boolean {
...
export function importsModule(file: SemanticSourceFileInput, modules: readonly string[]): boolean {
  if (!["typescript", "typescriptreact", "javascript", "javascriptreact"].includes(file.language))
    return false;
  ...
  const matchesModule = (value: string): boolean =>
    modules.some((module) => value === module || value.startsWith(`${module}/`));
  let found = false;
  const visit = (node: ts.Node): void => {
    ...
    if (
      ...
      matchesModule(node.moduleSpecifier.text)
    )
      found = true;
    else if (
      ...
      matchesModule(node.moduleReference.expression.text)
    )
      found = true;
    else if (
      ...
      matchesModule(node.arguments[0].text)
    )
      found = true;
