...
export class TypeScriptJavaScriptParser implements SemanticParser {
  ...
  async extract(request: SemanticProjectRequest): Promise<SemanticProjectResult> {
    ...
    if (request.reset) {
      ...
      throw new Error(
        "SEMANTIC_CONTEXT_MISSING: a complete semantic rebuild is required after worker restart.",
      );
    }
    for (const relativePath of request.removedPaths) {
      for (const [key, file] of state.files)
        if (file.relativePath === relativePath || file.fileId === relativePath)
    ...
    const universal = await this.universal.analyze(
      { ...request, changedFiles: inputs },
      result.entities,
    ...
    result.diagnostics = [...result.diagnostics, ...universal.diagnostics].sort(
      (left, right) =>
    ...
    result.cpg = request.reset
      ...
      : await this.cpg.update({
          ...
          evidence: result.evidence,
          analysisLevel: "basic",
          changedFileIds: request.changedFiles.map((file) => file.fileId),
          removedFileIds: request.removedPaths,
        });
    for (const artifact of [...result.cpg.scopes].sort(
      (left, right) =>
        cpgScopePriority(left.descriptor.kind) - cpgScopePriority(right.descriptor.kind),
    )) {
      const entity = result.entities.find(
        (item) => item.id === artifact.descriptor.semanticSymbolId,
      );
      if (!entity || entity.codeAnalysis) continue;
      entity.codeAnalysis = {
        scopeId: artifact.descriptor.id,
        structuralHash: artifact.descriptor.structuralHash,
        providerId: artifact.descriptor.providerId,
        providerVersion: artifact.descriptor.providerVersion,
        calculationMethod: `${artifact.descriptor.providerId}@${artifact.descriptor.providerVersion}:scope-summary`,
        confidence: artifact.descriptor.summary.approximateFlows > 0 ? 0.7 : 1,
        evidenceIds: entity.evidenceIds,
        branches: artifact.descriptor.summary.branches,
        calls: artifact.descriptor.summary.calls,
        reads: artifact.descriptor.summary.reads,
        writes: artifact.descriptor.summary.writes,
        unresolvedCalls: artifact.descriptor.summary.unresolvedCalls,
      };
    }
    return result;
  }
...
function mergeById<T extends { id: string }>(left: T[], right: T[]): T[] {
  return [...new Map([...left, ...right].map((item) => [item.id, item])).values()].sort(
    (first, second) => first.id.localeCompare(second.id),
  );
}

function augmentContributions(
  result: SemanticProjectResult,
  inputs: SemanticSourceFileInput[],
): FileContribution[] {
  const byFile = new Map(
    result.contributions.map((item) => [
      item.fileId,
      {
        ...
        diagnosticIds: [...item.diagnosticIds],
        dependencyFileIds: [...item.dependencyFileIds],
      },
    ]),
  );
  for (const input of inputs)
    if (!byFile.has(input.fileId))
      byFile.set(input.fileId, {
        fileId: input.fileId,
  ...
  for (const entity of result.entities)
    addContributionValue(byFile, entity.ownerFileId ?? entity.fileId, "entityIds", entity.id);
  for (const relationship of result.relationships) {
    if (!relationship.ownerFileId) continue;
    addContributionValue(byFile, relationship.ownerFileId, "relationshipIds", relationship.id);
    if (relationship.targetFileId && relationship.targetFileId !== relationship.ownerFileId)
      addContributionValue(
        byFile,
        relationship.ownerFileId,
        "dependencyFileIds",
        relationship.targetFileId,
      );
  }
  for (const evidence of result.evidence)
    if (evidence.ownerFileId)
      addContributionValue(byFile, evidence.ownerFileId, "evidenceIds", evidence.id);
  for (const diagnostic of result.diagnostics)
    if (diagnostic.ownerFileId && diagnostic.id)
      addContributionValue(byFile, diagnostic.ownerFileId, "diagnosticIds", diagnostic.id);
  return [...byFile.values()]
    .map((item) => ({
      ...item,
      entityIds: [...new Set(item.entityIds)].sort(),
      relationshipIds: [...new Set(item.relationshipIds)].sort(),
      evidenceIds: [...new Set(item.evidenceIds)].sort(),
      diagnosticIds: [...new Set(item.diagnosticIds)].sort(),
      dependencyFileIds: [...new Set(item.dependencyFileIds)].sort(),
    }))
    .sort((left, right) => left.fileId.localeCompare(right.fileId));
}

function addContributionValue(
  values: Map<string, FileContribution>,
  fileId: string,
  key: "entityIds" | "relationshipIds" | "evidenceIds" | "diagnosticIds" | "dependencyFileIds",
  value: string,
): void {
  const contribution = values.get(fileId);
  if (contribution) contribution[key].push(value);
}
...
class GraphAccumulator implements SemanticExtractionContext {
  ...
  extractSymbols(): void {
    for (const [source, input] of this.sourceInputs) {
      this.fileUpdates.set(input.fileId, {
        ...
        parseStatus: "parsed",
        shardReferences: [
          `entities/${input.fileId}`,
          `relationships/${input.fileId}`,
          `evidence/${input.fileId}`,
        ],
      });
  ...
  extractImportsAndExports(): void {
    for (const [source, input] of this.sourceInputs)
      this.extractImportsAndExportsForFile(source, input);
  }
  resolveTypes(): void {
    for (const [source, input] of this.sourceInputs) this.extractTypesForFile(source, input);
  }
  extractTests(): void {
    for (const [source, input] of this.sourceInputs) this.extractTestsForFile(source, input);
  }
  extractRoutes(): void {
    for (const [source, input] of this.sourceInputs) this.extractRoutesForFile(source, input);
  }
  resolveCalls(): void {
    for (const [source, input] of this.sourceInputs) this.extractCallsForFile(source, input);
  }
  resolveReferences(): void {
    for (const [source, input] of this.sourceInputs) this.extractReferencesForFile(source, input);
  }
  extractReact(): void {
    for (const [source, input] of this.sourceInputs) this.extractReactForFile(source, input);
  }
  extractConfiguration(): void {
    for (const [source, input] of this.sourceInputs)
      this.extractConfigurationForFile(source, input);
  }

  result(): SemanticProjectResult {
    const diagnostics = [...this.resolutionDiagnostics.all()];
    const entities = [...this.entities.values()].sort((left, right) =>
      left.id.localeCompare(right.id),
    );
    const relationships = [...this.relationships.values()].sort((left, right) =>
      left.id.localeCompare(right.id),
    );
    const evidence = [...this.evidence.values()].sort((left, right) =>
      left.id.localeCompare(right.id),
    );
    ...
    return {
      ...
      fileUpdates: [...this.fileUpdates.values()].sort((left, right) =>
        left.id.localeCompare(right.id),
      ),
      entities,
      relationships,
  ...
  extractPackages(): void {
    for (const input of this.inputs.filter((file) =>
      /(^|\/)package\.json$/.test(file.relativePath),
    )) {
      let value: unknown;
      try {
        value = JSON.parse(input.content) as unknown;
      } catch {
        this.resolutionDiagnostics.add(
          "parse-failure",
      ...
      const name =
        typeof record.name === "string"
          ? record.name
          : input.relativePath.replace(/\/package\.json$/, "") || "workspace";
      const packageEntity = this.addInputEntity(
        input,
        "keystone.core.Package",
        name,
        `package:${name}`,
        wholeFileRange(input.content),
        { exported: true },
      );
      ...
      if (scripts && typeof scripts === "object") {
        for (const scriptName of Object.keys(scripts).sort()) {
          const type = scriptName.includes("test")
          ...
          const script = this.addInputEntity(
            input,
            type,
            scriptName,
    ...
    for (const input of this.inputs.filter((file) =>
      /(^|\/)(tsconfig|jsconfig)(?:\.[^/]*)?\.json$/.test(file.relativePath),
    )) {
      const config = this.addInputEntity(
        input,
      ...
      const configValue = parsed.config as
        | {
      ...
      for (const alias of Object.keys(configValue?.compilerOptions?.paths ?? {}).sort()) {
        const key = this.addInputEntity(
          input,
          "keystone.core.ConfigurationKey",
          alias,
          `${input.relativePath}.paths.${alias}`,
          wholeFileRange(input.content),
        );
        this.addRelationship(
          config.id,
      ...
      for (const reference of configValue?.references ?? [])
        if (typeof reference.path === "string") {
          const key = this.addInputEntity(
            ...
            "keystone.core.ConfigurationKey",
            reference.path,
            `${input.relativePath}.references.${reference.path}`,
            wholeFileRange(input.content),
          );
          this.addRelationship(
            config.id,
            ...
            1,
            "exact",
            { configurationKind: "project-reference" },
          );
        }
      const update = this.fileUpdates.get(input.fileId) ?? { id: input.fileId };
  ...
  private addModule(source: ts.SourceFile, input: SemanticSourceFileInput): void {
    const module = this.addNodeEntity(
      source,
      input,
      "keystone.core.Module",
      input.relativePath,
      `module:${input.relativePath}`,
      undefined,
      { exported: true },
    );
    ...
    if (packageId) {
      this.addRelationship(
        module.id,
        packageId,
        "keystone.core.BELONGS_TO",
        input,
        rangeOf(source, source),
        "resolved",
        1,
        "exact",
      );
    ...
    const visit = (node: ts.Node): void => {
      ...
      if (type && nameNode) {
        ...
        if (name) {
          ...
          const entity = this.addNodeEntity(
            node,
            input,
            kind,
            name,
            ...
            signature,
            declarationProperties(node, this.program.getTypeChecker(), parentId),
          );
          this.declarationIds.set(node, entity.id);
          const symbol = this.program.getTypeChecker().getSymbolAtLocation(nameNode);
          if (symbol)
            this.symbolIds.set(resolveAlias(symbol, this.program.getTypeChecker()), entity.id);
          if (parentId)
            this.addRelationship(
              parentId,
              entity.id,
              ts.isParameter(node) ? "keystone.core.HAS_PARAMETER" : "keystone.core.HAS_MEMBER",
              input,
              rangeOf(node, source),
              "extracted",
              1,
  ...
  private extractImportsAndExportsForFile(
      ...
      if (ts.isImportDeclaration(statement) && ts.isStringLiteral(statement.moduleSpecifier)) {
        ...
        if (targetId) {
          ...
          this.addRelationship(
            ...
            input,
            rangeOf(statement.moduleSpecifier, source),
            "resolved",
            local ? 1 : 0.9,
            local ? "compiler" : "external",
            {
          ...
          this.resolutionDiagnostics.add(
            "unresolved-import",
            `Could not resolve ${statement.moduleSpecifier.text}.`,
            input.fileId,
            input.workspaceRootId,
            input.relativePath,
            rangeOf(statement.moduleSpecifier, source),
        ...
        for (const binding of bindings) {
          const alias = this.addNodeEntity(
            binding,
            input,
            "keystone.core.Alias",
            binding.text,
            qualifiedName(binding, binding.text),
            undefined,
            { parentId: input.fileId },
          );
          const symbol = checker.getSymbolAtLocation(binding);
          const target =
            named && ts.isNamespaceImport(named) && named.name === binding
              ? targetId
              : symbol
                ? this.symbolIds.get(resolveAlias(symbol, checker))
                : undefined;
          const typeOnly =
            statement.importClause?.isTypeOnly === true ||
            (ts.isImportSpecifier(binding.parent) && binding.parent.isTypeOnly);
          if (target) {
            this.addRelationship(
              alias.id,
              target,
              "keystone.core.ALIASES",
              input,
              rangeOf(binding, source),
              "resolved",
              1,
              "compiler",
              { alias: binding.text, typeOnly },
            );
            this.addRelationship(
              alias.id,
              target,
        ...
        if (statement.moduleSpecifier && ts.isStringLiteral(statement.moduleSpecifier)) {
          ...
          if (target)
            this.addRelationship(
              input.fileId,
              target,
              "keystone.core.RE_EXPORTS",
              input,
              rangeOf(statement, source),
              "resolved",
              1,
              target.startsWith("file:") ? "compiler" : "external",
              { moduleSpecifier: statement.moduleSpecifier.text, typeOnly: statement.isTypeOnly },
            );
        }
          ...
          for (const element of statement.exportClause.elements) {
            ...
            if (target)
              this.addRelationship(
                ...
                input,
                rangeOf(element, source),
                "resolved",
                1,
                "compiler",
                { exportedName: element.name.text, typeOnly: element.isTypeOnly },
              );
          }
        }
      }
    ...
    for (const entity of this.entities.values()) {
      ...
      this.addRelationship(
        ...
        1,
        "exact",
        { default: entity.defaultExport ?? false },
      );
    }
    const visit = (node: ts.Node): void => {
      if (
        ...
        if (target)
          this.addRelationship(
            this.enclosingEntity(node, input.fileId),
            target,
            "keystone.core.IMPORTS",
            input,
            rangeOf(node, source),
            "resolved",
            target.startsWith("file:") ? 1 : 0.9,
            target.startsWith("file:") ? "compiler" : "external",
            { moduleSpecifier: node.arguments[0].text, commonJs: true },
          );
      }
      if (
        ts.isCallExpression(node) &&
        ...
        if (target)
          this.addRelationship(
            ...
            1,
            target.startsWith("file:") ? "compiler" : "external",
            { moduleSpecifier: node.arguments[0].text, dynamic: true },
          );
      }
      if (
        ts.isBinaryExpression(node) &&
        node.operatorToken.kind === ts.SyntaxKind.EqualsToken &&
        isCommonJsExport(node.left)
      ) {
        const symbol = symbolForExpression(node.right, checker);
        const target =
          this.declarationIds.get(node.right) ??
          (symbol ? this.symbolIds.get(resolveAlias(symbol, checker)) : undefined);
        if (target)
          this.addRelationship(
            input.fileId,
            target,
  ...
  private extractTypesForFile(source: ts.SourceFile, input: SemanticSourceFileInput): void {
    ...
    const visit = (node: ts.Node): void => {
              ...
              if (target)
                this.addRelationship(
                  ...
                  clause.token === ts.SyntaxKind.ImplementsKeyword
                    ? "keystone.core.IMPLEMENTS"
                    : "keystone.core.EXTENDS",
                  input,
                  rangeOf(type, source),
                  "resolved",
                  1,
      ...
      if (isCallableDeclaration(node)) {
        ...
        if (sourceId) {
          const accepts =
            this.entities.get(sourceId)?.type === "keystone.core.Component"
              ? "keystone.core.ACCEPTS_PROPS"
              : "keystone.core.ACCEPTS_TYPE";
          for (const parameter of node.parameters)
            this.addTypeRelationship(sourceId, parameter.type, accepts, input, source, checker);
          this.addTypeRelationship(
            sourceId,
            node.type,
      ...
      if (ts.isCallExpression(node)) {
        ...
        if (title && ["describe", "suite"].includes(callName)) {
          ...
          const entity = this.addNodeEntity(
            ...
            title,
            `${input.relativePath}::${currentSuite ?? "root"}::${title}`,
          );
          this.testCases.set(node, entity.id);
          this.addRelationship(
            currentSuite ?? input.fileId,
          ...
          for (const parameter of callback?.parameters ?? []) {
            ...
            const fixture = this.addNodeEntity(
              parameter,
              input,
              "keystone.core.Fixture",
              name,
              `${entity.qualifiedName}.fixture.${name}`,
            );
            this.addRelationship(
              entity.id,
              fixture.id,
          ...
          ["beforeAll", "beforeEach", "afterAll", "afterEach", "before", "after"].includes(callName)
        ) {
          const entity = this.addNodeEntity(
            node,
            input,
            "keystone.core.TestHook",
            callName,
            `${input.relativePath}::${currentSuite ?? "root"}::${callName}@${node.getStart(source)}`,
          );
          this.testCases.set(node, entity.id);
          this.addRelationship(
            currentSuite ?? input.fileId,
  ...
  private extractCallsForFile(source: ts.SourceFile, input: SemanticSourceFileInput): void {
    ...
    const visit = (node: ts.Node): void => {
      if (ts.isCallExpression(node) || ts.isNewExpression(node)) {
        ...
        let target =
          this.importCallTarget(node.expression, source) ??
          (declaration
            ? (this.declarationIds.get(declaration) ??
              this.symbolIds.get(
                resolveAlias(
                  checker.getSymbolAtLocation(declarationNameNode(declaration) ?? declaration) ??
                    ({ flags: 0 } as ts.Symbol),
                  checker,
                ),
              ))
            : undefined);
        ...
        const relationshipType = ts.isNewExpression(node)
        ...
        if (target) {
          this.addRelationship(
            ...
            relationshipType,
            input,
            rangeOf(node.expression, source),
            "resolved",
            1,
            "compiler",
            {
              argumentCount: node.arguments?.length ?? 0,
              optional: ts.isCallExpression(node) && Boolean(node.questionDotToken),
          ...
          if (test && !this.isTestEntity(target)) {
            ...
            const targets = exactTestTargets.get(test) ?? new Set<string>();
            targets.add(target);
            exactTestTargets.set(test, targets);
          }
          const targetEntity = this.entities.get(target);
          if (
            targetEntity?.type === "keystone.core.Hook" ||
            /^use[A-Z0-9]/.test(targetEntity?.name ?? "")
          )
            this.addRelationship(
              caller,
              target,
              "keystone.core.USES_HOOK",
              input,
              rangeOf(node.expression, source),
              "framework-rule",
              1,
          ...
          this.resolutionDiagnostics.add(
            "unresolved-call",
            `The compiler could not resolve call target ${node.expression.getText(source)}.`,
            input.fileId,
            input.workspaceRootId,
            input.relativePath,
            rangeOf(node.expression, source),
        ...
        if (
          ts.isCallExpression(node) &&
          expressionName(node.expression) === "useContext" &&
          node.arguments[0]
        ) {
          const contextSymbol = symbolForExpression(node.arguments[0], checker);
          const contextTarget = contextSymbol
            ? this.symbolIds.get(resolveAlias(contextSymbol, checker))
            : undefined;
          if (contextTarget)
            this.addRelationship(
              caller,
              contextTarget,
              "keystone.core.CONSUMES_CONTEXT",
              input,
              rangeOf(node, source),
              "framework-rule",
              1,
        ...
        if (ts.isCallExpression(node) && isCallbackRegistration(node.expression)) {
          for (const argument of node.arguments) {
            ...
            const callbackTarget = callbackSymbol
              ? this.symbolIds.get(resolveAlias(callbackSymbol, checker))
              : undefined;
            if (callbackTarget)
              this.addRelationship(
                caller,
                ...
                input,
                rangeOf(argument, source),
                "resolved",
                1,
                "compiler",
                { registration: expressionName(node.expression) },
              );
          }
        }
      }
  ...
  private extractReferencesForFile(source: ts.SourceFile, input: SemanticSourceFileInput): void {
    ...
    const visit = (node: ts.Node): void => {
      if (
        ...
        } else if (!symbol && !KNOWN_GLOBALS.has(node.text))
          this.resolutionDiagnostics.add(
            "unresolved-symbol",
            `The compiler could not resolve ${node.text}.`,
            input.fileId,
            input.workspaceRootId,
            input.relativePath,
            rangeOf(node, source),
        ...
        if (ts.isPropertyAccessExpression(tag) && tag.name.text === "Provider") {
          ...
          if (target)
            this.addRelationship(
              this.enclosingEntity(node, input.fileId),
              target,
              "keystone.core.PROVIDES_CONTEXT",
              input,
              rangeOf(tag, source),
              "framework-rule",
              1,
  ...
  private extractRoutesForFile(source: ts.SourceFile, input: SemanticSourceFileInput): void {
    const visit = (node: ts.Node): void => {
      if (ts.isCallExpression(node)) this.extractRoute(node, input, source);
      ts.forEachChild(node, visit);
    };
    visit(source);
  }
  ...
  private extractRoute(
    ...
    if (
      routeMethods.has(name) &&
      first &&
      ts.isStringLiteralLike(first) &&
      (first.text.startsWith("/") || first.text === "*") &&
      ts.isPropertyAccessExpression(node.expression) &&
      isRouterReceiver(node.expression.expression)
    ) {
      method = name.toUpperCase();
      routeName = first.text;
      handlerArguments = node.arguments.slice(1);
    ...
    const route = this.addNodeEntity(
      node,
      input,
      type,
      routeName,
    ...
    for (let index = 0; index < handlerArguments.length; index++) {
      ...
      if (!target && (ts.isArrowFunction(argument) || ts.isFunctionExpression(argument))) {
        ...
        const callback = this.addNodeEntity(
          argument,
          input,
          role === "middleware" ? "keystone.core.Middleware" : "keystone.core.Function",
          `${method} ${routeName} ${role}`,
          `${input.relativePath}::${method} ${routeName}::${role}@${argument.getStart(source)}`,
          signatureOf(argument, checker),
        );
      ...
      if (!target) {
        this.resolutionDiagnostics.add(
          ...
          `Could not resolve handler for ${method} ${routeName}.`,
          input.fileId,
          input.workspaceRootId,
          input.relativePath,
          rangeOf(argument, source),
      ...
      const relation =
        index === handlerArguments.length - 1
      ...
      this.addRelationship(
        ...
        1,
        "framework",
        { method, path: routeName },
      );
    }
  }

  private extractConfigurationCall(
    node: ts.CallExpression,
    owner: string,
    input: SemanticSourceFileInput,
    source: ts.SourceFile,
    ...
    this.addRelationship(
      ...
      1,
      "exact",
      { key, fallback: node.arguments.length > 1 },
    );
  }

  private extractConfigurationAccess(
    ...
    this.addRelationship(
      ...
      1,
      "exact",
      { key, source: text.startsWith("process") ? "process.env" : "import.meta.env", fallback },
    );
  }

  private extractOverride(
    ...
    for (const base of checker.getBaseTypes(classType as ts.InterfaceType) ?? []) {
      ...
      if (target)
        this.addRelationship(
          sourceId,
          target,
          "keystone.core.OVERRIDES",
          input,
          rangeOf(node.name, source),
          "resolved",
          1,
          "compiler",
        );
  ...
  private addTypeRelationship(
    ...
    if (!typeNode) return;
    const symbol =
      checker.getSymbolAtLocation(typeNode) ?? checker.getTypeAtLocation(typeNode).getSymbol();
    const target = symbol ? this.symbolIds.get(resolveAlias(symbol, checker)) : undefined;
    if (target)
      this.addRelationship(
        sourceId,
        ...
        type,
        input,
        rangeOf(typeNode, source),
        "resolved",
        1,
        "compiler",
        { typeText: typeNode.getText(source) },
      );
  }

  private resolveModuleTarget(
    specifier: string,
    source: ts.SourceFile,
    input: SemanticSourceFileInput,
  ): string | undefined {
    ...
    const resolved = ts.resolveModuleName(specifier, source.fileName, this.options, {
      fileExists: (value) => this.host.fileExists(value),
      readFile: (value) => this.host.readFile(value),
      directoryExists: (value) => this.host.directoryExists?.(value) ?? false,
      getCurrentDirectory: () => this.host.getCurrentDirectory(),
      realpath: (value) => this.host.realpath?.(value) ?? value,
    }).resolvedModule;
    if (resolved) {
    ...
    if (!specifier.startsWith(".") && !specifier.startsWith("/"))
      return this.ensureExternalDependency(input, packageName(specifier)).id;
    return undefined;
  }

  private ensureExternalDependency(
    input: SemanticSourceFileInput,
    name: string,
  ): IntelligenceSymbolRecord {
    const existingId = this.externalByName.get(`${input.workspaceRootId}:${name}`);
    if (existingId) return this.entities.get(existingId)!;
    const entity = this.addInputEntity(
      input,
  ...
  private addInputEntity(
    input: SemanticSourceFileInput,
    type: string,
    name: string,
    qualified: string,
    range: SourceRange,
    properties: Partial<IntelligenceSymbolRecord> = {},
  ): IntelligenceSymbolRecord {
    const id = this.ids.entity(
      this.request.repositoryId,
      input.fileId,
      type,
      qualified,
    ...
    this.addEntity(
      ...
      input,
      range,
      `The ${type.replace("keystone.core.", "")} ${qualified} was extracted.`,
    );
    return this.entities.get(id)!;
  }

  private addNodeEntity(
    node: ts.Node,
    input: SemanticSourceFileInput,
    type: string,
    name: string,
    ...
    const jsDocRange =
      firstDoc && lastDoc
        ? rangeFromOffsets(source, firstDoc.getStart(source), lastDoc.getEnd())
        : undefined;
    return this.addInputEntity(input, type, name, qualified, range, {
      ...(signature ? { signature } : {}),
      ...(nameNode ? { nameRange: rangeOf(nameNode, source) } : {}),
      ...(jsDocRange ? { jsDocRange } : {}),
      ...properties,
    });
  }
  ...
  private addEntity(
    ...
    const evidence = this.evidenceFactory.create({
      ...
      range,
      sourceKind:
        entity.type.startsWith("keystone.core.Route") || entity.type.includes("Test")
          ? "framework-rule"
          : entity.type === "keystone.core.Package" ||
              entity.type === "keystone.core.ExternalDependency"
            ? "manifest"
            : "typescript-compiler",
  ...
  private addRelationship(
    ...
    confidence: number,
    resolution: NonNullable<IntelligenceRelationshipRecord["resolution"]>,
    ...
    const id = this.ids.relationship(
      this.request.repositoryId,
      sourceId,
      targetId,
      type,
    ...
    const evidence = this.evidenceFactory.create({
      ...
      relativePath: input.relativePath,
      range,
      sourceKind: derivation === "framework-rule" ? "framework-rule" : "typescript-compiler",
      extractorId: PARSER_ID,
      extractorVersion: PARSER_VERSION,
      derivation,
      contentHash: input.contentHash,
      ...
      generation: this.request.generation,
      confidence,
      statement: `${type.replace("keystone.core.", "")} was ${derivation} from this source range.`,
    });
    const targetFileId =
      this.entities.get(targetId)?.fileId ??
      this.inputs.find((file) => file.fileId === targetId)?.fileId;
  ...
  private enclosingTest(node: ts.Node): string | undefined {
    for (let current: ts.Node | undefined = node; current; current = current.parent)
      if (
        ...
        this.testCases.has(current) &&
        this.entities.get(this.testCases.get(current)!)?.type === "keystone.core.TestCase"
      )
        return this.testCases.get(current);
    return undefined;
  }

  private isTestEntity(id: string): boolean {
    return this.entities.get(id)?.type.startsWith("keystone.core.Test") ?? false;
  }

  private addNamingCandidates(
    input: SemanticSourceFileInput,
    source: ts.SourceFile,
    ...
    for (const [testNode, testId] of this.testCases) {
      if (
        testNode.getSourceFile() !== source ||
        this.entities.get(testId)?.type !== "keystone.core.TestCase"
      )
        continue;
      if ((exact.get(testId)?.size ?? 0) > 0) continue;
      const title = this.entities.get(testId)?.name.toLowerCase() ?? "";
      const candidate = [...this.entities.values()].find(
        (entity) =>
          entity.fileId !== input.fileId &&
          entity.name.length > 2 &&
          title.includes(entity.name.toLowerCase()) &&
          ["keystone.core.Function", "keystone.core.Method", "keystone.core.Class"].includes(
            entity.type,
          ),
      );
      if (candidate)
        this.addRelationship(
          ...
          "keystone.core.TESTS",
          input,
          rangeOf(testNode, source),
          "calculated",
          0.35,
          "candidate",
          { evidenceLevel: 4, candidateReason: "test-name" },
        );
    }
  }

  private registerImportTargets(
    ...
    if (clause?.name) {
      const target = [...this.entities.values()].find(
        (entity) => entity.fileId === targetFileId && entity.defaultExport,
      );
      if (target) targets.set(clause.name.text, target.id);
    }
    const bindings = clause?.namedBindings;
    if (bindings && ts.isNamespaceImport(bindings))
      targets.set(`${bindings.name.text}.*`, targetFileId);
    else if (bindings)
      for (const element of bindings.elements) {
        const importedName = element.propertyName?.text ?? element.name.text;
        const target = [...this.entities.values()].find(
          (entity) =>
            entity.fileId === targetFileId &&
            entity.name === importedName &&
            entity.type !== "keystone.core.Parameter",
        );
  ...
  private importCallTarget(expression: ts.Expression, source: ts.SourceFile): string | undefined {
    const targets = this.importTargets.get(source);
      ...
      if (fileId)
        return [...this.entities.values()].find(
          (entity) =>
            ...
            entity.name === expression.name.text &&
            entity.type !== "keystone.core.Parameter",
        )?.id;
    }
    return undefined;
    ...
    for (const item of this.program.getSyntacticDiagnostics(source)) {
      const range =
        item.start === undefined
          ? undefined
          : rangeFromOffsets(source, item.start, item.start + (item.length ?? 0));
      this.resolutionDiagnostics.add(
        "parse-failure",
      ...
      const update = this.fileUpdates.get(input.fileId);
      if (update) update.parseStatus = "failed";
    }
  }
    ...
    return this.inputs
      .map((file) => {
        ...
        const dependencies = new Set(
          ownedRelationships
            .map((item) => item.targetFileId)
            .filter((item): item is string => Boolean(item) && item !== file.fileId),
        );
        return {
          fileId: file.fileId,
          sourceHash: file.contentHash,
          structuralHash: this.fileUpdates.get(file.fileId)?.structuralHash,
          parserId: PARSER_ID,
          parserVersion: PARSER_VERSION,
          entityIds: ownedEntities.map((item) => item.id),
          relationshipIds: ownedRelationships.map((item) => item.id),
          evidenceIds: ownedEvidence.map((item) => item.id),
          diagnosticIds: ownedDiagnostics.flatMap((item) => (item.id ? [item.id] : [])),
          dependencyFileIds: [...dependencies].sort(),
          generation: this.request.generation,
        };
      })
      .sort((left, right) => left.fileId.localeCompare(right.fileId));
...
function createCompilerHost(
  ...
  virtual: Map<string, SemanticSourceFileInput>,
): ts.CompilerHost {
  const fallback = ts.createCompilerHost(options, true);
  const content = new Map(
    [...virtual].map(([path, input]) => [normalizeFileName(path), input.content]),
  );
  ...
  const libraryFile = (fileName: string): boolean => {
    ...
    return (
      posix.dirname(normalized) === libraryRoot &&
      /^lib(?:\..+)?\.d\.ts$/.test(posix.basename(normalized))
    );
  };
  return {
    ...fallback,
    fileExists: (fileName) =>
      content.has(normalizeFileName(fileName)) ||
      (libraryFile(fileName) && fallback.fileExists(fileName)),
    readFile: (fileName) =>
      content.get(normalizeFileName(fileName)) ??
      (libraryFile(fileName) ? fallback.readFile(fileName) : undefined),
    getSourceFile: (fileName, languageVersion) => {
      ...
      return value === undefined
        ? libraryFile(fileName)
          ...
          : undefined
        : ts.createSourceFile(fileName, value, languageVersion, true, scriptKind(fileName));
    },
    resolveModuleNames: (moduleNames, containingFile) =>
      moduleNames.map((specifier) => {
        ...
        const resolvedFileName = [
          ...
          `${base}/index.js`,
          `${base}/index.jsx`,
        ].find((candidate) => content.has(candidate));
        return resolvedFileName
          ? {
              resolvedFileName,
              extension: moduleExtension(resolvedFileName),
    ...
    directoryExists: (directoryName) => {
      ...
      return (
        ...
        normalized === "/__keystone__" ||
        [...content.keys()].some((path) => path.startsWith(`${normalized}/`))
      );
    },
    getDirectories: () => [],
...
function virtualPath(input: SemanticSourceFileInput): string {
  return normalizeFileName(
    `/__keystone__/${input.workspaceRootId.replace(/[^a-zA-Z0-9_-]/g, "_")}/${input.relativePath}`,
  );
}
...
function moduleExtension(path: string): ts.Extension {
  return path.endsWith(".tsx")
    ? ts.Extension.Tsx
    : path.endsWith(".jsx")
      ? ts.Extension.Jsx
...
function structuralHash(source: ts.SourceFile): string {
...
function sourceRoot(path: string): string {
  ...
  return first && ["src", "lib", "app", "test", "tests"].includes(first) ? first : "";
}
function cpgScopePriority(kind: string): number {
  return kind === "callback" || kind === "module" ? 1 : 0;
}
function wholeFileRange(content: string): SourceRange {
...
function rangeOf(node: ts.Node, source: ts.SourceFile): SourceRange {
  return rangeFromOffsets(source, node.getStart(source, false), node.getEnd());
}
...
function declarationType(node: ts.Node): string | undefined {
  ...
  if (
    ...
    return "keystone.core.Method";
  if (ts.isConstructorDeclaration(node)) return "keystone.core.Constructor";
  if (ts.isPropertyDeclaration(node) || ts.isPropertySignature(node))
    return "keystone.core.Property";
  if (ts.isVariableDeclaration(node) && ts.isIdentifier(node.name))
    return isConstVariable(node) ? "keystone.core.Constant" : "keystone.core.Variable";
...
function declarationName(node: ts.Node): string | undefined {
  return ts.isIdentifier(node) || ts.isPrivateIdentifier(node) || ts.isStringLiteralLike(node)
    ...
    : node.kind === ts.SyntaxKind.Constructor
      ? "constructor"
      : node.getText().slice(0, 80) || undefined;
}
function qualifiedName(node: ts.Node, name: string): string {
  ...
  for (let current = node.parent; current && !ts.isSourceFile(current); current = current.parent) {
    const parentName = declarationNameNode(current);
    const value = parentName ? declarationName(parentName) : undefined;
    if (value) names.unshift(value);
  }
  return names.join(".");
}
function findParentId(node: ts.Node | undefined, ids: Map<ts.Node, string>): string | undefined {
...
function isConstVariable(node: ts.VariableDeclaration): boolean {
  return (
...
function isCallableDeclaration(node: ts.Node): node is ts.SignatureDeclaration {
  return ts.isFunctionLike(node) && !ts.isFunctionTypeNode(node) && !ts.isConstructorTypeNode(node);
}
function signatureOf(node: ts.Node, checker: ts.TypeChecker): string | undefined {
  if (!isCallableDeclaration(node)) return undefined;
  const signature = checker.getSignatureFromDeclaration(node);
  return signature
    ? checker.signatureToString(signature, node, ts.TypeFormatFlags.NoTruncation)
    : undefined;
}
function reactKind(node: ts.Node, name: string, type: string): string {
  if (
    ts.isVariableDeclaration(node) &&
    node.initializer &&
    ts.isCallExpression(node.initializer) &&
    expressionName(node.initializer.expression) === "createContext"
  )
    return "keystone.react.Context";
  if ((ts.isFunctionLike(node) || ts.isVariableDeclaration(node)) && /^use[A-Z0-9]/.test(name))
    return "keystone.core.Hook";
  const componentCandidate =
    ...
    (ts.isVariableDeclaration(node) &&
      ...
      (ts.isArrowFunction(node.initializer) || ts.isFunctionExpression(node.initializer)));
  if (componentCandidate && /^[A-Z]/.test(name) && containsJsx(node))
    return "keystone.core.Component";
  return type;
}
function containsJsx(node: ts.Node): boolean {
  let found = false;
  const visit = (child: ts.Node): void => {
    if (isJsxTag(child) || ts.isJsxElement(child) || ts.isJsxFragment(child)) found = true;
...
function declarationProperties(
  ...
  checker: ts.TypeChecker,
  parentId?: string,
): Partial<IntelligenceSymbolRecord> {
  const modifierNode =
    ts.isVariableDeclaration(node) &&
  ...
  const parameters = isCallableDeclaration(node)
    ? node.parameters.map((item) => ({
        name: item.name.getText(),
        ...(item.type ? { type: item.type.getText() } : {}),
        ...(item.questionToken ? { optional: true } : {}),
        ...(item.dotDotDotToken ? { rest: true } : {}),
      }))
    : undefined;
  const signature = isCallableDeclaration(node)
    ? checker.getSignatureFromDeclaration(node)
    : undefined;
  const typeParameters = (node as { typeParameters?: ts.NodeArray<ts.TypeParameterDeclaration> })
    .typeParameters;
  const decorators = ts.canHaveDecorators(modifierNode)
    ? ts.getDecorators(modifierNode)?.map((item) => item.expression.getText())
    : undefined;
  return {
    ...(parentId ? { parentId } : {}),
    visibility: flags.has(ts.SyntaxKind.PrivateKeyword)
      ? "private"
      : flags.has(ts.SyntaxKind.ProtectedKeyword)
        ? "protected"
        : parentId
          ? "public"
          : "local",
    ...
    ...(typeParameters
      ? { typeParameters: [...typeParameters].map((item) => item.name.text) }
      : {}),
...
function isCommonJsExport(node: ts.Expression): boolean {
  ...
  return (
    text === "module.exports" || text.startsWith("exports.") || text.startsWith("module.exports.")
  );
}
...
function isJsxTag(node: ts.Node): node is ts.JsxOpeningElement | ts.JsxSelfClosingElement {
  return ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node);
}
function isJsxTagName(node: ts.Identifier): boolean {
  return (
    (isJsxTag(node.parent) && node.parent.tagName === node) ||
    (ts.isJsxClosingElement(node.parent) && node.parent.tagName === node)
  );
}
function isKnownFrameworkCall(node: ts.CallExpression | ts.NewExpression): boolean {
  const name = expressionName(node.expression);
  return [
    "describe",
    "suite",
...
function isCallbackRegistration(expression: ts.Expression): boolean {
  return ["on", "once", "addEventListener", "subscribe", "then", "catch", "finally"].includes(
    expressionName(expression),
  );
}
function isNonRepositoryCall(
  node: ts.CallExpression | ts.NewExpression,
  checker: ts.TypeChecker,
): boolean {
  if (
    ts.isPropertyAccessExpression(node.expression) &&
    NON_REPOSITORY_METHODS.has(node.expression.name.text)
  )
    return true;
  const symbol = symbolForExpression(node.expression, checker);
  ...
  if (
    rootSymbol?.declarations?.some(
      (declaration) =>
        declaration.getSourceFile().isDeclarationFile || isExternalImportDeclaration(declaration),
    ) ||
    (rootSymbol && isDerivedFromExternalImport(rootSymbol, checker))
  )
    return true;
  return false;
}
...
function isRouterReceiver(expression: ts.Expression): boolean {
  const root = rootIdentifier(expression);
  return Boolean(root && /^(?:app|api|router|routes|server)$/i.test(root.text));
}
...
function configurationKey(node: ts.CallExpression): string | undefined {
  ...
  if (
    ...
    if (
      ...
      parent.expression.name.text === "getConfiguration" &&
      parent.arguments[0] &&
      ts.isStringLiteralLike(parent.arguments[0])
    )
      return `${parent.arguments[0].text}.${node.arguments[0].text}`;
  }
  return undefined;
}
const NON_REPOSITORY_METHODS = new Set([
  "at",
  "catch",
  "charCodeAt",
  "concat",
