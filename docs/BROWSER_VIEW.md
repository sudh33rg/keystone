...
7. **Token Expiration**: The bootstrap token expires after 5 minutes
8. **Session Expiration**: The session expires after 30 minutes of inactivity
...
2. **Session Cookie**: An HttpOnly, SameSite=Strict cookie for authentication
   ...
   - Value: JWT token containing session ID and expiration
...
- **CORS**: Disables CORS to prevent cross-origin attacks
...
- **Token Expiration**: Bootstrap token expires after 5 minutes
...
- **Command Signing**: Commands can be signed for integrity
